...
public class ModuleHighlightUtil
{
	...
	{
		return Optional.ofNullable(module).map(m -> FilenameIndex.getVirtualFilesByName(module.getProject(), MODULE_INFO_FILE, m.getModuleScope(false))).map(c -> c.size() == 1 ? c.iterator().next()
				: null).map(f -> PsiManager.getInstance(module.getProject()).findFile(f)).map(f -> f instanceof PsiJavaFile ? ((PsiJavaFile) f).getModuleDeclaration() : null).orElse(null);
	}
	...
	static List<HighlightInfo> checkDuplicateStatements(@NotNull PsiJavaModule module)
	{
		...
		checkDuplicateRefs(module.getExports(), st -> Optional.ofNullable(st.getPackageReference()).map(ModuleHighlightUtil::refText), "module.duplicate.export", results);

		checkDuplicateRefs(psiTraverser().children(module).filter(PsiUsesStatement.class), st -> Optional.ofNullable(st.getClassReference()).map(ModuleHighlightUtil::refText), "module.duplicate" +
				".uses", results);

		checkDuplicateRefs(psiTraverser().children(module).filter(PsiProvidesStatement.class), st -> Optional.of(pair(st.getInterfaceReference(), st.getImplementationReference())).map(p -> p.first
				!= null && p.second != null ? refText(p.first) + " / " + refText(p.second) : null), "module.duplicate.provides", results);
