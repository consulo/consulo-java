...
public class NullableStuffInspectionBase extends BaseJavaBatchLocalInspectionTool
{
			...
			public void visitAnnotation(PsiAnnotation annotation)
			{
				...
				if(value instanceof PsiClassObjectAccessExpression)
				{
					...
					if(psiClass != null && !hasStringConstructor(psiClass))
					{
						...
						holder.registerProblem(value, "Custom exception class should have a constructor with a single message parameter of String type",
	...
	private static void checkNotNullFieldsInitialized(PsiField field, Annotated annotated, NullableNotNullManager manager, @NotNull ProblemsHolder holder)
	{
		if(annotated.isDeclaredNotNull && !HighlightControlFlowUtil.isFieldInitializedAfterObjectConstruction(field))
		{
			...
			if(annotation != null)
			{
				holder.registerProblem(annotation.isPhysical() ? annotation : field.getNameIdentifier(), "Not-null fields must be initialized", ProblemHighlightType.GENERIC_ERROR_OR_WARNING);
			}
		}
	}
	...
	private void checkConstructorParameters(PsiField field, Annotated annotated, NullableNotNullManager manager, String anno, List<String> annoToRemove, @NotNull ProblemsHolder holder)
	{
		...
		{
			holder.registerProblem(nameIdentifier, "@" + getPresentableAnnoName(field) + " field is always initialized not-null", ProblemHighlightType.GENERIC_ERROR_OR_WARNING,
					new AddNotNullAnnotationFix(field));
		}
	}
	...
	private static void reportPrimitiveType(final ProblemsHolder holder, final PsiElement psiElement, final PsiAnnotation annotation, final PsiModifierListOwner listOwner)
	{
		holder.registerProblem(psiElement.isPhysical() ? psiElement : listOwner.getNavigationElement(), InspectionsBundle.message("inspection.nullable.problems.primitive.type.annotation"),
	...
	private void checkNullableStuffForMethod(PsiMethod method, final ProblemsHolder holder)
	{
		...
		List<PsiMethod> superMethods = ContainerUtil.map(method.findSuperMethodSignaturesIncludingStatic(true), new Function<MethodSignatureBackedByPsiMethod, PsiMethod>()
		{
			@Override
		...
		if(REPORT_NOT_ANNOTATED_METHOD_OVERRIDES_NOTNULL)
		{
			for(PsiMethod superMethod : superMethods)
			{
				if(!nullableManager.hasNullability(method) && isNotNullNotInferred(superMethod, true, IGNORE_EXTERNAL_SUPER_NOTNULL))
				{
					...
					final LocalQuickFix fix = AnnotationUtil.isAnnotatingApplicable(method, defaultNotNull) ? createAnnotateMethodFix(defaultNotNull,
							annotationsToRemove) : createChangeDefaultNotNullFix(nullableManager, superMethod);
					holder.registerProblem(method.getNameIdentifier(), InspectionsBundle.message("inspection.nullable.problems.method.overrides.NotNull", getPresentableAnnoName(superMethod)),
							ProblemHighlightType.GENERIC_ERROR_OR_WARNING, wrapFix(fix));
					break;
				}
			}
		}
			...
			if(REPORT_NOT_ANNOTATED_METHOD_OVERRIDES_NOTNULL)
			{
				for(PsiParameter superParameter : superParameters)
				{
					if(!nullableManager.hasNullability(parameter) && isNotNullNotInferred(superParameter, false, IGNORE_EXTERNAL_SUPER_NOTNULL))
					{
						...
						holder.registerProblem(parameter.getNameIdentifier(), InspectionsBundle.message("inspection.nullable.problems.parameter.overrides.NotNull",
								getPresentableAnnoName(superParameter)), ProblemHighlightType.GENERIC_ERROR_OR_WARNING, wrapFix(fix));
						break;
					}
				}
			}
				...
				for(PsiParameter superParameter : superParameters)
				{
					if(!nullableManager.hasNullability(superParameter) && isNotNullNotInferred(parameter, false, false))
					{
						...
						holder.registerProblem(physical ? notNullAnnotation : parameter.getNameIdentifier(), InspectionsBundle.message("inspection.nullable.problems.NotNull.parameter.overrides.not" +
								".annotated", getPresentableAnnoName(parameter)), ProblemHighlightType.GENERIC_ERROR_OR_WARNING, wrapFix(fix));
						break;
					}
				}
			}
		...
		if(REPORT_ANNOTATION_NOT_PROPAGATED_TO_OVERRIDERS)
		{
			...
			if(hasAnnotatedParameter || annotated.isDeclaredNotNull)
			{
				...
				for(PsiMethod overriding : overridings)
				{
					...
					if(!methodQuickFixSuggested && annotated.isDeclaredNotNull && !isNotNullNotInferred(overriding, false, false) && (isNullableNotInferred(overriding,
							false) || !isNullableNotInferred(overriding, true)))
					{
					...
					if(hasAnnotatedParameter)
					{
						...
						for(int i = 0; i < psiParameters.length; i++)
						{
							...
							if(parameterAnnotated[i] && !isNotNullNotInferred(parameter, false, false) && !isNullableNotInferred(parameter, false))
							{
								...
								holder.registerProblem(psiElement, InspectionsBundle.message("nullable.stuff.problems.overridden.method.parameters.are.not.annotated"),
										ProblemHighlightType.GENERIC_ERROR_OR_WARNING, wrapFix(!applicable ? createChangeDefaultNotNullFix(nullableManager,
										parameters[i]) : new AnnotateOverriddenMethodParameterFix(defaultNotNull, nullableManager.getDefaultNullable())));
	...
	{
		final String bothNullableNotNullMessage = InspectionsBundle.message("inspection.nullable.problems.Nullable.NotNull.conflict", getPresentableAnnoName(declaredNullable),
				getPresentableAnnoName(declaredNotNull));
		holder.registerProblem(declaredNotNull.isPhysical() ? declaredNotNull : listOwner.getNavigationElement(), bothNullableNotNullMessage, ProblemHighlightType.GENERIC_ERROR_OR_WARNING,
				new RemoveAnnotationQuickFix(declaredNotNull, listOwner));
		holder.registerProblem(declaredNullable.isPhysical() ? declaredNullable : listOwner.getNavigationElement(), bothNullableNotNullMessage, ProblemHighlightType.GENERIC_ERROR_OR_WARNING,
				new RemoveAnnotationQuickFix(declaredNullable, listOwner));
	}
