...
public class ControlFlowAnalyzer extends JavaElementVisitor
{
	...
	private static Condition<String> parseFalseGetters()
	{
		try
		{
			final Pattern pattern = Pattern.compile("java\\.nio\\..*ByteBuffer\\.get.*");
	...
	/**
	 * A mock variable for try-related control transfers. Contains exceptions or an (Throwable-inconvertible) string to indicate return inside
	 * finally
	 */
					...
					else
					{
						try
						{
							...
							if(caseValue != null &&
									...
									JavaPsiFacade.getInstance(body.getProject()).getConstantEvaluationHelper().computeConstantExpression(caseValue)
											!= null)
							{
	...
	private static class ApplyNotNullInstruction extends Instruction
	{
		...
		public DfaInstructionState[] accept(DataFlowRunner runner, DfaMemoryState state, InstructionVisitor visitor)
		{
			...
			if(state.applyCondition(factory.getRelationFactory().createRelation(value, factory.getConstFactory().getNull(), JavaTokenType.EQEQ,
					true)))
			{
	...
	public void visitCatchSection(PsiCatchSection section)
	{
		...
		if(catchBlock != null)
		{
			...
			List<PsiType> flattened = declaredType instanceof PsiDisjunctionType ? ((PsiDisjunctionType) declaredType).getDisjunctions() :
	...
	private void acceptBinaryRightOperand(@Nullable IElementType op, PsiType type, PsiExpression lExpr, PsiType lType, PsiExpression rExpr,
			PsiType rType)
	{
	...
	public void visitConditionalExpression(PsiConditionalExpression expression)
	{
		...
		final ControlFlow.ControlFlowOffset elseOffset = elseExpression == null ? ControlFlow.deltaOffset(getEndOffset(expression),
				-1) : getStartOffset(elseExpression);
	...
	public void visitMethodCallExpression(PsiMethodCallExpression expression)
	{
		...
		addConditionalRuntimeThrow();
		List<MethodContract> contracts = method instanceof PsiMethod ? getMethodContracts((PsiMethod) method) : Collections
				.<MethodContract>emptyList();
	...
	static List<MethodContract> getMethodContracts(@NotNull final PsiMethod method)
	{
		...
		if(owner != null)
		{
			...
			if("java.lang.System".equals(className))
			{
				...
				{
					return Collections.singletonList(new MethodContract(MethodContract.createConstraintArray(paramCount),
							ValueConstraint.THROW_EXCEPTION));
				}
			}
			else if("junit.framework.Assert".equals(className) || "org.junit.Assert".equals(className) ||
					"junit.framework.TestCase".equals(className) || "org.testng.Assert".equals(className) || "org.testng.AssertJUnit".equals
					(className))
			{
