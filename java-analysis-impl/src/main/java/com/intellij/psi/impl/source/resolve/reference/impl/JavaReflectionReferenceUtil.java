...
public final class JavaReflectionReferenceUtil
{
	...
	static boolean isAtomicallyUpdateable(@Nonnull PsiField field)
	{
	...
	@Nullable
	static LookupElement lookupMethod(@Nonnull PsiMethod method, @Nullable InsertHandler<LookupElement> insertHandler)
	{
	...
	public static List<PsiExpression> getVarargs(@Nullable PsiExpression maybeArray)
	{
		...
		if(isVarargAsArray(maybeArray))
		{
			...
			if(argumentsDefinition instanceof PsiNewExpression)
			{
				...
				{ // new Object[length] or new Class<?>[length]
