...
class MethodReturnInferenceVisitor
{
	...
	private ReturnValue findVariableValue(LighterASTNode expr, LighterASTNode target)
	{
		...
		while(true)
		{
			...
			if(!ElementType.EXPRESSION_BIT_SET.contains(type))
			{
				break;
			}
	...
	private ReturnValue findValueBeforeStatement(LighterASTNode statement, LighterASTNode target)
	{
		...
		if(parent.getTokenType() == CODE_BLOCK)
		{
			...
			if(grandParent == null || grandParent.getTokenType() != BLOCK_STATEMENT)
			{
				return ReturnValue.UNKNOWN;
			}
	...
	private ReturnValue findValueInStatement(LighterASTNode statement, LighterASTNode target)
	{
		...
		if(tokenType == DECLARATION_STATEMENT)
		{
			...
			for(int i = declaredElements.size() - 1; i >= 0; i--)
			{
				...
				if(declared.getTokenType() != LOCAL_VARIABLE)
				{
					continue;
				}
		...
		if(isAssignedInside(statement, target))
		{
			return ReturnValue.UNKNOWN;
		}
		return null;
	...
	private boolean isAssignedInside(LighterASTNode element, LighterASTNode target)
	{
		...
		while(!workList.isEmpty())
		{
			...
			if(isReferenceToLocal(node, target))
			{
				...
				{
					IElementType type = parent.getTokenType();
					if(type == ASSIGNMENT_EXPRESSION && node.equals(findExpressionChild(tree, parent)))
					{
						return true;
					}
					if(type == POSTFIX_EXPRESSION || type == PREFIX_EXPRESSION)
					{
						return true;
					}
				}
	...
	private boolean isDereferencedInside(LighterASTNode expression, LighterASTNode target)
	{
		...
		while(!workList.isEmpty())
		{
			...
			if(isReferenceToLocal(node, target))
			{
				...
				{
					IElementType type = parent.getTokenType();
					if(type == REFERENCE_EXPRESSION || type == ARRAY_ACCESS_EXPRESSION)
					{
						return true;
					}
				}
	...
	{
		expr = skipParenthesesCastsDown(tree, expr);
		if(expr == null || isAssignedInside(expr, var))
		{
			return false;
		}
		...
		if(type == BINARY_EXPRESSION || type == POLYADIC_EXPRESSION)
		{
			...
			if(firstChildOfType(tree, expr, negated ? JavaTokenType.OROR : JavaTokenType.ANDAND) == null)
			{
				return false;
			}
			for(LighterASTNode t : operands)
			{
				if(isNullCheck(t, var, negated))
				{
					return true;
				}
			}
	...
	private boolean isReferenceToLocal(@Nullable LighterASTNode operand, LighterASTNode var)
	{
	...
	private boolean isNullLiteral(LighterASTNode value)
	{
	...
	private static class ReturnValue
	{
		...
		private ReturnValue(EnumSet<Nullability> nullability,
							@Nullable String method,
							List<ExpressionRange> ranges)
		{
		...
		{
			if(!myNullability.contains(Nullability.NULLABLE))
			{
				return this;
			}
		...
		static ReturnValue delegate(String method, ExpressionRange range)
		{
			return new ReturnValue(EnumSet.noneOf(Nullability.class), method, Collections.singletonList(range));
		}

		static ReturnValue merge(ReturnValue left, ReturnValue right)
		{
