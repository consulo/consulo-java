...
public class HighlightUtil extends HighlightUtilBase {
    ...
    static {
        ...
        ourInterfaceIncompatibleModifiers.put(
            PsiModifier.PACKAGE_LOCAL,
            Set.of(PsiModifier.PRIVATE, PsiModifier.PUBLIC, PsiModifier.PROTECTED)
        );
        ourInterfaceIncompatibleModifiers.put(
        ...
        ourMethodIncompatibleModifiers.put(PsiModifier.DEFAULT, Set.of(PsiModifier.ABSTRACT, PsiModifier.STATIC, PsiModifier.PRIVATE));
    ...
    public static void registerAccessQuickFixAction(
        ...
        registerReplaceInaccessibleFieldWithGetterSetterFix(refElement, place, accessObjectClass, hlBuilder);
    ...
    @Nullable
    @RequiredReadAction
    private static PsiClass getPackageLocalClassInTheMiddle(PsiElement place) {
        if (place instanceof PsiReferenceExpression refExpr) {
            ...
            while (true) {
                ...
                if (!(refExpr.getQualifierExpression() instanceof PsiReferenceExpression qualifierRefExpr)) {
                    break;
                }
    ...
    @RequiredReadAction
    public static HighlightInfo.@Nullable Builder checkInstanceOfApplicable(PsiInstanceOfExpression expression) {
        ...
        if (TypeConversionUtil.isPrimitiveAndNotNull(operandType)
            ...
            return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                .range(expression)
                .descriptionAndTooltip(JavaCompilationErrorLocalize.castInconvertible(
     ...
     * - No two listed types may be subtypes of different parametrization of the same generic interface.
     */
    @RequiredReadAction
    public static HighlightInfo.@Nullable Builder checkIntersectionInTypeCast(
        ...
        if (castTypeElement != null && isIntersection(castTypeElement, castTypeElement.getType())) {
            HighlightInfo.Builder hlBuilder = checkFeature(expression, JavaFeature.INTERSECTION_CASTS, languageLevel, file);
            if (hlBuilder != null) {
                return hlBuilder;
            }
            ...
            if (conjuncts != null) {
                    ...
                    if (conjType instanceof PsiClassType classType) {
                        ...
                        if (aClass != null && !aClass.isInterface()) {
                            return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                                ...
                                .registerFix(new FlipIntersectionSidesFix(aClass.getName(), conjList, conjunct, castTypeElement));
                        }
                    }
                    else {
                        return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                            .range(conjunct)
                            .descriptionAndTooltip(JavaCompilationErrorLocalize.castIntersectionUnexpectedType());
                    }
                    if (!erasures.add(TypeConversionUtil.erasure(conjType))) {
                        return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                            .range(conjunct)
                            .descriptionAndTooltip(JavaCompilationErrorLocalize.castIntersectionRepeatedInterface())
                            .registerFix(new DeleteRepeatedInterfaceFix(conjunct, conjList));
                    }
                }
                ...
                if (sameGenericParameterization != null) {
                    return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                        .range(expression)
                        .descriptionAndTooltip(JavaErrorLocalize.classCannotBeInheritedWithDifferentArguments(
                            formatClass(sameGenericParameterization),
    ...
    @Nullable
    @RequiredReadAction
    public static HighlightInfo checkInconvertibleTypeCast(PsiTypeCastExpression expression) {
        PsiTypeElement castTypeElement = expression.getCastType();
    ...
    @Nullable
    @RequiredReadAction
    public static HighlightInfo checkVariableExpected(PsiExpression expression) {
        ...
        if (lValue != null && !TypeConversionUtil.isLValue(lValue)) {
            errorResult = HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                .range(lValue)
                .descriptionAndTooltip(JavaCompilationErrorLocalize.lvalueVariableExpected())
                .create();
        }
    ...
    @Nullable
    @RequiredReadAction
    public static HighlightInfo checkAssignmentOperatorApplicable(PsiAssignmentExpression assignment) {
        ...
        if (!TypeConversionUtil.isBinaryOperatorApplicable(opSign, lType, rType, true)) {
            ...
            LocalizeValue message = JavaCompilationErrorLocalize.binaryOperatorNotApplicable(
                operatorText,
    ...
    @RequiredReadAction
    public static HighlightInfo.@Nullable Builder checkAssignmentCompatibleTypes(PsiAssignmentExpression assignment) {
        ...
        if (JavaTokenType.EQ.equals(sign)) {
            hlBuilder = checkAssignability(lType, rType, rExpr, assignment);
        }
        else {
            ...
            hlBuilder = createIncompatibleTypeHighlightInfo(lType, type, assignment.getTextRange(), 0);
            hlBuilder.registerFix(QuickFixFactory.getInstance().createChangeToAppendFix(sign, lType, assignment));
        }
        if (hlBuilder == null) {
            return null;
        }
        registerChangeVariableTypeFixes(lExpr, rType, rExpr, hlBuilder);
        if (lType != null) {
            registerChangeVariableTypeFixes(rExpr, lType, lExpr, hlBuilder);
        }
        return hlBuilder;
    }
    ...
    private static void registerChangeVariableTypeFixes(
        ...
        if (lExpr instanceof PsiMethodCallExpression methodCall
            ...
            if (method != null && PsiType.VOID.equals(method.getReturnType())) {
                highlightInfo.registerFix(new ReplaceAssignmentFromVoidWithStatementIntentionAction(assignment, lExpr));
            }
        }
    }

    private static boolean isCastIntentionApplicable(PsiExpression expression, @Nullable PsiType toType) {
    ...
    @RequiredReadAction
    public static HighlightInfo.@Nullable Builder checkVariableInitializerType(PsiVariable variable) {
        ...
        HighlightInfo.Builder hlBuilder = checkAssignability(lType, rType, initializer, new TextRange(start, end), 0);
        if (hlBuilder != null) {
            registerChangeVariableTypeFixes(variable, rType, variable.getInitializer(), hlBuilder);
            registerChangeVariableTypeFixes(initializer, lType, null, hlBuilder);
        }
        return hlBuilder;
    }

    @RequiredReadAction
    public static HighlightInfo.@Nullable Builder checkAssignability(
    ...
    @RequiredReadAction
    private static HighlightInfo.@Nullable Builder checkAssignability(
        @Nullable PsiType lType,
        @Nullable PsiType rType,
        @Nullable PsiExpression expression,
        TextRange textRange,
        ...
        HighlightInfo.Builder hlBuilder = createIncompatibleTypeHighlightInfo(lType, rType, textRange, navigationShift);
        ...
        if (rType != null && expression != null && isCastIntentionApplicable(expression, lType)) {
            hlBuilder.registerFix(factory.createAddTypeCastFix(lType, expression));
        }
        if (expression != null) {
            hlBuilder.registerFix(factory.createWrapWithAdapterFix(lType, expression))
                .registerFix(factory.createWrapWithOptionalFix(lType, expression))
                .registerFix(factory.createWrapExpressionFix(lType, expression))
                .registerFix(factory.createWrapStringWithFileFix(lType, expression));
            AddTypeArgumentsConditionalFix.register(hlBuilder, expression, lType);
            registerCollectionToArrayFixAction(hlBuilder, rType, lType, expression);
        }
        ChangeNewOperatorTypeFix.register(hlBuilder, expression, lType);
        return hlBuilder;
    }

    @RequiredReadAction
    public static HighlightInfo.@Nullable Builder checkReturnStatementType(PsiReturnStatement statement) {
        ...
        if (method == null && lambda != null) {
            ...
            return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                .range(statement)
                .descriptionAndTooltip(JavaCompilationErrorLocalize.returnOutsideMethod());
        }
        else {
            ...
            if (returnValue != null) {
                ...
                if (isMethodVoid) {
                    HighlightInfo.Builder errorResult = HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                        .range(statement)
                        .descriptionAndTooltip(JavaCompilationErrorLocalize.returnFromVoidMethod());
                    if (valueType != null) {
                        errorResult.registerFix(QuickFixFactory.getInstance().createMethodReturnFix(method, valueType, true));
                    }
                    return errorResult;
                }
                else {
                    ...
                    HighlightInfo.Builder hlBuilder =
                        checkAssignability(returnType, valueType, returnValue, textRange, returnValue.getStartOffsetInParent());
                    if (hlBuilder != null && valueType != null) {
                        if (!PsiType.VOID.equals(valueType)) {
                            hlBuilder.registerFix(QuickFixFactory.getInstance().createMethodReturnFix(method, valueType, true));
                        }
                        registerChangeParameterClassFix(returnType, valueType, hlBuilder);
                        if (returnType instanceof PsiArrayType arrayType) {
                            ...
                            if (erasedValueType != null
                                ...
                                hlBuilder.registerFix(QuickFixFactory.getInstance().createSurroundWithArrayFix(null, returnValue));
                            }
                        }
                        registerCollectionToArrayFixAction(hlBuilder, valueType, returnType, returnValue);
                        return hlBuilder;
                    }
                }
            }
                ...
                return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                    .range(statement)
                    .descriptionAndTooltip(JavaCompilationErrorLocalize.returnValueMissing())
                    .navigationShift(PsiKeyword.RETURN.length())
                    .registerFix(QuickFixFactory.getInstance().createMethodReturnFix(method, PsiType.VOID, true));
            }
        }
        return null;
    }
    ...
    private static void registerCollectionToArrayFixAction(
        ...
        if (hlBuilder != null && toType instanceof PsiArrayType arrayType) {
    ...
    public static LocalizeValue getUnhandledExceptionsDescriptor(Collection<PsiClassType> unhandled) {
        return getUnhandledExceptionsDescriptor(unhandled, null);
    }

    private static LocalizeValue getUnhandledExceptionsDescriptor(Collection<PsiClassType> unhandled, @Nullable String source) {
        String exceptions = formatTypes(unhandled);
        return source != null
            ? JavaCompilationErrorLocalize.exceptionUnhandledClose(exceptions, unhandled.size())
            : JavaCompilationErrorLocalize.exceptionUnhandled(exceptions, unhandled.size());
    }
    ...
    @Nullable
    @RequiredReadAction
    public static HighlightInfo checkVariableAlreadyDefined(PsiVariable variable) {
        ...
        if (oldVariable != null) {
            ...
            HighlightInfo.Builder highlightInfo = HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                .range(identifier)
                .descriptionAndTooltip(JavaCompilationErrorLocalize.variableAlreadyDefined(variable.getName()));
            if (variable instanceof PsiLocalVariable localVar) {
                highlightInfo.registerFix(QuickFixFactory.getInstance().createReuseVariableDeclarationFix(localVar));
            }
    ...
    @RequiredReadAction
    public static HighlightInfo.@Nullable Builder checkUnhandledExceptions(PsiElement element, @Nullable TextRange textRange) {
        ...
        HighlightInfo.Builder hlBuilder = HighlightInfo.newHighlightInfo(highlightType)
            .range(textRange)
            .descriptionAndTooltip(getUnhandledExceptionsDescriptor(unhandledExceptions));
        return registerUnhandledExceptionFixes(element, hlBuilder, unhandledExceptions);
    }

    @Nullable
    @RequiredReadAction
    public static HighlightInfo checkUnhandledCloserExceptions(PsiResourceListElement resource) {
        ...
        HighlightInfo.Builder highlight = HighlightInfo.newHighlightInfo(highlightType)
            .range(resource)
            .descriptionAndTooltip(getUnhandledExceptionsDescriptor(unhandled, "auto-closeable resource"));
        return registerUnhandledExceptionFixes(resource, highlight, unhandled).create();
    }
    ...
    private static HighlightInfo.Builder registerUnhandledExceptionFixes(
        ...
        QuickFixFactory factory = QuickFixFactory.getInstance();
        hlBuilder.registerFix(factory.createAddExceptionToCatchFix())
            .registerFix(factory.createAddExceptionToThrowsFix(element))
            .registerFix(factory.createAddExceptionFromFieldInitializerToConstructorThrowsFix(element))
            .registerFix(factory.createSurroundWithTryCatchFix(element));
        if (unhandled.size() == 1) {
            hlBuilder.registerFix(factory.createGeneralizeCatchFix(element, unhandled.get(0)));
        }
        return hlBuilder;
    }
    ...
    @Nullable
    @RequiredReadAction
    public static HighlightInfo checkBreakOutsideLoop(PsiBreakStatement statement) {
        if (statement.getLabelIdentifier() == null) {
            if (new PsiMatcherImpl(statement).ancestor(EnclosingLoopOrSwitchMatcherExpression.INSTANCE).getElement() == null) {
                return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                    .range(statement)
                    .descriptionAndTooltip(JavaCompilationErrorLocalize.breakOutsideSwitchOrLoop())
                    .create();
            }
        }
        else {
    ...
    @Nullable
    @RequiredReadAction
    public static HighlightInfo checkContinueOutsideLoop(PsiContinueStatement statement) {
        if (statement.getLabelIdentifier() == null) {
            if (new PsiMatcherImpl(statement).ancestor(EnclosingLoopMatcherExpression.INSTANCE).getElement() == null) {
                return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                    .range(statement)
                    .descriptionAndTooltip(JavaCompilationErrorLocalize.continueOutsideLoop())
                    .create();
            }
        }
        else {
    ...
    @Nullable
    @RequiredReadAction
    public static HighlightInfo checkIllegalModifierCombination(PsiKeyword keyword, PsiModifierList modifierList) {
        ...
        if (incompatible != null) {
            return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                .range(keyword)
                .descriptionAndTooltip(JavaCompilationErrorLocalize.modifierIncompatible(modifier, incompatible))
    ...
    public static HighlightInfo checkNotAllowedModifier(PsiKeyword keyword, PsiModifierList modifierList) {
        ...
        if (modifierOwner instanceof PsiClass aClass) {
            ...
            if (PsiModifier.PRIVATE.equals(modifier)) {
                isAllowed &= modifierOwnerParent instanceof PsiClass modifierClass && (!modifierClass.isInterface()
                    || PsiUtil.isLanguageLevel9OrHigher(method) && !modifierClass.isAnnotationType());
            }
            else if (PsiModifier.STRICTFP.equals(modifier)) {
                isAllowed &= modifierOwnerParent instanceof PsiClass modifierClass
                    && (!modifierClass.isInterface() || PsiUtil.isLanguageLevel8OrHigher(method));
            }
        ...
        if (!isAllowed) {
            return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                .range(keyword)
                .descriptionAndTooltip(JavaCompilationErrorLocalize.modifierNotAllowed(modifier))
                .registerFix(QuickFixFactory.getInstance().createModifierFixBuilder(modifierList).remove(modifier).create())
                .create();
        }
    ...
    @RequiredReadAction
    public static HighlightInfo.@Nullable Builder checkLiteralExpressionParsingError(
        ...
        if (file != null) {
            if (isFP) {
                if (text.startsWith(PsiLiteralUtil.HEX_PREFIX)) {
                    HighlightInfo.Builder hlBuilder = checkFeature(expression, JavaFeature.HEX_FP_LITERALS, level, file);
                    if (hlBuilder != null) {
                        return hlBuilder;
                    }
                }
            ...
            if (isInt) {
                if (text.startsWith(PsiLiteralUtil.BIN_PREFIX)) {
                    HighlightInfo.Builder hlBuilder = checkFeature(expression, JavaFeature.BIN_LITERALS, level, file);
                    if (hlBuilder != null) {
                        return hlBuilder;
                    }
                }
            ...
            if ((isInt || isFP) && text.contains("_")) {
                HighlightInfo.Builder hlBuilder = checkFeature(expression, JavaFeature.UNDERSCORES, level, file);
                if (hlBuilder == null) {
                    hlBuilder = checkUnderscores(expression, text, isInt);
                }
                if (hlBuilder != null) {
                    return hlBuilder;
                }
        ...
        if (type == JavaTokenType.INTEGER_LITERAL) {
            ...
            if (!(cleanText.equals(PsiLiteralUtil._2_IN_31)
                ...
                if (cleanText.equals(PsiLiteralUtil.HEX_PREFIX)) {
                    return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                        .range(expression)
                        .descriptionAndTooltip(JavaCompilationErrorLocalize.literalHexadecimalNoDigits());
                }
                if (cleanText.equals(PsiLiteralUtil.BIN_PREFIX)) {
                    return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                        .range(expression)
                        .descriptionAndTooltip(JavaCompilationErrorLocalize.literalBinaryNoDigits());
                }
                if (value == null || cleanText.equals(PsiLiteralUtil._2_IN_31)) {
                    return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                        .range(expression)
                        .descriptionAndTooltip(JavaCompilationErrorLocalize.literalIntegerTooLarge());
                }
            }
        }
            ...
            if (!(cleanText.equals(PsiLiteralUtil._2_IN_63)
                ...
                if (value == null || cleanText.equals(PsiLiteralUtil._2_IN_63)) {
                    return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                        .range(expression)
                        .descriptionAndTooltip(JavaCompilationErrorLocalize.literalLongTooLarge());
                }
            }
        }
        else if (isFP) {
            if (value == null) {
                return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                    .range(expression)
                    .descriptionAndTooltip(JavaCompilationErrorLocalize.literalFloatingMalformed());
            }
        }
            ...
            if (value == null) {
                ...
                if (!StringUtil.endsWithChar(text, '\'') || text.length() == 1) {
                    return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                        .range(expression)
                        .descriptionAndTooltip(JavaCompilationErrorLocalize.literalCharacterUnclosed());
                }
                ...
                if (chars == null) {
                    return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                        .range(expression)
                        .descriptionAndTooltip(JavaCompilationErrorLocalize.literalCharacterIllegalEscape());
                }
                int length = chars.length();
                if (length > 1) {
                    return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                        .range(expression)
                        .descriptionAndTooltip(JavaCompilationErrorLocalize.literalCharacterTooLong())
                        .registerFix(QuickFixFactory.getInstance().createConvertToStringLiteralAction());
                }
                else if (length == 0) {
                    return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                        .range(expression)
                        .descriptionAndTooltip(JavaCompilationErrorLocalize.literalCharacterEmpty());
                }
            }
        }
            ...
            if (type == JavaTokenType.STRING_LITERAL) {
                if (value == null) {
                    ...
                    if (CodeInsightUtilCore.parseStringCharacters(text, null) == null) {
                        return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                            .range(expression)
                            .descriptionAndTooltip(JavaCompilationErrorLocalize.literalStringIllegalEscape());
                    }
                }
            }
            else if (value == null) {
                if (!text.endsWith("\"\"\"")) {
                    ...
                    return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                        .range(p, p)
                        .endOfLine()
                        .descriptionAndTooltip(JavaCompilationErrorLocalize.literalTextBlockUnclosed());
                }
                else {
                    ...
                    if (!success) {
                        ...
                        return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                            .range(expression)
                            .descriptionAndTooltip(JavaCompilationErrorLocalize.literalTextBlockNoNewLine());
                    }
                }
            }
            else {
                if (file != null && containsUnescaped(text, "\\\n")) {
                    HighlightInfo.Builder hlBuilder = checkFeature(expression, JavaFeature.TEXT_BLOCK_ESCAPES, level, file);
                    if (hlBuilder != null) {
                        return hlBuilder;
                    }
                }
        ...
        if (value instanceof Float number) {
            if (number.isInfinite()) {
                return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                    .range(expression)
                    .descriptionAndTooltip(JavaCompilationErrorLocalize.literalFloatingTooLarge());
            }
            if (number == 0 && !TypeConversionUtil.isFPZero(text)) {
                return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                    .range(expression)
                    .descriptionAndTooltip(JavaCompilationErrorLocalize.literalFloatingTooSmall());
            }
        }
    ...
    private static boolean containsUnescaped(String text, String subText) {
        int start = 0;
    ...
    @RequiredReadAction
    private static HighlightInfo.Builder checkUnderscores(PsiElement expression, String text, boolean isInt) {
        ...
        for (String part : parts) {
            if (part != null && (StringUtil.startsWithChar(part, '_') || StringUtil.endsWithChar(part, '_'))) {
                return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                    .range(expression)
                    .descriptionAndTooltip(JavaCompilationErrorLocalize.literalIllegalUnderscore());
            }
        }
    ...
    @RequiredReadAction
    public static HighlightInfo.@Nullable Builder checkMustBeBoolean(PsiExpression expr, PsiType type) {
        ...
        if (parent instanceof PsiIfStatement || parent instanceof PsiWhileStatement
            ...
            if (!TypeConversionUtil.isBooleanType(type)) {
                HighlightInfo.Builder hlBuilder = createIncompatibleTypeHighlightInfo(PsiType.BOOLEAN, type, expr.getTextRange(), 0);
                if (expr instanceof PsiMethodCallExpression methodCall) {
                    PsiMethod method = methodCall.resolveMethod();
                    if (method != null && PsiType.VOID.equals(method.getReturnType())) {
                        hlBuilder.registerFix(QuickFixFactory.getInstance().createMethodReturnFix(method, PsiType.BOOLEAN, true));
                    }
                }
                else if (expr instanceof PsiAssignmentExpression assignment && assignment.getOperationTokenType() == JavaTokenType.EQ) {
                    hlBuilder.registerFix(QuickFixFactory.getInstance().createAssignmentToComparisonFix(assignment));
                }
                return hlBuilder;
            }
        }
    ...
    @RequiredReadAction
    public static Set<PsiClassType> collectUnhandledExceptions(PsiTryStatement statement) {
    ...
    @RequiredReadAction
    public static List<HighlightInfo> checkExceptionThrownInTry(PsiParameter parameter, Set<PsiClassType> thrownTypes) {
    ...
    private static HighlightInfo checkSimpleCatchParameter(
        ...
        Collection<PsiClassType> thrownTypes,
        PsiClassType caughtType
    ) {
        ...
        return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
            .range(parameter)
            .descriptionAndTooltip(JavaCompilationErrorLocalize.exceptionNeverThrownTry(JavaHighlightUtil.formatType(caughtType)))
            .registerFix(QuickFixFactory.getInstance().createDeleteCatchFix(parameter))
            .create();
    }
        ...
        for (PsiTypeElement typeElement : typeElements) {
            ...
            if (!used) {
                HighlightInfo highlight = HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                    .range(typeElement)
                    .descriptionAndTooltip(JavaCompilationErrorLocalize.exceptionNeverThrownTry(JavaHighlightUtil.formatType(catchType)))
                    .registerFix(QuickFixFactory.getInstance().createDeleteMultiCatchFix(typeElement))
                    .create();
    ...
    public static Collection<HighlightInfo> checkWithImprovedCatchAnalysis(
        PsiParameter parameter,
        Collection<PsiClassType> thrownInTryStatement,
                ...
                HighlightInfo highlightInfo = HighlightInfo.newHighlightInfo(HighlightInfoType.WARNING)
                    ...
                    .registerFix(
                        isMultiCatch
                            ...
                            : QuickFixFactory.getInstance().createDeleteCatchFix(parameter)
                    )
                    .create();
    ...
    @Nullable
    @RequiredReadAction
    public static HighlightInfo checkNotAStatement(PsiStatement statement) {
        if (!PsiUtil.isStatement(statement) && !PsiUtilCore.hasErrorElementChild(statement)) {
            ...
            LocalizeValue description = isDeclarationNotAllowed
                ? JavaCompilationErrorLocalize.statementDeclarationNotAllowed()
                : JavaCompilationErrorLocalize.statementBadExpression();
            HighlightInfo.Builder error = HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                .range(statement)
    ...
    @RequiredReadAction
    public static HighlightInfo.@Nullable Builder checkSwitchBlockStatements(
        ...
        if (body != null) {
            ...
            while (element != null && !PsiUtil.isJavaToken(element, JavaTokenType.RBRACE)) {
                if (element instanceof PsiSwitchLabeledRuleStatement) {
                    if (!levelChecked) {
                        HighlightInfo.Builder hlBuilder = checkFeature(element, JavaFeature.ENHANCED_SWITCH, languageLevel, file);
                        if (hlBuilder != null) {
                            return hlBuilder;
                        }
                        levelChecked = true;
                ...
                if (!levelChecked && element instanceof PsiSwitchLabelStatementBase switchLabelStatementBase) {
                    ...
                    if (values != null && values.getExpressionCount() > 1) {
                        HighlightInfo.Builder hlBuilder = checkFeature(values, JavaFeature.ENHANCED_SWITCH, languageLevel, file);
                        if (hlBuilder != null) {
                            return hlBuilder;
                        }
                        levelChecked = true;
            ...
            if (alien != null) {
                if (enhancedLabels && !(alien instanceof PsiSwitchLabelStatementBase)) {
                    ...
                    if (previousRule != null) {
                        hlBuilder.registerFix(QuickFixFactory.getInstance().createWrapSwitchRuleStatementsIntoBlockFix(previousRule));
                    }
                    return hlBuilder;
                }
                return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                    .range(alien)
                    .descriptionAndTooltip(JavaErrorLocalize.differentCaseKindsInSwitch());
            }
        }
    ...
    @Nullable
    @RequiredReadAction
    public static HighlightInfo checkSwitchSelectorType(PsiSwitchBlock switchBlock, LanguageLevel level) {
        ...
        if (kind == null || requiredLevel != null && !level.isAtLeast(requiredLevel)) {
            ...
            HighlightInfo info = HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                .range(expression)
                .descriptionAndTooltip(JavaCompilationErrorLocalize.typeIncompatible(expected, JavaHighlightUtil.formatType(type)))
                .create();
    ...
    private static SelectorKind getSwitchSelectorKind(PsiType type) {
    ...
    @Nullable
    @RequiredReadAction
    public static HighlightInfo checkPolyadicOperatorApplicable(PsiPolyadicExpression expression) {
    ...
    public static HighlightInfo checkUnaryOperatorApplicable(@Nullable PsiJavaToken token, @Nullable PsiExpression expression) {
        if (token != null && expression != null && !TypeConversionUtil.isUnaryOperatorApplicable(token, expression)) {
            ...
            HighlightInfo highlightInfo = HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                .range(parentExpr)
                .descriptionAndTooltip(JavaCompilationErrorLocalize.unaryOperatorNotApplicable(token.getText(), JavaHighlightUtil.formatType(type)))
                .create();
    ...
    public static HighlightInfo checkThisOrSuperExpressionInIllegalContext(
        ...
        if (expr instanceof PsiSuperExpression && !(expr.getParent() instanceof PsiReferenceExpression)) {
            ...
            return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                .range(o, o + 1)
                .descriptionAndTooltip(JavaCompilationErrorLocalize.expressionSuperDotExpected())
                .create();
        }
        ...
        if (qualifier != null) {
            ...
            if (resolved instanceof PsiClass resolvedClass) {
                ...
                return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                    .range(qualifier)
                    .descriptionAndTooltip(JavaCompilationErrorLocalize.expressionQualifiedClassExpected())
                    .create();
            }
        }
        else {
            ...
            if (aClass instanceof PsiAnonymousClass anonymousClass
                && PsiTreeUtil.isAncestor(anonymousClass.getArgumentList(), expr, false)) {
        ...
        if (qualifier != null && aClass.isInterface() && expr instanceof PsiSuperExpression
            ...
            if (classT != null) {
                ...
                if (!classT.isInheritor(aClass, false)) {
                    return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                        .range(qualifier)
                        .descriptionAndTooltip(JavaCompilationErrorLocalize.expressionSuperNoEnclosingInstance(format(aClass)))
                        .create();
                }
            }
        }
    ...
    public static HighlightInfo checkUnqualifiedSuperInDefaultMethod(
        ...
        if (languageLevel.isAtLeast(LanguageLevel.JDK_1_8) && qualifier instanceof PsiSuperExpression superExpr) {
            ...
            if (method != null && method.hasModifierProperty(PsiModifier.DEFAULT) && superExpr.getQualifier() == null) {
                HighlightInfo.Builder hlBuilder = HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                    .range(expr)
                    .descriptionAndTooltip(JavaCompilationErrorLocalize.expressionSuperUnqualifiedDefaultMethod());
                QualifySuperArgumentFix.registerQuickFixAction(superExpr, hlBuilder);
    ...
    private static boolean resolvesToImmediateSuperInterface(
        ...
        return expr instanceof PsiSuperExpression
            && qualifier != null
            && languageLevel.isAtLeast(LanguageLevel.JDK_1_8)
            && expr.getType() instanceof PsiClassType superClassType
            && aClass.equals(superClassType.resolve())
            && PsiUtil.getEnclosingStaticElement(expr, PsiTreeUtil.getParentOfType(expr, PsiClass.class)) == null;
    }

    @RequiredReadAction
    public static LocalizeValue buildProblemWithStaticDescription(PsiElement refElement) {
        ...
        return JavaCompilationErrorLocalize.referenceNonStaticFromStaticContext(type, name);
    }
    ...
    @RequiredReadAction
    public static LocalizeValue buildProblemWithAccessDescription(PsiElement reference, JavaResolveResult result) {
    ...
    @RequiredReadAction
    public static HighlightInfo.@Nullable Builder checkValidArrayAccessExpression(PsiArrayAccessExpression arrayAccessExpression) {
        ...
        if (arrayExpressionType != null && !(arrayExpressionType instanceof PsiArrayType)) {
            return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                ...
                .descriptionAndTooltip(JavaCompilationErrorLocalize.arrayTypeExpected(JavaHighlightUtil.formatType(arrayExpressionType)))
                .registerFix(QuickFixFactory.getInstance().createReplaceWithListAccessFix(arrayAccessExpression));
        }
    ...
    @RequiredReadAction
    public static HighlightInfo.@Nullable Builder checkCatchParameterIsThrowable(PsiParameter parameter) {
        if (parameter.getDeclarationScope() instanceof PsiCatchSection) {
            return checkMustBeThrowable(parameter.getType(), parameter, true);
        }
        return null;
    }

    @RequiredReadAction
    public static HighlightInfo.@Nullable Builder checkTryResourceIsAutoCloseable(PsiResourceListElement resource) {
    ...
    @Nullable
    @RequiredReadAction
    public static HighlightInfo checkResourceVariableIsFinal(PsiResourceExpression resource) {
        ...
        if (expression instanceof PsiReferenceExpression refExpr) {
            ...
            if (target instanceof PsiVariable variable) {
                ...
                if (!(variable instanceof PsiField)
                    && HighlightControlFlowUtil.isEffectivelyFinal(variable, resource, refExpr)) {
                    return null;
                }
            }
            ...
            return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                .range(expression)
                .descriptionAndTooltip(JavaErrorLocalize.resourceVariableMustBeFinal())
                .create();
        }

        return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
            .range(expression)
            .descriptionAndTooltip(JavaErrorLocalize.declarationOrVariableExpected())
            .create();
    }
    ...
    public static Collection<HighlightInfo> checkArrayInitializer(PsiExpression initializer, PsiType type) {
        ...
        for (PsiExpression expression : initializers) {
            HighlightInfo.Builder hlBuilder = checkArrayInitializerCompatibleTypes(expression, componentType);
            if (hlBuilder != null) {
                ...
                if (fix != null) {
                    hlBuilder.registerFix(new LocalQuickFixOnPsiElementAsIntentionAdapter(fix));
                }
                result.add(hlBuilder.create());
            }
        }
        return result;
    }
    ...
    @RequiredReadAction
    private static HighlightInfo.@Nullable Builder checkArrayInitializerCompatibleTypes(PsiExpression initializer, PsiType componentType) {
        PsiType initializerType = initializer.getType();
        if (initializerType == null) {
            return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                .range(initializer)
                .descriptionAndTooltip(JavaCompilationErrorLocalize.arrayIllegalInitializer(JavaHighlightUtil.formatType(componentType)));
        }
    ...
    public static HighlightInfo checkExpressionRequired(
        ...
        JavaResolveResult resultForIncompleteCode
    ) {
    ...
    public static HighlightInfo checkArrayInitializerApplicable(PsiArrayInitializerExpression expression) {
        ...
        return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
            .range(expression)
            .descriptionAndTooltip(JavaCompilationErrorLocalize.arrayInitializerNotAllowed())
            .registerFix(QuickFixFactory.getInstance().createAddNewArrayExpressionFix(expression))
            .create();
    }

    @Nullable
    @RequiredReadAction
    public static HighlightInfo checkCaseStatement(PsiSwitchLabelStatementBase statement) {
        PsiSwitchBlock switchBlock = statement.getEnclosingSwitchBlock();
        if (switchBlock == null) {
            return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                .range(statement)
                .descriptionAndTooltip(JavaCompilationErrorLocalize.statementCaseOutsideSwitch())
                .create();
        }
    ...
    @RequiredReadAction
    public static Collection<HighlightInfo> checkSwitchLabelValues(PsiSwitchBlock switchBlock) {
        PsiCodeBlock body = switchBlock.getBody();
        if (body == null) {
            return Collections.emptyList();
        }
                    ...
                    for (PsiExpression expr : expressionList.getExpressions()) {
                        ...
                        if (value == null) {
                            results.add(
                                HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                                    .range(expr)
                                    .descriptionAndTooltip(JavaCompilationErrorLocalize.switchLabelConstantExpected())
                                    .create()
                            );
                            continue;
                        }
        ...
        for (Map.Entry<Object, Collection<PsiElement>> entry : values.entrySet()) {
            if (entry.getValue().size() > 1) {
                ...
                LocalizeValue description = value == defaultValue
                    ? JavaCompilationErrorLocalize.switchLabelDuplicateDefault()
                    : JavaCompilationErrorLocalize.switchLabelDuplicate(value);
    ...
    public static HighlightInfo checkIllegalForwardReferenceToField(
        ...
        LocalizeValue description = isIllegalForwardReference
            ? JavaErrorLocalize.illegalForwardReference()
            : JavaErrorLocalize.illegalSelfReference();
        return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
            .range(expression)
            .descriptionAndTooltip(description)
            .create();
    ...
    public static HighlightInfo checkIllegalType(@Nullable PsiTypeElement typeElement) {
        ...
        if (componentType instanceof PsiClassType) {
            ...
            if (aClass == null) {
                ...
                HighlightInfo info = HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                    .range(typeElement)
                    .descriptionAndTooltip(JavaCompilationErrorLocalize.typeUnknownClass(canonicalText))
                    .create();
    ...
    @RequiredReadAction
    public static HighlightInfo.@Nullable Builder checkIllegalVoidType(PsiKeyword type) {
        ...
        return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
            .range(type)
            .descriptionAndTooltip(JavaCompilationErrorLocalize.typeVoidIllegal());
    }

    @RequiredReadAction
    public static HighlightInfo.@Nullable Builder checkMemberReferencedBeforeConstructorCalled(
    ...
    @RequiredReadAction
    private static HighlightInfo.@Nullable Builder checkReferenceToOurInstanceInsideThisOrSuper(
        PsiElement expression,
        PsiClass referencedClass,
        String resolvedName,
        PsiFile containingFile
        ...
        while (element != null) {
            ...
            if (RefactoringChangeUtil.isSuperOrThisMethodCall(element)) {
                ...
                if (expression instanceof PsiReferenceExpression refExpr && PsiUtil.isInnerClass(aClass)) {
                    ...
                    if (fieldInContainingClass != null && refExpr.getQualifierExpression() == null) {
                        hlBuilder.registerFix(new QualifyWithThisFix(containingClass, refExpr));
                    }
                }
            ...
            if (element instanceof PsiReferenceExpression refExpr) {
                PsiElement resolve;
                if (refExpr instanceof PsiReferenceExpressionImpl refExprImpl) {
    ...
    private static HighlightInfo.Builder createMemberReferencedError(String resolvedName, TextRange textRange) {
        return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
            .range(textRange)
    ...
    @RequiredReadAction
    public static HighlightInfo.@Nullable Builder checkImplicitThisReferenceBeforeSuper(
        PsiClass aClass,
    ...
    @RequiredReadAction
    public static HighlightInfo.@Nullable Builder checkLabelWithoutStatement(PsiLabeledStatement statement) {
        if (statement.getStatement() == null) {
            return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                .range(statement)
                .descriptionAndTooltip(JavaCompilationErrorLocalize.labelWithoutStatement());
        }
        return null;
    }

    @RequiredReadAction
    public static HighlightInfo.@Nullable Builder checkLabelAlreadyInUse(PsiLabeledStatement statement) {
        ...
        while (element != null) {
            ...
            if (element instanceof PsiLabeledStatement labeledStmt
                && labeledStmt != statement
                && Objects.equals(labeledStmt.getLabelIdentifier().getText(), text)) {
                return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                    .range(identifier)
                    .descriptionAndTooltip(JavaCompilationErrorLocalize.labelDuplicate(text));
            }
            element = element.getParent();
        }
        return null;
    ...
    public static HighlightInfo checkUnclosedComment(PsiComment comment) {
        ...
        if (!comment.getText().endsWith("*/")) {
            ...
            return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                .range(start, end)
                .descriptionAndTooltip(JavaCompilationErrorLocalize.commentUnclosed())
                .create();
        }
        return null;
    }
    ...
    @RequiredReadAction
    public static Collection<HighlightInfo> checkCatchTypeIsDisjoint(PsiParameter parameter) {
        ...
        for (int i = 0, size = typeElements.size(); i < size; i++) {
            ...
            for (int j = i + 1; j < size; j++) {
                ...
                if (sub || sup) {
                    ...
                    HighlightInfo highlight = HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                        .range(element)
                        .descriptionAndTooltip(JavaCompilationErrorLocalize.exceptionMustBeDisjoint(sub ? name1 : name2, sub ? name2 : name1))
                        .registerFix(QuickFixFactory.getInstance().createDeleteMultiCatchFix(element))
                        .create();
    ...
    @RequiredReadAction
    public static Collection<HighlightInfo> checkExceptionAlreadyCaught(PsiParameter parameter) {
        PsiElement scope = parameter.getDeclarationScope();
        if (!(scope instanceof PsiCatchSection catchSection)) {
            return Collections.emptyList();
        }
        ...
        for (PsiTypeElement typeElement : typeElements) {
            ...
            for (int i = startFrom; i >= 0; i--) {
                ...
                if (highlight) {
                    ...
                    HighlightInfo highlightInfo = HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                        .range(typeElement)
                        .descriptionAndTooltip(JavaCompilationErrorLocalize.exceptionAlreadyCaught(className))
    ...
    private static boolean checkMultipleTypes(PsiClass catchClass, List<PsiType> upperCatchTypes) {
    ...
    @RequiredReadAction
    public static HighlightInfo.@Nullable Builder checkTernaryOperatorConditionIsBoolean(PsiExpression expression, PsiType type) {
    ...
    @Nullable
    @RequiredReadAction
    public static HighlightInfo checkStatementPrependedWithCaseInsideSwitch(PsiSwitchStatement statement) {
    ...
    @RequiredReadAction
    public static HighlightInfo.@Nullable Builder checkAssertOperatorTypes(PsiExpression expression, @Nullable PsiType type) {
        if (type == null) {
            return null;
        }
        ...
        if (expression == assertStatement.getAssertCondition() && !TypeConversionUtil.isBooleanType(type)) {
            ...
            HighlightInfo.Builder hlBuilder =
                createIncompatibleTypeHighlightInfo(PsiType.BOOLEAN, type, expression.getTextRange(), 0);
        ...
        if (expression == assertStatement.getAssertDescription() && TypeConversionUtil.isVoidType(type)) {
            return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                .range(expression)
                .descriptionAndTooltip(JavaCompilationErrorLocalize.typeVoidNotAllowed());
        }
        return null;
    }

    @RequiredReadAction
    public static HighlightInfo.@Nullable Builder checkSynchronizedExpressionType(
        PsiExpression expression,
    ...
    @RequiredReadAction
    public static HighlightInfo.@Nullable Builder checkConditionalExpressionBranchTypesMatch(PsiExpression expression, PsiType type) {
        if (!(expression.getParent() instanceof PsiConditionalExpression conditional)) {
            return null;
        }
        // check else branches only
        if (conditional.getElseExpression() != expression) {
            return null;
        }
        ...
        if (conditional.getType() == null) {
            if (PsiUtil.isLanguageLevel8OrHigher(conditional) && PsiPolyExpressionUtil.isPolyExpression(conditional)) {
                return null;
            }
    ...
    public static HighlightInfo.Builder createIncompatibleTypeHighlightInfo(
        PsiType lType,
        PsiType rType,
        TextRange textRange,
        int navigationShift
        ...
        return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
            .range(textRange)
            .description(JavaCompilationErrorLocalize.typeIncompatible(JavaHighlightUtil.formatType(lType1), JavaHighlightUtil.formatType(rType1)))
            .escapedToolTip(toolTip)
            .navigationShift(navigationShift);
    }

    @RequiredReadAction
    public static HighlightInfo.@Nullable Builder checkSingleImportClassConflict(
        ...
        if (statement.resolve() instanceof PsiClass psiClass) {
            ...
            if (importedClass != null && !containingFile.getManager().areElementsEquivalent(importedClass, psiClass)) {
                return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                    .range(statement)
                    .descriptionAndTooltip(JavaErrorLocalize.singleImportClassConflict(formatClass(importedClass)));
            }
            importedClasses.put(name, Pair.create(null, psiClass));
        }
        return null;
    }
    ...
    @RequiredReadAction
    public static HighlightInfo.@Nullable Builder checkMustBeThrowable(
        @Nullable PsiType type,
        PsiElement context,
        ...
        if (!TypeConversionUtil.isAssignable(throwable, type)) {
            ...
            if (addCastIntention && TypeConversionUtil.areTypesConvertible(type, throwable)
                && context instanceof PsiExpression contextExpr) {
                hlBuilder.registerFix(QuickFixFactory.getInstance().createAddTypeCastFix(throwable, contextExpr));
            }

            PsiClass aClass = PsiUtil.resolveClassInClassTypeOnly(type);
            if (aClass != null) {
                hlBuilder.registerFix(QuickFixFactory.getInstance().createExtendsListFix(aClass, throwable, true));
            }
            return hlBuilder;
        }
        return null;
    ...
    @RequiredReadAction
    private static HighlightInfo.@Nullable Builder checkMustBeThrowable(@Nullable PsiClass aClass, PsiElement context) {
        if (aClass == null) {
            return null;
        }
    ...
    public static HighlightInfo checkLabelDefined(@Nullable PsiIdentifier labelIdentifier, @Nullable PsiStatement exitedStatement) {
        ...
        if (exitedStatement == null) {
            return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                ...
                .descriptionAndTooltip(JavaCompilationErrorLocalize.labelUnresolved(label))
                .create();
        }
        return null;
    }
    ...
    @RequiredReadAction
    public static HighlightInfo.@Nullable Builder checkReference(
        ...
        {
            HighlightInfo.Builder hlBuilder = checkMemberReferencedBeforeConstructorCalled(ref, resolved, containingFile);
            if (hlBuilder != null) {
                return hlBuilder;
            }
        }
        ...
        if (refParent instanceof PsiReferenceExpression refExpr && refExpr.getParent() instanceof PsiMethodCallExpression methodCall) {
            ...
            if (qualifierExpression == ref && resolved != null && !(resolved instanceof PsiClass) && !(resolved instanceof PsiVariable)) {
                return HighlightInfo.newHighlightInfo(HighlightInfoType.WRONG_REF)
                    ...
                    .descriptionAndTooltip(JavaCompilationErrorLocalize.referenceQualifierNotExpression());
            }
        }
        ...
        if (resolved == null) {
            ...
            if (results.length > 1) {
                ...
                description = JavaCompilationErrorLocalize.referenceAmbiguous(refName.getText(), t1, t2);
            }
            else {
                description = JavaCompilationErrorLocalize.referenceUnresolved(refName.getText());
            }

            HighlightInfo.Builder hlBuilder = HighlightInfo.newHighlightInfo(HighlightInfoType.WRONG_REF)
                .range(refName)
                .descriptionAndTooltip(description);
            UnresolvedReferenceQuickFixUpdater.getInstance(containingFile.getProject()).registerQuickFixesLater(ref, hlBuilder);
            return hlBuilder;
        }
        ...
        if (!result.isValidResult() && !PsiUtil.isInsideJavadocComment(ref)) {
            if (!result.isAccessible()) {
                ...
                if (result.isStaticsScopeCorrect()) {
                    registerAccessQuickFixAction(
                        (PsiMember) resolved,
                        ref,
                        hlBuilder,
                        refName.getTextRange(),
                        result.getCurrentFileResolveScope()
                    );
                    if (ref instanceof PsiReferenceExpression refExpr) {
                        hlBuilder.registerFix(QuickFixFactory.getInstance().createRenameWrongRefFix(refExpr));
                    }
                }
                UnresolvedReferenceQuickFixUpdater.getInstance(containingFile.getProject()).registerQuickFixesLater(ref, hlBuilder);
                return hlBuilder;
            ...
            if (!result.isStaticsScopeCorrect()) {
                ...
                registerStaticProblemQuickFixAction(resolved, hlBuilder, ref);
                if (ref instanceof PsiReferenceExpression refExpr) {
                    hlBuilder.registerFix(QuickFixFactory.getInstance().createRenameWrongRefFix(refExpr));
                }
                return hlBuilder;
        ...
        if (resolved instanceof PsiClass resolvedClass && resolvedClass.getContainingClass() == null
            ...
            return HighlightInfo.newHighlightInfo(HighlightInfoType.WRONG_REF)
                .range(refName)
                .descriptionAndTooltip(JavaCompilationErrorLocalize.referenceUnresolved(refName.getText()));
        }
    ...
    @Nullable
    @RequiredReadAction
    public static HighlightInfo checkPackageAndClassConflict(PsiJavaCodeReferenceElement ref, PsiFile containingFile) {
            ...
            if (file != null) {
                ...
                if (module != null) {
                    ...
                    if (aClass != null) {
                        return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                            .range(ref)
                            .descriptionAndTooltip(JavaCompilationErrorLocalize.packageClashesWithClass(ref.getText()))
                            .create();
                    }
                }
            }
    ...
    @RequiredReadAction
    public static HighlightInfo.@Nullable Builder checkElementInReferenceList(
        PsiJavaCodeReferenceElement ref,
        ...
        if (resolved instanceof PsiClass aClass) {
            if (refGrandParent instanceof PsiClass parentClass) {
                if (refGrandParent instanceof PsiTypeParameter typeParameter) {
                    return GenericsHighlightUtil.checkElementInTypeParameterExtendsList(referenceList, typeParameter, resolveResult, ref);
                }
                else if (referenceList.equals(parentClass.getImplementsList()) ||
                    referenceList.equals(parentClass.getExtendsList())) {
                    HighlightInfo.Builder hlBuilder =
                        HighlightClassUtil.checkExtendsClassAndImplementsInterface(referenceList, resolveResult, ref);
                    if (hlBuilder == null) {
                        hlBuilder = HighlightClassUtil.checkCannotInheritFromFinal(aClass, ref);
                    }
                    if (hlBuilder == null) {
                        // TODO highlightInfo = HighlightClassUtil.checkExtendsProhibitedClass(aClass, parentClass, ref);
                    }
                    if (hlBuilder == null) {
                        hlBuilder = GenericsHighlightUtil.checkCannotInheritFromTypeParameter(aClass, ref);
                    }
                    if (hlBuilder == null) {
                ...
                return checkMustBeThrowable(aClass, ref);
            }
        }
    ...
    public static HighlightInfo checkClassReferenceAfterQualifier(PsiReferenceExpression expression, PsiElement resolved) {
        ...
        return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
            .range(qualifier)
            .descriptionAndTooltip(JavaCompilationErrorLocalize.classOrPackageExpected())
            .registerFix(QuickFixFactory.getInstance().createRemoveQualifierFix(qualifier, expression, resolvedClass))
            .create();
    }
    ...
    public static void registerChangeVariableTypeFixes(
        ...
        if (expr instanceof PsiMethodCallExpression methodCall) {
            PsiMethod method = methodCall.resolveMethod();
            if (method != null) {
                highlightInfo.registerFix(PriorityActionWrapper.lowPriority(
                    method,
                    QuickFixFactory.getInstance().createMethodReturnFix(method, parameter.getType(), true)
                ));
            }
        }
    }

    public static List<IntentionAction> getChangeVariableTypeFixes(PsiVariable parameter, PsiType itemType) {
        if (itemType instanceof PsiMethodReferenceType) {
            return Collections.emptyList();
        }
        List<IntentionAction> result = new ArrayList<>();
        if (itemType != null) {
            parameter.getApplication().getExtensionPoint(ChangeVariableTypeQuickFixProvider.class)
                .forEach(fixProvider -> Collections.addAll(result, fixProvider.getFixes(parameter, itemType)));
        }
    ...
    @Nullable
    @RequiredReadAction
    public static HighlightInfo checkAnnotationMethodParameters(PsiParameterList list) {
        PsiElement parent = list.getParent();
        if (PsiUtil.isAnnotationMethod(parent) && list.getParametersCount() > 0) {
            return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                .range(list)
                .descriptionAndTooltip(JavaCompilationErrorLocalize.annotationMemberMayNotHaveParameters())
                .registerFix(QuickFixFactory.getInstance().createRemoveParameterListFix((PsiMethod) parent))
                .create();
        }
        return null;
    }
    ...
    @Nullable
    @RequiredReadAction
    public static HighlightInfo checkForStatement(PsiForStatement statement) {
        PsiStatement init = statement.getInitialization();
        if (init == null
            || init instanceof PsiEmptyStatement
            || init instanceof PsiDeclarationStatement declaration
            && ArrayUtil.getFirstElement(declaration.getDeclaredElements()) instanceof PsiLocalVariable
        ...
        return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
            .range(init)
            .descriptionAndTooltip(JavaCompilationErrorLocalize.statementInvalid())
            .create();
    }

    private static void registerChangeParameterClassFix(PsiType lType, PsiType rType, HighlightInfo.Builder hlBuilder) {
        IntentionAction fix = getChangeParameterClassFix(lType, rType);
        if (fix != null) {
                ...
                if (PsiUtil.isOnAssignmentLeftHand(placeRefExpr)) {
                    ...
                    if (setter != null && PsiUtil.isAccessible(setter, placeRefExpr, accessObjectClass)) {
                        ...
                        if (element instanceof PsiAssignmentExpression assignment && assignment.getOperationTokenType() == JavaTokenType.EQ) {
                            hlBuilder.registerFix(QuickFixFactory.getInstance().createReplaceInaccessibleFieldWithGetterSetterFix(
                                placeRefExpr,
                                setter,
                                true
                            ));
    ...
    @RequiredReadAction
    public static HighlightInfo.@Nullable Builder checkFeature(
        PsiElement element,
        JavaFeature feature,
        LanguageLevel level,
        PsiFile file
        ...
        if (file.getManager().isInProject(file) && !level.isAtLeast(feature.getMinimumLevel())) {
            return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR)
                ...
                .registerFix(QuickFixFactory.getInstance().createIncreaseLanguageLevelFix(feature.getMinimumLevel()))
                .registerFix(QuickFixFactory.getInstance().createShowModulePropertiesFix(element));
        }
    ...
    private static LocalizeValue getUnsupportedFeatureMessage(PsiElement element, JavaFeature feature, LanguageLevel level, PsiFile file) {
        ...
        if (module != null) {
            ...
            if (moduleLanguageLevel.isAtLeast(feature.getMinimumLevel())) {
                return module.getApplication().getExtensionPoint(FilePropertyPusher.class).computeSafeIfAny(
                    pusher -> {
