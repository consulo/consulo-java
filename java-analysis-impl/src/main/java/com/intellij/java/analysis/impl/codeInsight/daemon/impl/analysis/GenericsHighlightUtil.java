...
public class GenericsHighlightUtil {
        ...
        if (registerIntentions) {
          if (typeParameterListOwner instanceof PsiClass) {
            QuickFixAction.registerQuickFixAction(highlightInfo, QuickFixFactory.getInstance()
                                                                                .createChangeClassSignatureFromUsageFix((PsiClass) typeParameterListOwner, referenceParameterList));
          }
          ...
          if (grandParent instanceof PsiTypeElement) {
            ...
            if (variable instanceof PsiVariable) {
              if (targetParametersNum == 0) {
                QuickFixAction.registerQuickFixAction(highlightInfo, QuickFixFactory.getInstance().createRemoveTypeArgumentsFix(variable));
              }
      ...
      if (!bound.equalsToText(JavaClassNames.JAVA_LANG_OBJECT) && GenericsUtil.checkNotInBounds(type, bound, referenceParameterList)) {
        ...
        if (bound instanceof PsiClassType && referenceClass != null && info != null) {
          QuickFixAction.registerQuickFixAction(info, QuickFixFactory.getInstance()
                                                                     .createExtendsListFix(referenceClass, (PsiClassType) bound, true), null);
        }
        return info;
      }
    }
  ...
  @Nullable
  public static HighlightInfo checkElementInTypeParameterExtendsList(@Nonnull PsiReferenceList referenceList,
    ...
    if (!extendFrom.isInterface() && referenceElements.length != 0 && element != referenceElements[0]) {
      ...
      QuickFixAction.registerQuickFixAction(errorResult, QuickFixFactory.getInstance().createMoveBoundClassToFrontFix(aClass, type), null);
      ...
      errorResult = HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR).range(element).descriptionAndTooltip(description).create();
      PsiClassType type = JavaPsiFacade.getInstance(aClass.getProject()).getElementFactory().createType(extendFrom, resolveResult.getSubstitutor());
      QuickFixAction.registerQuickFixAction(errorResult, QuickFixFactory.getInstance().createExtendsListFix(aClass, type, false), null);
    }
    return errorResult;
  }
    ...
    for (Set<PsiMethod> overrideEquivalentMethods : overrideEquivalent.values()) {
      ...
      if (!hasConcrete && defaults != null) {
        ...
        if (!aClass.hasModifierProperty(PsiModifier.ABSTRACT) && !(aClass instanceof PsiTypeParameter) && abstracts != null && unrelatedMethodContainingClass.isInterface()) {
          ...
          QuickFixAction.registerQuickFixAction(info, QuickFixFactory.getInstance().createImplementMethodsFix(aClass));
          return info;
        }
  ...
  @Nullable
  private static HighlightInfo checkSameErasureNotSubSignatureInner(@Nonnull HierarchicalMethodSignature signature,
  ...
  private static HighlightInfo getSameErasureMessage(final boolean sameClass, @Nonnull PsiMethod method, @Nonnull PsiMethod superMethod, TextRange textRange) {
  ...
  public static HighlightInfo checkReferenceTypeUsedAsTypeArgument(PsiTypeElement typeElement, LanguageLevel level) {
    ...
    if (type != PsiType.NULL && type instanceof PsiPrimitiveType || type instanceof PsiWildcardType && ((PsiWildcardType) type).getBound() instanceof PsiPrimitiveType) {
      ...
      if (toConvert instanceof PsiPrimitiveType) {
        ...
        if (boxedType != null) {
          QuickFixAction.registerQuickFixAction(highlightInfo, QuickFixFactory.getInstance()
                                                                              .createReplacePrimitiveWithBoxedTypeAction(typeElement, toConvert.getPresentableText(), ((PsiPrimitiveType)
  ...
  public static HighlightInfo checkForeachExpressionTypeIsIterable(PsiExpression expression) {
    ...
    if (itemType == null) {
      ...
      QuickFixAction.registerQuickFixAction(highlightInfo, QuickFixFactory.getInstance().createNotIterableForEachLoopFix(expression));
      return highlightInfo;
    }
    return null;
  }
  ...
  @Nullable
  public static HighlightInfo checkAccessStaticFieldFromEnumConstructor(@Nonnull PsiReferenceExpression expr, @Nonnull JavaResolveResult result) {
  ...
  @Nullable
  public static HighlightInfo checkTypeParametersList(PsiTypeParameterList list, PsiTypeParameter[] parameters, @Nonnull LanguageLevel level) {
  ...
  @Nullable
  public static HighlightInfo checkOverrideAnnotation(@Nonnull PsiMethod method, @Nonnull PsiAnnotation overrideAnnotation, @Nonnull LanguageLevel languageLevel) {
    try {
      ...
      if (languageLevel.equals(LanguageLevel.JDK_1_5) &&
        ...
        QuickFixAction.registerQuickFixAction(info, QuickFixFactory.getInstance().createIncreaseLanguageLevelFix(LanguageLevel.JDK_1_6));
        return info;
      }
      return null;
  ...
  public static void checkEnumConstantForConstructorProblems(@Nonnull PsiEnumConstant enumConstant, @Nonnull HighlightInfoHolder holder, @Nonnull JavaSdkVersion javaSdkVersion) {
  ...
  public static HighlightInfo checkVarArgParameterIsLast(@Nonnull PsiParameter parameter) {
    ...
    if (declarationScope instanceof PsiMethod) {
      ...
      if (params[params.length - 1] != parameter) {
        ...
        QuickFixAction.registerQuickFixAction(info, QuickFixFactory.getInstance().createMakeVarargParameterLastFix(parameter));
        return info;
      }
    }
    return null;
  ...
  public static HighlightInfo checkGenericCannotExtendException(PsiReferenceList list) {
    ...
    for (PsiJavaCodeReferenceElement referenceElement : referenceElements) {
      ...
      if (InheritanceUtil.isInheritorOrSelf((PsiClass) resolved, throwableClass, true)) {
        ...
        QuickFixAction.registerQuickFixAction(highlightInfo, QuickFixFactory.getInstance().createExtendsListFix(aClass, classType, false));
        return highlightInfo;
      }
    }
    return null;
