...
public class GenericsHighlightUtil {
  ...
  public static HighlightInfo checkInferredTypeArguments(PsiTypeParameterListOwner listOwner, PsiElement call, PsiSubstitutor substitutor) {
    return checkInferredTypeArguments(listOwner.getTypeParameters(), call, substitutor);
  }

  @Nullable
  private static HighlightInfo checkInferredTypeArguments(PsiTypeParameter[] typeParameters, PsiElement call, PsiSubstitutor substitutor) {
    ...
    if (inferredTypeArgument != null) {
      ...
      @NonNls String messageKey = boundClass == null || typeParameter.isInterface() == boundClass.isInterface() ? "generics.inferred.type.for.type.parameter.is.not.within.its.bound.extend" :
          "generics.inferred.type.for.type.parameter.is.not.within.its.bound.implement";

      String description = JavaErrorBundle.message(messageKey, HighlightUtil.formatClass(typeParameter), JavaHighlightUtil.formatType(extendsType), JavaHighlightUtil.formatType(substitutor
    ...
    if (targetParametersNum != refParametersNum && refParametersNum != 0) {
      ...
      if (targetParametersNum == 0) {
        if (PsiTreeUtil.getParentOfType(referenceParameterList, PsiCall.class) != null &&
          ...
          description = JavaErrorBundle.message("generics.type.or.method.does.not.have.type.parameters", typeParameterListOwnerCategoryDescription(typeParameterListOwner),
  ...
  private static HighlightInfo checkTypeParameterWithinItsBound(PsiTypeParameter classParameter,
    ...
    for (PsiClassType type1 : bounds) {
      ...
      if (!bound.equalsToText(JavaClassNames.JAVA_LANG_OBJECT) && GenericsUtil.checkNotInBounds(type, bound, referenceParameterList)) {
        ...
        @NonNls final String messageKey = boundClass == null || referenceClass == null || referenceClass.isInterface() == boundClass.isInterface() ? "generics.type.parameter.is.not.within" +
            ".its.bound.extend" : "generics.type.parameter.is.not.within.its.bound.implement";

        String description = JavaErrorBundle.message(messageKey, referenceClass != null ? HighlightUtil.formatClass(referenceClass) : type.getPresentableText(), JavaHighlightUtil
            .formatType(bound));
  ...
  @jakarta.annotation.Nullable
  public static HighlightInfo checkElementInTypeParameterExtendsList(@jakarta.annotation.Nonnull PsiReferenceList referenceList,
      ...
      if (inheritedSubstitutor != null) {
        ...
        for (PsiTypeParameter typeParameter : typeParameters) {
          ...
          if (!Comparing.equal(type1, type2)) {
            String description = JavaErrorBundle.message("generics.cannot.be.inherited.with.different.type.arguments", HighlightUtil.formatClass(superClass), JavaHighlightUtil
                .formatType(type1), JavaHighlightUtil.formatType(type2));
  ...
  @Nonnull
  public static Collection<HighlightInfo> checkOverrideEquivalentMethods(@jakarta.annotation.Nonnull PsiClass aClass) {
  ...
  public static HighlightInfo checkDefaultMethodOverrideEquivalentToObjectNonPrivate(@Nonnull LanguageLevel languageLevel,
                                                                                     @Nonnull PsiClass aClass,
  ...
  public static HighlightInfo checkUnrelatedDefaultMethods(@jakarta.annotation.Nonnull PsiClass aClass, @Nonnull PsiIdentifier classIdentifier) {
    ...
    for (Set<PsiMethod> overrideEquivalentMethods : overrideEquivalent.values()) {
      ...
      if (!hasConcrete && defaults != null) {
        ...
        if (!aClass.hasModifierProperty(PsiModifier.ABSTRACT) && !(aClass instanceof PsiTypeParameter) && abstracts != null && unrelatedMethodContainingClass.isInterface()) {
          if (defaultMethodContainingClass.isInheritor(unrelatedMethodContainingClass, true) && MethodSignatureUtil.isSubsignature(unrelatedMethod.getSignature(TypeConversionUtil
              .getSuperClassSubstitutor(unrelatedMethodContainingClass, defaultMethodContainingClass, PsiSubstitutor.EMPTY)), defaultMethod.getSignature(PsiSubstitutor.EMPTY))) {
            continue;
          }
          ...
          final String message = JavaErrorBundle.message(key, HighlightUtil.formatClass(aClass, false), JavaHighlightUtil.formatMethod(abstracts.get(0)), HighlightUtil.formatClass
              (unrelatedMethodContainingClass, false));
        ...
        if (isInterface || abstracts == null || unrelatedMethodContainingClass.isInterface()) {
          ...
          if (unrelatedDefaults == null && (abstracts == null || !hasNotOverriddenAbstract(defaultContainingClasses, unrelatedMethodContainingClass))) {
            continue;
          }

          final String message = unrelatedDefaults != null ? " inherits unrelated defaults for " : " inherits abstract and default for ";
          final HighlightInfo info = HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR).range(classIdentifier).descriptionAndTooltip(HighlightUtil.formatClass(aClass) +
              message +
              JavaHighlightUtil.formatMethod(defaultMethod) + " from types " +
              (unrelatedDefaults != null ? unrelatedDefaults : HighlightUtil.formatClass(defaultMethodContainingClass) + " and " + HighlightUtil.formatClass
  ...
  private static boolean belongToOneHierarchy(@Nonnull PsiClass defaultMethodContainingClass, @Nonnull PsiClass unrelatedMethodContainingClass) {
  ...
  private static boolean hasNotOverriddenAbstract(List<PsiClass> defaultContainingClasses, @Nonnull PsiClass abstractMethodContainingClass) {
  ...
  public static HighlightInfo checkUnrelatedConcrete(@Nonnull PsiClass psiClass, @jakarta.annotation.Nonnull PsiIdentifier classIdentifier) {
    ...
    if (superClass != null && superClass.hasTypeParameters()) {
      ...
      for (HierarchicalMethodSignature hms : visibleSignatures) {
        ...
        if (foundMethod != null &&
          ...
          final String description = "Methods " +
  ...
  @jakarta.annotation.Nullable
  private static HighlightInfo checkSameErasureNotSubSignatureInner(@jakarta.annotation.Nonnull HierarchicalMethodSignature signature,
                                                                    @jakarta.annotation.Nonnull PsiManager manager,
                                                                    @Nonnull PsiClass aClass,
                                                                    @jakarta.annotation.Nonnull Map<MethodSignature, MethodSignatureBackedByPsiMethod> sameErasureMethods) {
    ...
    if (sameErasure != null) {
      if (aClass instanceof PsiTypeParameter ||
          ...
          !(InheritanceUtil.isInheritorOrSelf(sameErasure.getMethod().getContainingClass(), method.getContainingClass(), true) || InheritanceUtil.isInheritorOrSelf(method
  ...
  private static HighlightInfo getSameErasureMessage(final boolean sameClass, @jakarta.annotation.Nonnull PsiMethod method, @Nonnull PsiMethod superMethod, TextRange textRange) {
    @NonNls final String key = sameClass ? "generics.methods.have.same.erasure" : method.hasModifierProperty(PsiModifier.STATIC) ? "generics.methods.have.same.erasure.hide" : "generics.methods" +
        ".have.same.erasure.override";
  ...
  public static HighlightInfo checkTypeParameterInstantiation(PsiNewExpression expression) {
    ...
    if (element instanceof PsiTypeParameter) {
      String description = JavaErrorBundle.message("generics.type.parameter.cannot.be.instantiated", HighlightUtil.formatClass((PsiTypeParameter) element));
  ...
  public static HighlightInfo checkReferenceTypeUsedAsTypeArgument(PsiTypeElement typeElement, LanguageLevel level) {
    ...
    if (type != PsiType.NULL && type instanceof PsiPrimitiveType || type instanceof PsiWildcardType && ((PsiWildcardType) type).getBound() instanceof PsiPrimitiveType) {
      final PsiElement element = new PsiMatcherImpl(typeElement).parent(PsiMatchers.hasClass(PsiReferenceParameterList.class)).parent(PsiMatchers.hasClass(PsiJavaCodeReferenceElement.class,
      ...
      if (toConvert instanceof PsiPrimitiveType) {
        ...
        if (boxedType != null) {
          QuickFixAction.registerQuickFixAction(highlightInfo, QUICK_FIX_FACTORY.createReplacePrimitiveWithBoxedTypeAction(typeElement, toConvert.getPresentableText(), ((PsiPrimitiveType)
  ...
  public static HighlightInfo checkForeachExpressionTypeIsIterable(PsiExpression expression) {
    ...
    if (itemType == null) {
      String description = JavaErrorBundle.message("foreach.not.applicable", JavaHighlightUtil.formatType(expression.getType()));
  ...
  public static HighlightInfo checkForEachParameterType(@jakarta.annotation.Nonnull PsiForeachStatement statement, @Nonnull PsiParameter parameter) {
  ...
  @Nullable
  public static HighlightInfo checkAccessStaticFieldFromEnumConstructor(@jakarta.annotation.Nonnull PsiReferenceExpression expr, @jakarta.annotation.Nonnull JavaResolveResult result) {
    ...
    final PsiClass aClass = constructorOrInitializer instanceof PsiEnumConstantInitializer ? (PsiClass) constructorOrInitializer : constructorOrInitializer.getContainingClass();
    ...
    String description = JavaErrorBundle.message("illegal.to.access.static.member.from.enum.constructor.or.instance.initializer", HighlightMessageUtil.getSymbolName(resolved, result
        .getSubstitutor()));
  ...
  public static HighlightInfo checkEnumInstantiation(PsiElement expression, PsiClass aClass) {
    if (aClass != null && aClass.isEnum() &&
        (!(expression instanceof PsiNewExpression) || ((PsiNewExpression) expression).getArrayDimensions().length == 0 && ((PsiNewExpression) expression).getArrayInitializer() == null)) {
  ...
  public static boolean isEnumSyntheticMethod(MethodSignature methodSignature, Project project) {
    ...
    final MethodSignature valueOfMethod = MethodSignatureUtil.createMethodSignature("valueOf", new PsiType[]{javaLangString}, PsiTypeParameter.EMPTY_ARRAY, PsiSubstitutor.EMPTY);
    return valueOfMethod.equals(methodSignature);
  }

  @Nullable
  public static HighlightInfo checkTypeParametersList(PsiTypeParameterList list, PsiTypeParameter[] parameters, @jakarta.annotation.Nonnull LanguageLevel level) {
  ...
  @Nullable
  public static HighlightInfo checkOverrideAnnotation(@Nonnull PsiMethod method, @jakarta.annotation.Nonnull PsiAnnotation overrideAnnotation, @Nonnull LanguageLevel languageLevel) {
    try {
  ...
  public static HighlightInfo checkSafeVarargsAnnotation(PsiMethod method, LanguageLevel languageLevel) {
    ...
    try {
      ...
      if (!isSafeVarargsNoOverridingCondition(method, languageLevel)) {
        return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR).range(safeVarargsAnnotation).descriptionAndTooltip("@SafeVarargs is not allowed on non-final instance methods")
            .create();
      }
  ...
  public static void checkEnumConstantForConstructorProblems(@jakarta.annotation.Nonnull PsiEnumConstant enumConstant, @Nonnull HighlightInfoHolder holder, @jakarta.annotation.Nonnull JavaSdkVersion javaSdkVersion) {
  ...
  public static HighlightInfo checkParametersOnRaw(PsiReferenceParameterList refParamList) {
    ...
    if (resolveResult != null) {
      ...
      if (containingClass != null && PsiUtil.isRawSubstitutor(containingClass, resolveResult.getSubstitutor())) {
        ...
        final String message = element instanceof PsiClass ? JavaErrorBundle.message("generics.type.arguments.on.raw.type") : JavaErrorBundle.message("generics.type.arguments.on.raw" +
            ".method");
  ...
  public static HighlightInfo checkCannotInheritFromEnum(PsiClass superClass, PsiElement elementToHighlight) {
    ...
    if (Comparing.strEqual("java.lang.Enum", superClass.getQualifiedName())) {
      String message = JavaErrorBundle.message("classes.extends.enum");
  ...
  public static HighlightInfo checkGenericCannotExtendException(PsiReferenceList list) {
    ...
    for (PsiJavaCodeReferenceElement referenceElement : referenceElements) {
      ...
      if (InheritanceUtil.isInheritorOrSelf((PsiClass) resolved, throwableClass, true)) {
        ...
        HighlightInfo highlightInfo = HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR).range(referenceElement).descriptionAndTooltip(message).create();
    ...
    if (resolved instanceof PsiClass && ((PsiClass) resolved).hasModifierProperty(PsiModifier.STATIC)) {
      ...
      if (qualifier instanceof PsiJavaCodeReferenceElement) {
        ...
        if (parameterList != null && parameterList.getTypeArguments().length > 0) {
          final String message = JavaErrorBundle.message("generics.select.static.class.from.parameterized.type", HighlightUtil.formatClass((PsiClass) resolved));
   ...
   */
  public static HighlightInfo checkRawOnParameterizedType(@jakarta.annotation.Nonnull PsiJavaCodeReferenceElement parent, PsiElement resolved) {
    ...
    if (qualifier instanceof PsiJavaCodeReferenceElement &&
      ...
      return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR).range(parent).descriptionAndTooltip("Improper formed type; some type parameters are missing").create();
    }
    return null;
  }
      ...
      if (PsiTreeUtil.isAncestor(psiClass.getExtendsList(), ref, false) || PsiTreeUtil.isAncestor(psiClass.getImplementsList(), ref, false)) {
        ...
        if (qualifier instanceof PsiJavaCodeReferenceElement && ((PsiJavaCodeReferenceElement) qualifier).resolve() == psiClass) {
          ...
          if (psiClass.isInheritor(containingClass, true) ||
            ...
            return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR).descriptionAndTooltip(((PsiClass) resolve).getName() + " is not accessible in current context").range(ref)
                .create();
          }
        }
      }
  ...
  private static void registerVariableParameterizedTypeFixes(@jakarta.annotation.Nullable HighlightInfo highlightInfo,
                                                             @Nonnull PsiVariable variable,
                                                             @Nonnull PsiReferenceParameterList parameterList,
                                                             @jakarta.annotation.Nonnull JavaSdkVersion version) {
    ...
    for (Map.Entry<PsiTypeParameter, PsiType> typeEntry : substitutor.getSubstitutionMap().entrySet()) {
      ...
      if (type instanceof PsiIntersectionType) {
        ...
        if (conflictingConjunctsMessage != null) {
          return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR).descriptionAndTooltip("Type parameter " + parameterName + " has incompatible upper bounds: " +
              conflictingConjunctsMessage).range(ref).create();
        }
      }
    }
    return null;
  ...
  public static HighlightInfo areSupersAccessible(@jakarta.annotation.Nonnull PsiClass aClass) {
    return areSupersAccessible(aClass, aClass.getResolveScope(), HighlightNamesUtil.getClassDeclarationTextRange(aClass), true);
  }

  public static HighlightInfo areSupersAccessible(@jakarta.annotation.Nonnull PsiClass aClass, PsiReferenceExpression ref) {
  ...
  private static HighlightInfo areSupersAccessible(@Nonnull PsiClass aClass, GlobalSearchScope resolveScope, TextRange range, boolean checkParameters) {
