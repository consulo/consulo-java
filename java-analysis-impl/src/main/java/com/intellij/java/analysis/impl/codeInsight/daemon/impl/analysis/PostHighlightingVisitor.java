...
public class PostHighlightingVisitor {
  ...
  private boolean isUnusedImportEnabled(HighlightDisplayKey unusedImportKey) {
    ...
    return myProject.getExtensionPoint(ImplicitUsageProvider.class).findFirstSafe(
  ...
  private HighlightInfo processIdentifier(
    ...
    if (parent instanceof PsiParameter parameter) {
      ...
      if (declarationScope instanceof PsiMethod method
        ? compareVisibilities(method, myUnusedSymbolInspection.getParameterVisibility()) : myUnusedSymbolInspection.LOCAL_VARIABLE) {
    ...
    if (parent instanceof PsiClass psiClass) {
      final String acceptedVisibility = psiClass.getContainingClass() == null
        ? myUnusedSymbolInspection.getClassVisibility() : myUnusedSymbolInspection.getInnerClassVisibility();
