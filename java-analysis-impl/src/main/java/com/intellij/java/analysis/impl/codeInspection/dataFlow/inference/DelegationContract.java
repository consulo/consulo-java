...
class DelegationContract implements PreContract {
  ...
  public List<StandardMethodContract> toContracts(PsiMethod method, Supplier<PsiCodeBlock> body) {
    ...
    if (NullableNotNullManager.isNotNull(targetMethod)) {
      List<StandardMethodContract> contracts = ContainerUtil.map(fromDelegate, DelegationContract::returnNotNull);
      fromDelegate = Lists.append(contracts, new StandardMethodContract(emptyConstraints(method), ContractReturnValue.returnNotNull()));
    }
    return fromDelegate;
  }
