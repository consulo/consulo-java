...
public class RefClassImpl extends RefJavaElementImpl implements RefClass {
  ...
  private static final List<RefMethod> EMPTY_METHOD_LIST = new ArrayList<>(0);
  ...
  public RefClassImpl(PsiClass psiClass, RefManager manager) {
  ...
  private static boolean isSelfInheritor(PsiClass psiClass, ArrayList<PsiClass> visited) {
    ...
    for (PsiClass aSuper : psiClass.getSupers()) {
      if (isSelfInheritor(aSuper, visited)) {
        return true;
      }
    }
  ...
  public void addClassExporter(RefJavaElement exporter) {
    if (myClassExporters == null) {
      myClassExporters = new ArrayList<>(1);
    }
    if (myClassExporters.contains(exporter)) {
      return;
    }
