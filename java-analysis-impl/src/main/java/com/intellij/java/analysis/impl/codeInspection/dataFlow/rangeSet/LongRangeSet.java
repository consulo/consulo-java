...
import static com.intellij.java.analysis.impl.codeInspection.dataFlow.rangeSet.LongRangeUtil.*;
...
public abstract class LongRangeSet {
    ...
    public LongRangeSet bitwiseAnd(LongRangeSet other) {
        ...
        if (left.length > 6) {
            left = splitAtZero(new long[]{
                left[0],
                left[left.length - 1]
            });
        }
        if (right.length > 6) {
            right = splitAtZero(new long[]{
                right[0],
    ...
    public LongRangeSet div(LongRangeSet divisor, boolean isLong) {
        ...
        long[] right = splitAtZero(new long[]{
            divisor.min(),
            divisor.max()
        });
        LongRangeSet result = empty();
    ...
    public static LongRangeSet fromType(PsiType type) {
        if (!(type instanceof PsiPrimitiveType) && !TypeConversionUtil.isPrimitiveWrapper(type)) {
            return null;
        }
    ...
    static class Range extends LongRangeSet {
        ...
        public LongRangeSet subtract(@Nonnull LongRangeSet other) {
            ...
            if (other instanceof Point) {
                ...
                if (value < myFrom || value > myTo) {
                    return this;
                }
                if (value == myFrom) {
                    return range(myFrom + 1, myTo);
                }
                if (value == myTo) {
                    return range(myFrom, myTo - 1);
                }
                return new RangeSet(new long[]{
                    myFrom,
                    value - 1,
        ...
        public LongRangeSet castTo(PsiPrimitiveType type) {
            ...
            if (PsiType.CHAR.equals(type)) {
                if (myFrom <= Character.MIN_VALUE && myTo >= Character.MAX_VALUE) {
                    return CHAR_RANGE;
                }
                if (myFrom >= Character.MIN_VALUE && myTo <= Character.MAX_VALUE) {
                    return this;
        ...
        private LongRangeSet mask(int size, PsiPrimitiveType type) {
            ...
            if (myFrom <= -addend && myTo >= addend - 1) {
                return Objects.requireNonNull(fromType(type));
            }
            if (myFrom >= -addend && myTo <= addend - 1) {
                return this;
            }
        ...
        public LongRangeSet plus(LongRangeSet other, boolean isLong) {
            ...
            if (isLong && equals(LONG_RANGE) || !isLong && equals(INT_RANGE)) {
                return this;
            }
    ...
    static final class ModRange extends Range {
        ...
        public boolean isCardinalityBigger(long cutoff) {
            ...
            if (bottom >= myFrom && top > bottom && myTo >= top) {
                ...
                if (wholeCount < 0 || wholeCount > cutoff) {
                    return true;
                }
                for (long i = myFrom; i < bottom; i++) {
                    if (isSet(myBits, remainder(i, myMod))) {
                        wholeCount++;
                    }
                }
        ...
        public boolean intersects(LongRangeSet other) {
            ...
            if (other instanceof ModRange) {
                ModRange modRange = (ModRange)other;
                int lcm = lcm(modRange.myMod);
                if (lcm <= Long.SIZE && (modRange.widenBits(lcm) & widenBits(lcm)) == 0) {
                    return false;
                }
            }
        ...
        private LongRangeSet plus(long min, long max, LongRangeSet other, boolean isLong) {
            if (Integer.bitCount(myMod) == 1 || !subtractionMayOverflow(other.negate(isLong), isLong)) {
                int bit = other instanceof Point
                    ? remainder(((Point)other).myValue, myMod)
                    : Long.numberOfTrailingZeros(((ModRange)other).myBits);
    ...
    static final class RangeSet extends LongRangeSet {
        ...
        RangeSet(long[] ranges) {
            ...
            for (int i = 0; i < ranges.length; i += 2) {
                ...
                if (i > 0 && (ranges[i - 1] == Long.MAX_VALUE || 1 + ranges[i - 1] > ranges[i])) {
                    throw new IllegalArgumentException(
                        "Bad sub-ranges #" + (i / 2 - 1) + " and #" + (i / 2) + " " + Arrays.toString(ranges)
                    );
                }
            }
            myRanges = ranges;
        ...
        public boolean intersects(LongRangeSet other) {
            ...
            while (true) {
                ...
                if (aFrom > bTo) {
                    b += 2;
                    if (b >= otherRanges.length) {
                        return false;
                    }
                }
                else {
                    a += 2;
                    if (a >= myRanges.length) {
                        return false;
                    }
                }
            }
        ...
        public boolean isCardinalityBigger(long cutoff) {
            ...
            for (int i = 0; i < myRanges.length; i += 2) {
                ...
                totalDiff += diff + 1;
                if (totalDiff < 0 || totalDiff > cutoff) {
                    return true;
                }
            }
            return false;
