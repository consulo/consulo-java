        ...
        try {
            for (int i = 0; i < expressions.length; i++) {
                ...
                for (CandidateInfo candidate : methodCandidates) {
                    ...
                    if (!GenericsUtil.isFromExternalTypeLanguage(parameterType)) {
                        continue;
                    }
                    if (suggestedCasts.contains(parameterType.getCanonicalText())) {
                        continue;
                    }
                    // strict compare since even widening cast may help
                    if (Comparing.equal(exprType, parameterType)) {
                        continue;
                    }
