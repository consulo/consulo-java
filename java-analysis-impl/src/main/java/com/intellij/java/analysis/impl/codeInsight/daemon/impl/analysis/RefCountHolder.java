...
final class RefCountHolder {
        ...
        if (isDeadCodeEnabled && !inLibrary) {
            return new GlobalUsageHelperBase() {
                final Map<PsiMember, Boolean> myEntryPointCache = FactoryMap.create((PsiMember member) -> {
                    if (deadCodeInspection.isEntryPoint(member, deadCodeState)) {
                        return true;
                    }
                    if (member instanceof PsiClass) {
                        return !JBTreeTraverser.<PsiMember>from(
                                m -> m instanceof PsiClass
    ...
    void registerReference(@Nonnull PsiReference ref, @Nonnull JavaResolveResult resolveResult) {
        ...
        if (psiFile != null) {
            // look at navigation elements because all references resolve into Cls elements when highlighting library source
        ...
        for (PsiReference ref : array) {
            ...
            if (resolved != null) {
                ...
                if (access != null && access.isReferencedForRead()) {
                    if (isJustIncremented(access, refElement)) {
                        continue;
                    }
                    return true;
                }
