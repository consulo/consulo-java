...
public class TrackingRunner extends StandardDataFlowRunner {
  ...
  public static class CauseItem {
    ...
    CauseItem(@Nonnull String problem, @Nullable PsiElement target) {
    ...
    CauseItem(@Nonnull String problem, @Nonnull MemoryStateChange change) {
  ...
  static class RangeDfaProblemType extends DfaProblemType {
    ...
    RangeDfaProblemType(@Nonnull String template, @Nonnull LongRangeSet set, @Nullable PsiPrimitiveType type) {
      myTemplate = template;
      myRangeSet = set;
      myType = type;
    }
  ...
  private CauseItem fromCallContract(MemoryStateChange history, PsiCallExpression call, ContractReturnValue contractReturnValue) {
    ...
    if (call instanceof PsiMethodCallExpression) {
      ...
      if (nonIntersecting != null) {
		  final Collection<? extends MethodContract> items = ContainerUtil.filter(nonIntersecting, mc -> contractReturnValue.isSuperValueOf(mc.getReturnValue()));
		  MethodContract onlyContract = ContainerUtil.getOnlyItem(items, null);
  ...
  private CauseItem fromSingleContract(@Nonnull MemoryStateChange history, @Nonnull PsiMethodCallExpression call,
									   @Nonnull PsiMethod method, @Nonnull String prefix, @Nonnull MethodContract contract) {
    List<ContractValue> conditions = contract.getConditions();
