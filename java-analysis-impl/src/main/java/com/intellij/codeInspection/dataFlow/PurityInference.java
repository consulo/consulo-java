...
public class PurityInference
{
	...
	static class PurityInferenceVisitor
	{
		...
		{
			if(calls.size() > 1 || !hasReturns)
			{
				return null;
			}

			int bodyStart = body.getStartOffset();
			return new PurityInferenceResult(ContainerUtil.map(mutatedRefs, node -> ExpressionRange.create(node, bodyStart)), calls.isEmpty() ? null : ExpressionRange.create(calls.get(0),
					bodyStart));
		}
	}
