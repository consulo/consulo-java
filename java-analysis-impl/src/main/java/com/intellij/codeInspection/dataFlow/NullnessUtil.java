...
public class NullnessUtil
{
	...
	{
		return CachedValuesManager.getCachedValue(field, () -> CachedValueProvider.Result.create(isImplicitlyInitializedNotNull(field) && weAreSureThereAreNoExplicitWrites(field),
	...
	public static Nullness getExpressionNullness(@Nullable PsiExpression expression)
	{
		...
		if(expression.textMatches(PsiKeyword.NULL))
		{
			return Nullness.NULLABLE;
		}
		if(expression instanceof PsiNewExpression || expression instanceof PsiLiteralExpression || expression instanceof PsiPolyadicExpression || expression instanceof PsiFunctionalExpression ||
