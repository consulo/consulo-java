...
class ContractInferenceInterpreter
{
	...
	@Nullable
	private LighterASTNode getCodeBlock(@javax.annotation.Nullable LighterASTNode parent)
	{
	...
	@javax.annotation.Nullable
	private List<PreContract> handleDelegation(@javax.annotation.Nullable LighterASTNode expression, boolean negated)
	{
		if(expression == null)
		{
			return null;
	...
	private boolean isNegationExpression(@javax.annotation.Nullable LighterASTNode expression)
	{
	...
	private List<PreContract> visitExpression(final List<ValueConstraint[]> states, @Nullable LighterASTNode expr)
	{
		...
		if(type == CONDITIONAL_EXPRESSION)
		{
			...
			return ContainerUtil.concat(visitExpression(antecedentsReturning(conditionResults, TRUE_VALUE), children.get(1)), visitExpression(antecedentsReturning(conditionResults, FALSE_VALUE),
					children.get(2)));
		}
	...
	@Nonnull
	private List<PreContract> visitPolyadic(List<ValueConstraint[]> states, @Nonnull LighterASTNode expr)
	{
	...
	@Nullable
	private static StandardMethodContract contractWithConstraint(ValueConstraint[] state, int parameter, ValueConstraint paramConstraint, ValueConstraint returnValue)
	{
	...
	private List<StandardMethodContract> visitEqualityComparison(List<ValueConstraint[]> states, LighterASTNode op1, LighterASTNode op2, boolean equality)
	{
		...
		if(parameter >= 0 && constraint != null)
		{
			...
			for(ValueConstraint[] state : states)
			{
				...
				else
				{
					...
					ContainerUtil.addIfNotNull(result, contractWithConstraint(state, parameter, constraint.negate(), equality ? FALSE_VALUE : TRUE_VALUE));
				}
			}
			return result;
		}
	...
	private static class CodeBlockContracts
	{
		...
		void registerDeclaration(@Nonnull LighterASTNode declStatement, @Nonnull LighterAST tree, int scopeStart)
		{
	...
	@Nonnull
	static ValueConstraint getLiteralConstraint(@Nonnull IElementType literalTokenType)
	{
	...
	private int resolveParameter(@javax.annotation.Nullable LighterASTNode expr)
	{
