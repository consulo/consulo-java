...
public class StreamChainInliner implements CallInliner
{
	...
	private static final CallMatcher MATCH_TERMINAL = instanceCall(JAVA_UTIL_STREAM_BASE_STREAM, "anyMatch", "allMatch", "noneMatch").parameterCount(1);
	private static final CallMatcher SUM_TERMINAL = instanceCall(JAVA_UTIL_STREAM_BASE_STREAM, "sum", "count").parameterCount(0);
	private static final CallMatcher OPTIONAL_TERMINAL = anyOf(instanceCall(JAVA_UTIL_STREAM_BASE_STREAM, "min", "max").parameterCount(0), instanceCall(JAVA_UTIL_STREAM_BASE_STREAM, "min", "max",
			"reduce").parameterCount(1), instanceCall(JAVA_UTIL_STREAM_BASE_STREAM, "findFirst", "findAny").parameterCount(0));

	private static final CallMatcher SKIP_STEP = instanceCall(JAVA_UTIL_STREAM_BASE_STREAM, "unordered", "parallel", "sequential", "sorted").parameterCount(0);
	...
	private static final CallMatcher STATE_FILTER = anyOf(instanceCall(JAVA_UTIL_STREAM_BASE_STREAM, "distinct").parameterCount(0), instanceCall(JAVA_UTIL_STREAM_BASE_STREAM, "skip", "limit")
			.parameterCount(1));
	private static final CallMatcher BOXED = instanceCall(JAVA_UTIL_STREAM_BASE_STREAM, "boxed").parameterCount(0);
	private static final CallMatcher MAP = instanceCall(JAVA_UTIL_STREAM_BASE_STREAM, "map", "mapToInt", "mapToLong", "mapToDouble", "mapToObj").parameterCount(1);
	private static final CallMatcher FLAT_MAP = instanceCall(JAVA_UTIL_STREAM_BASE_STREAM, "flatMap", "flatMapToInt", "flatMapToLong", "flatMapToDouble").parameterCount(1);
	private static final CallMatcher PEEK = instanceCall(JAVA_UTIL_STREAM_BASE_STREAM, "peek").parameterCount(1);

	private static final CallMatcher STREAM_GENERATE = anyOf(staticCall(JAVA_UTIL_STREAM_STREAM, "generate").parameterCount(1), staticCall(JAVA_UTIL_STREAM_INT_STREAM, "generate").parameterCount(1),
			staticCall(JAVA_UTIL_STREAM_LONG_STREAM, "generate").parameterCount(1), staticCall(JAVA_UTIL_STREAM_DOUBLE_STREAM, "generate").parameterCount(1));
	private static final CallMatcher STREAM_EMPTY = anyOf(staticCall(JAVA_UTIL_STREAM_STREAM, "empty").parameterCount(0), staticCall(JAVA_UTIL_STREAM_INT_STREAM, "empty").parameterCount(0),
			staticCall(JAVA_UTIL_STREAM_LONG_STREAM, "empty").parameterCount(0), staticCall(JAVA_UTIL_STREAM_DOUBLE_STREAM, "empty").parameterCount(0));
	private static final CallMatcher STREAM_OF = anyOf(staticCall(JAVA_UTIL_STREAM_STREAM, "of").parameterTypes("T"), staticCall(JAVA_UTIL_STREAM_INT_STREAM, "of").parameterTypes("int"), staticCall
			(JAVA_UTIL_STREAM_LONG_STREAM, "of").parameterTypes("long"), staticCall(JAVA_UTIL_STREAM_DOUBLE_STREAM, "of").parameterTypes("double"));
	private static final CallMatcher STREAM_OF_ARRAY = anyOf(staticCall(JAVA_UTIL_STREAM_STREAM, "of").parameterTypes("T[]"), staticCall(JAVA_UTIL_STREAM_INT_STREAM, "of").parameterTypes("int[]"),
			staticCall(JAVA_UTIL_STREAM_LONG_STREAM, "of").parameterTypes("long[]"), staticCall(JAVA_UTIL_STREAM_DOUBLE_STREAM, "of").parameterTypes("double[]"));
	...
	private static final CallMapper<UnaryOperator<Step>> INTERMEDIATE_STEP_MAPPER = new CallMapper<UnaryOperator<Step>>().register(FILTER, (PsiMethodCallExpression call) -> (Step next) -> new
			FilterStep(call, next)).register(MAP, (PsiMethodCallExpression call) -> (Step next) -> new MapStep(call, next)).register(FLAT_MAP, (PsiMethodCallExpression call) -> (Step next) -> new
			FlatMapStep(call, next)).register(PEEK, (PsiMethodCallExpression call) -> (Step next) -> new PeekStep(call, next)).register(SORTED, (PsiMethodCallExpression call) -> (Step next) -> new
			SortedStep(call, next)).register(BOXED, (PsiMethodCallExpression call) -> (Step next) -> new BoxedStep(call, next)).register(STATE_FILTER, (PsiMethodCallExpression call) -> (Step next)
			-> new StateFilterStep(call, next));

	private static final CallMapper<Step> TERMINAL_STEP_MAPPER = new CallMapper<Step>().register(FOR_TERMINAL, LambdaTerminalStep::new).register(MATCH_TERMINAL, MatchTerminalStep::new).register
			(SUM_TERMINAL, SumTerminalStep::new).register(OPTIONAL_TERMINAL, OptionalTerminalStep::new);
	...
	static abstract class Step
	{
		...
		void pushResult(CFGBuilder builder)
		{
			...
			{
				builder.push(builder.getFactory().createTypeValue(myCall.getType(), DfaPsiUtil.getElementNullability(myCall.getType(), myCall.resolveMethod())));
			}
		}
	}
	...
	static abstract class TerminalStep extends Step
	{
		...
		void before(CFGBuilder builder)
		{
			...
			builder.pushVariable(myResult).chain(this::pushInitialValue).assign().pop().chain(super::before);
		}
	...
	static class MatchTerminalStep extends TerminalStep
	{
		...
		void iteration(CFGBuilder builder)
		{
			builder.invokeFunction(1, myFunction).ifConditionIs(!"allMatch".equals(myCall.getMethodExpression().getReferenceName())).pushVariable(myResult).push(builder.getFactory().getBoolean
					("anyMatch".equals(myCall.getMethodExpression().getReferenceName()))).assign().pop().endIf();
		}
	}
	...
	static class FilterStep extends Step
	{
		...
		void iteration(CFGBuilder builder)
		{
			builder.dup().invokeFunction(1, myFunction).ifConditionIs(true).chain(myNext::iteration).elseBranch().pop().endIf();
		}
	}
	...
	static class MapStep extends Step
	{
		...
		void iteration(CFGBuilder builder)
		{
			builder.invokeFunction(1, myFunction).assignTo(builder.createTempVariable(StreamApiUtil.getStreamElementType(myCall.getType()))).chain(myNext::iteration);
		}
	}
	...
	static class FlatMapStep extends Step
	{
		...
		void iteration(CFGBuilder builder)
		{
			...
			else
			{
				...
				builder.pop().doWhile().push(builder.getFactory().createTypeValue(outType, Nullness.UNKNOWN)).chain(myNext::iteration).endWhileUnknown();
			}
		}
	}
	...
	static class PeekStep extends Step
	{
		...
		void iteration(CFGBuilder builder)
		{
			builder.dup().invokeFunction(1, myFunction).pop().chain(myNext::iteration);
		}
	}
	...
	static class StateFilterStep extends Step
	{
		...
		void iteration(CFGBuilder builder)
		{
			builder.pushUnknown().ifConditionIs(true).chain(myNext::iteration).elseBranch().pop().endIf();
		}
	}
	...
	static void buildStreamCFG(CFGBuilder builder, Step firstStep, PsiExpression originalQualifier)
	{
		...
		if(STREAM_GENERATE.test(sourceCall))
		{
			...
			builder.evaluateFunction(fn).chain(firstStep::before).doWhile().pushVariable(builder.createTempVariable(inType)).invokeFunction(0, fn).assign().chain(firstStep::iteration)
					.endWhileUnknown();
			return;
		}
		...
		if(STREAM_OF_ARRAY.test(sourceCall))
		{
			...
			{
				builder.pushExpression(originalQualifier).pop().chain(firstStep::before).chain(b -> makeMainLoop(b, firstStep, inType));
				return;
			}
		}
		if(empty)
		...
		if(array)
		{
			...
			{
				builder.pushExpression(qualifierExpression).chain(firstStep::before).checkNotNull(qualifierExpression, NullabilityProblem.passingNullableToNotNullParameter).pop().push(SpecialField
						.ARRAY_LENGTH.createValue(builder.getFactory(), qualifierValue)).push(builder.getFactory().getConstFactory().createFromValue(0, PsiType.INT, null)).ifCondition(JavaTokenType
						.GT).chain(b -> makeMainLoop(b, firstStep, inType)).endIf();
				return;
			}
		}
		if(COLLECTION_STREAM.test(sourceCall))
		{
			...
			if(qualifierValue != null)
			{
				builder.pushExpression(qualifierExpression).chain(firstStep::before).checkNotNull(sourceCall, NullabilityProblem.callNPE).pop().push(SpecialField.COLLECTION_SIZE.createValue(builder
						.getFactory(), qualifierValue)).push(builder.getFactory().getConstFactory().createFromValue(0, PsiType.INT, null)).ifCondition(JavaTokenType.GT).chain(b -> makeMainLoop(b,
						firstStep, inType)).endIf();
				return;
			}
		}
		builder.pushExpression(originalQualifier).checkNotNull(firstStep.myCall, NullabilityProblem.callNPE).pop().chain(firstStep::before).pushUnknown().ifConditionIs(true).chain(b -> makeMainLoop
				(b, firstStep, inType)).endIf();
	}

	private static void makeMainLoop(CFGBuilder builder, Step firstStep, PsiType inType)
	{
		builder.doWhile().pushVariable(builder.createTempVariable(inType)).push(builder.getFactory().createTypeValue(inType, DfaPsiUtil.getTypeNullability(inType))).assign().chain
				(firstStep::iteration).endWhileUnknown();
	}
