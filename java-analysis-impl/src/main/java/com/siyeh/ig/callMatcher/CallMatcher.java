...
public interface CallMatcher extends Predicate<PsiMethodCallExpression>
{
	...
	class Simple implements CallMatcher
	{
		...
		private boolean parametersMatch(@Nonnull PsiParameterList parameterList)
		{
			...
			return StreamEx.zip(myParameters, parameterList.getParameters(), Simple::parameterTypeMatches).allMatch(Boolean.TRUE::equals);
		}
		...
		private boolean methodMatches(PsiMethod method)
		{
			...
			if(myStatic != method.getModifierList().hasExplicitModifier(PsiModifier.STATIC) || (myStatic && !myClassName.equals(aClass.getQualifiedName())) || (!myStatic && !InheritanceUtil
					.isInheritor(aClass, myClassName)))
			{
				return false;
			}
			return true;
