...
public class TestUtils
{
	...
	private static final CallMatcher ASSERT_THROWS = CallMatcher.staticCall(JUnitCommonClassNames.ORG_JUNIT_JUPITER_API_ASSERTIONS, "assertThrows");
	...
	public static boolean isRunnable(PsiMethod method)
	{
		...
		if(method.hasModifierProperty(PsiModifier.ABSTRACT) || method.hasModifierProperty(PsiModifier.STATIC) || !method.hasModifierProperty(PsiModifier.PUBLIC))
		{
			return false;
		}
	...
	public static boolean isJUnit4TestClass(@Nullable PsiClass aClass, boolean runWithIsTestClass)
	{
		...
		if(AnnotationUtil.isAnnotated(aClass, RUN_WITH, CHECK_HIERARCHY))
		{
	...
	{
		if(!isInTestSourceContent(element))
		{
			return false;
		}
		for(; element != null && !(element instanceof PsiFile); element = element.getParent())
		{
			...
			if(element instanceof PsiLambdaExpression)
			{
				...
				if(expressionList != null)
				{
					...
					if(parent instanceof PsiMethodCallExpression && ASSERT_THROWS.test((PsiMethodCallExpression) parent))
					{
						return true;
					}
				}
	...
	public static boolean hasExpectedExceptionAnnotation(PsiMethod method)
	{
		...
		return hasAnnotationWithParameter(modifierList, "org.junit.Test", "expected") || hasAnnotationWithParameter(modifierList, "org.testng.annotations.Test", "expectedExceptions");
	}
