...
public class ConcatenationInjector implements ConcatenationAwareInjector {
    ...
    public ConcatenationInjector(Project project,
                                 TemporaryPlacesRegistry temporaryPlacesRegistry) {
        ...
        myXmlIndex = CachedValuesManager.getManager(myProject).createCachedValue((CachedValueProvider<Collection<String>>) () -> {
            ...
            return new CachedValueProvider.Result<>(stringSet, myConfiguration);
        }, false);
        myAnnoIndex = CachedValuesManager.getManager(myProject).createCachedValue(() -> {
            ...
            return new CachedValueProvider.Result<>(result, PsiModificationTracker.MODIFICATION_COUNT, myConfiguration);
        }, false);
    }
    ...
    public void inject(@Nonnull final MultiHostRegistrar registrar, @Nonnull PsiElement... operands) {
        ...
        InjectionProcessor injectionProcessor = new InjectionProcessor(myConfiguration, mySupport, operands) {
            ...
            protected Pair<PsiLanguageInjectionHost, Language> processInjection(Language language,
                ...
                InjectorUtils.putInjectedFileUserData(host,
                    language,
                    InjectedLanguageManager.FRANKENSTEIN_INJECTION,
                    unparsable ? Boolean.TRUE : null);
    ...
    public static class InjectionProcessor {
        ...
        private List<Pair<PsiLanguageInjectionHost, Language>> processInjectionWithContext(BaseInjection injection, boolean settingsAvailable) {
            ...
            final List<Object> objects =
                ContextComputationProcessor.collectOperands(injection.getPrefix(), injection.getSuffix(), unparsableRef, myOperands);
            if (objects.isEmpty()) {
                return List.of();
            }
            ...
            if (separateFiles) {
                ...
                if (isReferenceInject(language)) {
                    ...
                    for (Trinity<PsiLanguageInjectionHost, InjectedLanguage, TextRange> trinity : result) {
                        ContainerUtil.addIfNotNull(res,
                            processInjection(language,
                                Collections.singletonList(trinity),
                                settingsAvailable,
                                unparsableRef.get()));
                    }
                }
