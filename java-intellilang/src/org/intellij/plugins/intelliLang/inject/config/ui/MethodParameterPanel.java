...
public class MethodParameterPanel extends AbstractInjectionPanel<MethodParameterInjection>
{
	...
	public MethodParameterPanel(MethodParameterInjection injection, final Project project)
	{
		...
		myParamsTable.getTree().setCellRenderer(new ColoredTreeCellRenderer()
		{
			...
			public void customizeCellRenderer(final JTree tree, final Object value, final boolean selected, final boolean expanded, final boolean leaf, final int row, final boolean hasFocus)
			{
				...
				{
					name = PsiFormatUtil.formatMethod((PsiMethod) o, PsiSubstitutor.EMPTY, PsiFormatUtil.SHOW_NAME | PsiFormatUtil.SHOW_PARAMETERS, PsiFormatUtil.SHOW_NAME | PsiFormatUtil.SHOW_TYPE);
				}
	...
	private ColumnInfo[] createColumnInfos()
	{
		return new ColumnInfo[]{
				new ColumnInfo<DefaultMutableTreeNode, Boolean>(" ")
				{ // "" for the first column's name isn't a good idea
		...
		public void calcData(final DataKey key, final DataSink sink)
		{
			if(LangDataKeys.PSI_ELEMENT.equals(key))
			{
				...
				if(!selection.isEmpty())
				{
					...
					{
						sink.put(LangDataKeys.PSI_ELEMENT, (PsiElement) o);
					}
				}
			}
		}
