...
public class AnnotationDocGenerator
{
	...
	{
		return myTargetClass != null ? !JavaDocInfoGenerator.isDocumentedAnnotationType(myTargetClass) : isKnownNonDocumented(myAnnotation.getQualifiedName());
	}
	...
	void generateAnnotation(StringBuilder buffer, AnnotationFormat format)
	{
		...
		PsiClassType type = myTargetClass != null && qualifiedName != null && JavaDocUtil.findReferenceTarget(myOwner.getManager(), qualifiedName, myOwner) != null ? JavaPsiFacade.getElementFactory
				(myOwner.getProject()).createType(myTargetClass, PsiSubstitutor.EMPTY) : null;
	...
	public static List<AnnotationDocGenerator> getAnnotationsToShow(@Nonnull PsiModifierListOwner owner)
	{
		...
		for(PsiAnnotation annotation : AnnotationUtil.getAllAnnotations(owner, false, null))
		{
			...
			if(!(shownAnnotations.add(annotation.getQualifiedName()) || JavaDocInfoGenerator.isRepeatableAnnotationType(annotation)))
			{
				continue;
			}
