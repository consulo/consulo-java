...
public class MethodDuplicatesHandler implements RefactoringActionHandler, ContextAwareActionHandler {
  ...
  public static void invokeOnScope(final Project project, final Set<PsiMember> members, AnalysisScope scope, boolean silent) {
    ...
    scope.accept(new PsiRecursiveElementVisitor() {
      ...
      public void visitFile(PsiFile file) {
        ...
        Module targetModule = ModuleUtilCore.findModuleForPsiElement(file);
    ...
    if (duplicates.isEmpty()) {
      if (!silent) {
        Runnable nothingFoundRunnable = () -> {
          LocalizeValue message = RefactoringLocalize.ideaHasNotFoundAnyCodeThatCanBeReplacedWithMethodCall(Application.get().getName());
          Messages.showInfoMessage(project, message.get(), REFACTORING_NAME);
        };
            ...
            CommandProcessor.getInstance().executeCommand(project, new Runnable() {
              ...
              public void run() {
                PostprocessReformattingAspect.getInstance(project).postponeFormattingInside(new Runnable() {
                  @Override
                  public void run() {
                    MatchProvider matchProvider = member instanceof PsiMethod ? new MethodDuplicatesMatchProvider((PsiMethod) member,
  ...
  public static DuplicatesFinder createDuplicatesFinder(PsiMember member) {
    ...
    if (member instanceof PsiMethod) {
      ...
      if (statements.length != 1 || !(statements[0] instanceof PsiReturnStatement)) {
        PsiStatement lastStatement = statements.length > 0 ? statements[statements.length - 1] : null;
        if (lastStatement instanceof PsiReturnStatement) {
          PsiExpression returnValue = ((PsiReturnStatement) lastStatement).getReturnValue();
    ...
    List<? extends PsiVariable> inputVariables = member instanceof PsiMethod ? Arrays.asList(((PsiMethod) member).getParameterList().getParameters()) : new ArrayList<PsiVariable>();
  ...
  static String getStatusMessage(int duplicatesNo) {
    return RefactoringLocalize.methodDuplicatesFoundMessage(duplicatesNo).get();
  }
