...
public class PullUpProcessor extends BaseRefactoringProcessor implements PullUpData {
  ...
  @Override
    ...
    protected void performRefactoring(@Nonnull UsageInfo[] usages) {
        ...
        application.invokeLater(this::processMethodsDuplicates, application.getNoneModalityState(), myProject.getDisposed());
    }
    ...
    private void processMethodsDuplicates() {
        ProgressManager.getInstance().runProcessWithProgressSynchronously(
            () -> myProject.getApplication().runReadAction(() -> {
                ...
                Query<PsiClass> search = ClassInheritorsSearch.search(myTargetSuperClass);
                Set<VirtualFile> hierarchyFiles = new HashSet<>();
                ...
                Set<PsiMember> methodsToSearchDuplicates = new HashSet<>();
                for (PsiMember psiMember : myMembersAfterMove) {
                    if (psiMember instanceof PsiMethod method && method.isValid() && method.getBody() != null) {
    ...
    public void moveMembersToBase() throws IncorrectOperationException {
        ...
        for (PsiMember member : myMembersAfterMove) {
            ...
            JavaRefactoringListenerManager listenerManager = JavaRefactoringListenerManager.getInstance(myProject);
    ...
    private PsiSubstitutor upDownSuperClassSubstitutor() {
        ...
        Map<PsiTypeParameter, PsiType> substitutionMap = TypeConversionUtil.getSuperClassSubstitutor
        ...
        for (PsiTypeParameter parameter : substitutionMap.keySet()) {
            ...
            if (PsiUtil.resolveClassInType(type) instanceof PsiTypeParameter typeParam) {
                substitutor = substitutor.put(typeParam, JavaPsiFacade.getElementFactory(myProject).createType(parameter));
            }
        }
        return substitutor;
    }
    ...
    public void moveFieldInitializations() throws IncorrectOperationException {
        ...
        Set<PsiField> movedFields = new LinkedHashSet<>();
        for (PsiMember member : myMembersAfterMove) {
    ...
    public static boolean checkedInterfacesContain(
        Collection<? extends MemberInfoBase<? extends PsiMember>>
            memberInfos,
        PsiMethod psiMethod
    ) {
        for (MemberInfoBase<? extends PsiMember> memberInfo : memberInfos) {
            if (memberInfo.isChecked()
                && memberInfo.getMember() instanceof PsiClass psiClass
                && Boolean.FALSE.equals(memberInfo.getOverrides())
                && psiClass.findMethodBySignature(psiMethod, true) != null) {
                return true;
            }
        }
        return false;
