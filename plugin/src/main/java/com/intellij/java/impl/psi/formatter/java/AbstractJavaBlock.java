...
import static com.intellij.java.impl.psi.formatter.java.MultipleFieldDeclarationHelper.findLastFieldInGroup;
...
public abstract class AbstractJavaBlock extends AbstractBlock implements JavaBlock, ReservedWrapsProvider {
  ...
  private final BlockFactory myBlockFactory = new BlockFactory() {
    @Override
    public Block createBlock(ASTNode node, Indent indent, Alignment alignment, Wrap wrap, FormattingMode formattingMode) {
  ...
  protected AbstractJavaBlock(ASTNode node,
                              Wrap wrap,
                              Alignment alignment,
                              Indent indent,
  ...
  private AbstractJavaBlock(ASTNode ignored,
  ...
  @Nullable
  private static Alignment createBlockAlignment(AlignmentStrategy strategy, ASTNode node) {
  ...
  public Block createJavaBlock(ASTNode child,
                               CommonCodeStyleSettings settings,
                               JavaCodeStyleSettings javaSettings,
  ...
  private Block createJavaBlock(ASTNode child,
                                CommonCodeStyleSettings settings,
                                JavaCodeStyleSettings javaSettings,
                                @Nullable Indent indent,
                                Wrap wrap,
    ...
    SimpleJavaBlock simpleJavaBlock =
      new SimpleJavaBlock(child, wrap, alignmentStrategy, actualIndent, settings, javaSettings, myFormattingMode);
    simpleJavaBlock.setStartOffset(startOffset);
    return simpleJavaBlock;
  }

  public static Block newJavaBlock(ASTNode child,
                                   CommonCodeStyleSettings settings,
                                   JavaCodeStyleSettings javaSettings,
                                   FormattingMode formattingMode) {
    Indent indent = getDefaultSubtreeIndent(child, settings);
  ...
  @Nullable
  private static Indent getDefaultSubtreeIndent(ASTNode child, CommonCodeStyleSettings settings) {
    ...
    ASTNode prevElement = skipCommentsAndWhitespacesBackwards(child);
    ...
    if (parent != null) {
      Indent defaultChildIndent = getChildIndent(parent, indentOptions);
  ...
  @Nullable
  private static ASTNode skipCommentsAndWhitespacesBackwards(ASTNode node) {
  ...
  @Nullable
  private static Indent getChildIndent(ASTNode parent, CommonCodeStyleSettings.IndentOptions indentOptions) {
...
//    if (parentType instanceof JspClassLevelDeclarationStatementType) return Indent.getNormalIndent();
    ...
    // TODO if (parentType == JavaElementType.IMPLICIT_CLASS) return Indent.getNoneIndent();
  ...
  private boolean isTernaryOperatorToken(ASTNode child) {
    IElementType nodeType = myNode.getElementType();
  ...
  private static boolean isTextBlock(PsiElement childPsi) {
    PsiLiteralExpression literal = ObjectUtil.tryCast(childPsi, PsiLiteralExpression.class);
  ...
  protected ASTNode processChild(List<Block> result,
    ...
    if (childType == JavaElementType.METHOD_CALL_EXPRESSION) {
      ...
      if (childType == JavaTokenType.LBRACE && nodeType == JavaElementType.ARRAY_INITIALIZER_EXPRESSION) {
        ...
        myJavaSettings.ALIGN_CONSECUTIVE_ASSIGNMENTS && childType == JavaElementType.EXPRESSION_STATEMENT) {
        ...
        Wrap wrap = Wrap.createWrap(getWrapType(myJavaSettings.SWITCH_EXPRESSIONS_WRAP), false);
        WrappingStrategy wrapStrategy = WrappingStrategy.createDoNotWrapCommaStrategy(wrap);
        ...
        if ((myAlignmentStrategy.getAlignment(nodeType, childType) != null &&
          ...
          || (myJavaSettings.ALIGN_CONSECUTIVE_ASSIGNMENTS && ((nodeType == JavaElementType.EXPRESSION_STATEMENT
        ...
        Block block =
          createJavaBlock(child, mySettings, myJavaSettings, childIndent, wrap, alignmentStrategyToUse, childOffset, myFormattingMode);
  ...
  @Nullable
  private ChildAttributes getDelegateAttributes(List<? extends Block> result) {
  ...
  private static boolean isAfterErrorElement(ASTNode currNode) {
  ...
  private static boolean isInsideMethodCall(PsiElement element) {
  ...
  private ASTNode processField(List<? super Block> result,
                               ASTNode child,
                               AlignmentStrategy alignmentStrategy,
                               Wrap defaultWrap,
                               Indent childIndent) {
  ...
  @Nullable
  private ASTNode processSwitchExpression(List<? super Block> result,
                                          ASTNode child,
  ...
  @Nullable
  private ASTNode processTernaryOperationRange(List<? super Block> result,
                                               ASTNode child,
                                               Wrap defaultWrap,
                                               Indent childIndent) {
    ArrayList<Block> localResult = new ArrayList<>();
    Wrap wrap = arrangeChildWrap(child, defaultWrap);
    Alignment alignment = myReservedAlignment;
    Alignment alignment2 = myReservedAlignment2;
    ...
    result.add(new SyntheticCodeBlock(localResult,
                                      chooseAlignment(alignment, alignment2, child),
                                      getSettings(),
                                      myJavaSettings,
                                      null,
                                      wrap));
  ...
  private Block createMethodCallExpressionBlock(ASTNode node, Wrap blockWrap, Alignment alignment, Indent indent) {
  ...
  private boolean shouldAlignChild(ASTNode child) {
   ...
   */
  private boolean shouldAlignFieldInColumns(ASTNode child) {
  ...
  private ASTNode processParenthesisBlock(List<Block> result,
                                          ASTNode child,
                                          WrappingStrategy wrappingStrategy,
                                          boolean doAlign) {
  ...
  private ASTNode processParenthesisBlock(IElementType from,
                                          @Nullable IElementType to,
                                          List<Block> result,
                                          ASTNode child,
                                          WrappingStrategy wrappingStrategy,
                                          boolean doAlign) {
    ...
    while (child != null) {
      if (!FormatterUtil.containsWhiteSpacesOnly(child) && child.getTextLength() > 0) {
        if (child.getElementType() == from) {
          ...
          processChild(result,
                       child,
                       alignmentStrategyToUse.getAlignment(elementType),
  ...
  private static boolean canUseAnonymousClassAlignment(ASTNode child) {
  ...
  @Nullable
  private ASTNode processEnumBlock(List<? super Block> result,
                                   @Nullable ASTNode child,
                                   ASTNode last) {
    WrappingStrategy wrappingStrategy = new WrappingStrategy(Wrap.createWrap(getWrapType(mySettings.ENUM_CONSTANTS_WRAP), true)) {
      @Override
    ...
    while (child != null) {
      if (!FormatterUtil.containsWhiteSpacesOnly(child) && child.getTextLength() > 0) {
        result.add(createJavaBlock(child,
                                   mySettings,
                                   myJavaSettings,
                                   Indent.getNormalIndent(),
                                   wrappingStrategy.getWrap(child.getElementType()),
                                   AlignmentStrategy.getNullStrategy(),
                                   myFormattingMode));
  ...
  private int getBraceStyle() {
    PsiElement psiNode = SourceTreeToPsiMap.treeElementToPsi(myNode);
  ...
  protected Indent getCodeBlockInternalIndent(int baseChildrenIndent) {
    ...
    int shift = braceStyle == CommonCodeStyleSettings.NEXT_LINE_SHIFTED ? 1 : 0;
  ...
  @Nullable
  protected ASTNode composeCodeBlock(List<? super Block> result,
                                     ASTNode child,
                                     Indent indent,
                                     int childrenIndent,
    ...
    while (child != null) {
      ...
      boolean isRBrace = isRBrace(child);
  ...
  protected ChildAlignmentStrategyProvider getStrategyProvider() {
    ...
    if (parentType == JavaElementType.METHOD || myNode instanceof PsiCodeBlock) {
      if (mySettings.ALIGN_CONSECUTIVE_VARIABLE_DECLARATIONS || myJavaSettings.ALIGN_CONSECUTIVE_ASSIGNMENTS) {
        ...
        if (myJavaSettings.ALIGN_CONSECUTIVE_ASSIGNMENTS) {
  ...
  public SyntheticCodeBlock createCodeBlockBlock(List<Block> localResult, Indent indent, int childrenIndent) {
    SyntheticCodeBlock result = new SyntheticCodeBlock(localResult, null, getSettings(), myJavaSettings, indent, null);
