...
class ExcludeTable extends ListTableWithButtons<ExcludeTable.Item> {
    ...
    private static final ColumnInfo<Item, String> NAME_COLUMN = new ColumnInfo<>("Class/package/member qualified name mask") {
        @Nullable
        @Override
    ...
    private static final ColumnInfo<Item, ExclusionScope> SCOPE_COLUMN = new ColumnInfo<>("Scope") {
        @Nullable
        @Override
    ...
    boolean isModified() {
        return !getExcludedPackages(ExclusionScope.IDE).equals(Arrays.asList(CodeInsightSettings.getInstance().EXCLUDED_PACKAGES)) || !getExcludedPackages(
            ExclusionScope.Project).equals
