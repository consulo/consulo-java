...
public class Result {
        ...
        if (mySettings.dropObsoleteCasts()) {
            ...
            if (myBinding != null) {
                for (Map.Entry<PsiTypeCastExpression, PsiType> entry : myCastToOperandType.entrySet()) {
                    ...
                    if (!(operandType instanceof PsiTypeVariable)
                        && castType != null
                        && !isBottomArgument(operandType)
                        && castType.isAssignableFrom(operandType)) {
                        set.add(cast);
                    }
                }
            }
    ...
    public void apply(Set<PsiElement> victims) {
        for (PsiElement element : victims) {
            if (element instanceof PsiTypeCastExpression cast && myCastToOperandType.containsKey(cast)) {
                try {
    ...
    public String getReport() {
        return RefactoringLocalize.typeCookReport(
            getRatio(myCookedNumber, myVictims.size()),
