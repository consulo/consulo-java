...
public class JavaIntroduceParameterMethodUsagesProcessor implements IntroduceParameterMethodUsagesProcessor {
    ...
    public boolean processChangeMethodUsage(
        ...
        if (method != null && IntroduceParameterUtil.isMethodInUsages(data, method, usages)) {
            argList
                .addAfter(
                    JavaPsiFacade.getElementFactory(data.getProject()).createExpressionFromText(data.getParameterName(), argList),
                    anchor
                );
        }
        else {
    ...
    private static void substituteTypeParametersInInitializer(
        PsiElement initializer,
                ...
                if ((method.isVarArgs() && actualParamLength + 1 < parametersCount) ||
                    ...
                    conflicts.putValue(
                        ...
                        LocalizeValue.localizeTODO(
                            "Incomplete call(" + call.getText() + "): " + parametersCount + " parameters expected " +
                                "but only " + actualParamLength + " found"
                        )
                    );
                }
                data.getParametersToRemove().forEach(paramNum -> {
                    if (paramNum >= actualParamLength) {
                        conflicts.putValue(
                            call,
                            LocalizeValue.localizeTODO(
                                "Incomplete call(" + call.getText() + "): expected to delete the " + paramNum + " parameter " +
                                    "but only " + actualParamLength + " parameters found"
                            )
                        );
                    }
                });
