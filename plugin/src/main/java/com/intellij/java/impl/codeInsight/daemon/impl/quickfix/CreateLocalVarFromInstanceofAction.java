...
public class CreateLocalVarFromInstanceofAction extends BaseIntentionAction {
    ...
    public void invoke(@Nonnull final Project project, final Editor editor, final PsiFile file) {
        ...
        try {
            PsiStatement statementInside = isNegated(instanceOfExpression) ? null : getExpressionStatementInside(file, editor, instanceOfExpression.getOperand());
    ...
    protected static PsiStatement getExpressionStatementInside(PsiFile file, Editor editor, @Nonnull PsiExpression operand) {
        ...
        if (blockStatement == null) {
            PsiIfStatement ifStm = PsiTreeUtil.getParentOfType(elementAt, PsiIfStatement.class);
            if (ifStm != null) {
                ...
                PsiWhileStatement whileStatement = PsiTreeUtil.getParentOfType(elementAt, PsiWhileStatement.class);
    ...
    @Nullable
    private static PsiDeclarationStatement createLocalVariableDeclaration(PsiInstanceOfExpression instanceOfExpression,
                                                                          PsiStatement statementInside) throws IncorrectOperationException {
    ...
    private static void reformatNewCodeBlockBraces(PsiElement start, PsiBlockStatement end)
