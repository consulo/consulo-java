...
public class TypeMigrationLabeler {
    ...
    @RequiredReadAction
    public LocalizeValue[] getFailedConversionsReport() {
        final LocalizeValue[] report = new LocalizeValue[myFailedConversions.size()];
        int j = 0;
        ...
        for (final Pair<SmartPsiElementPointer<PsiExpression>, PsiType> p : myFailedConversions.keySet()) {
            ...
            report[j++] = LocalizeValue.localizeTODO(
                "Cannot convert type of expression <b>" + StringUtil.escapeXml(element.getText()) + "</b>" +
        ...
        return ContainerUtil.map2Array(usages, new UsageInfo[usages.size()], pair ->
        {
            ...
            return new UsageInfo(expr) {
                ...
                public String getTooltipText() {
                    ...
                    return "Cannot convert type of the expression from " + type.getCanonicalText() + " to " + pair.getSecond()
                        .getCanonicalText();
                }
            };
        });
                    ...
                    for (Map.Entry<TypeMigrationUsageInfo, PsiType> info : myNewExpressionTypeChange.entrySet()) {
                        ...
                        if (expression.equals(expressionToReplace)) {
                            final PsiNewExpression newExpression =
                                TypeMigrationReplacementUtil.replaceNewExpressionType(
                                    project,
                                    (PsiNewExpression) expressionToReplace,
                                    info
                                );
                ...
                if (migrations.size() != 1) {
                    ...
                    if (variablesByMigrationType.size() == 1) {
                        ...
                        final PsiVariable[] variables =
                            PsiTreeUtil.getChildrenOfType(typeElement.getParent().getParent(), PsiVariable.class);
    ...
    void convertExpression(final PsiExpression expr, final PsiType toType, final PsiType fromType, final boolean isCovariantPosition) {
        final TypeConversionDescriptorBase conversion = myRules.findConversion(
            fromType,
            toType,
            ...
            for (int i = -1; i < methods.length; i++) {
                ...
                if (i < 0) {
                    ...
                    if (newMethodName != null) {
                        final MigrateGetterNameSetting migrateGetterNameSetting =
                            myRules.getConversionSettings(MigrateGetterNameSetting.class);
    ...
    private String isMethodNameCanBeChanged(PsiMethod method) {
        ...
        final PsiMethod[] getters =
            GetterSetterPrototypeProvider.findGetters(
                ...
                while (parent != null) {
                    ...
                    if (psiClass != null && containingClass != null) {
                        final PsiSubstitutor classSubstitutor =
                            TypeConversionUtil.getClassSubstitutor(psiClass, containingClass, PsiSubstitutor.EMPTY);
    ...
    void markFailedConversion(final Pair<PsiType, PsiType> typePair, final PsiExpression expression) {
        ...
        final Pair<SmartPsiElementPointer<PsiExpression>, PsiType> key =
            Pair.create(SmartPointerManager.getInstance(expression.getProject()).createSmartPsiElementPointer(expression), typePair
                .getSecond());
    ...
    void migrateMethodCallExpressions(final PsiType migrationType, final PsiParameter param, final PsiClass psiClass) {
        ...
        final PsiType strippedType =
            migrationType instanceof PsiEllipsisType ellipsisType ? ellipsisType.getComponentType() : migrationType;
                        ...
                        for (int idx = index; idx < (param.isVarArgs() ? expressions.length : index + 1); idx++) {
                            ...
                            if (type != null) {
                                migrateExpressionType(
                                    ...
                                    strippedType,
                                    parent,
                                    TypeConversionUtil.isAssignable(strippedType, type),
                                    true
                                );
                            }
                        }
                    ...
                    if (value != null) {
                        ...
                        if (type != null && !type.equals(migrationType)) {
                            migrateExpressionType(
                                ...
                                migrationType,
                                statement,
                                TypeConversionUtil.isAssignable(migrationType, type),
                                true
                            );
                        }
                    }
