...
public class JavaDocLocalInspection extends BaseLocalInspectionTool {
    ...
    public ProblemDescriptor[] checkClass(
        ...
        if (docComment == null) {
            return required
                ? new ProblemDescriptor[]{createDescriptor(
                elementToHighlight,
                InspectionLocalize.inspectionJavadocProblemDescriptor().get(),
                manager,
                isOnTheFly
            )}
                : null;
        ...
        LocalizeValue[] absentDescriptionKeys = {
    ...
    public ProblemDescriptor[] checkMethod(
        ...
        if (IGNORE_DEPRECATED && (method.isDeprecated() || method.getContainingClass().isDeprecated())) {
            return null;
        }
        if (myIgnoreSimpleAccessors && PropertyUtil.isSimplePropertyAccessor(method)) {
            return null;
        }
        PsiDocComment docComment = method.getDocComment();
        PsiMethod[] superMethods = method.findSuperMethods();
        boolean required = isJavaDocRequired(method);
        if (docComment == null) {
            if (required) {
                if (superMethods.length > 0) {
                    return null;
                }
                boolean javaDocNotNecessary = method.getApplication().getExtensionPoint(JavaDocNotNecessaryFilter.class)
                    .anyMatchSafe(filter -> filter.isJavaDocNotNecessary(method));
                if (javaDocNotNecessary) {
                    return null;
                }
        ...
        for (PsiElement descriptionElement : descriptionElements) {
            if (descriptionElement instanceof PsiInlineDocTag inlineDocTag && "inheritDoc".equals(inlineDocTag.getName())) {
                return null;
            }
        }
        ...
        if (tagByName != null) {
            ...
            if (tagInfo != null && tagInfo.isValidInContext(method)) {
                return null;
            }
        }
        ...
        if (required && superMethods.length == 0 && isTagRequired(method, "param")) {
        ...
        if (required && superMethods.length == 0 && isTagRequired(method, "@throws")
            && method.getThrowsList().getReferencedTypes().length > 0) {
            ...
            if (!declaredExceptions.isEmpty()) {
                for (PsiClassType declaredException : declaredExceptions.keySet()) {
                    problems.add(createMissingThrowsTagDescriptor(method, manager, declaredException, isOnTheFly));
                }
            }
        }

        List<ProblemDescriptor> tagProblems = getTagValuesProblems(method, tags, manager, isOnTheFly);
        if (tagProblems != null) {
            problems.addAll(tagProblems);
        }
        ...
        for (PsiDocTag tag : tags) {
            if ("param".equals(tag.getName())) {
                if (extractTagDescription(tag).isEmpty() && tag.getValueElement() instanceof PsiDocParamRef paramRef) {
    ...
    private void processThrowsTags(
        ...
        for (PsiDocTag tag : tags) {
            if ("throws".equals(tag.getName()) || "exception".equals(tag.getName())) {
                ...
                if (!(firstChild != null && firstChild.getFirstChild() instanceof PsiJavaCodeReferenceElement ref)) {
                    continue;
                }
