...
public class ChangeClassSignatureDialog extends RefactoringDialog {
    ...
    public ChangeClassSignatureDialog(
        ...
        myTypeParameterInfos = new ArrayList<>(typeParameterInfos);
        myTypeCodeFragments = new ArrayList<>(typeCodeFragments);
    ...
    protected JComponent createCenterPanel() {
        ...
        panel.add(
            SeparatorFactory.createSeparator(RefactoringLocalize.changeclasssignatureParametersPanelBorderTitle().get(), myTable),
            BorderLayout.NORTH
        );
    ...
    protected void doAction() {
        ...
        if (message != null) {
            CommonRefactoringUtil.showErrorMessage(
                RefactoringLocalize.errorIncorrectData(),
        ...
        ChangeClassSignatureProcessor processor = new ChangeClassSignatureProcessor(
            myClass.getProject(),
            myClass,
    ...
    private String validateAndCommitData() {
        ...
        for (int i = 0; i < myTypeCodeFragments.size(); i++) {
            ...
            if (info.getOldParameterIndex() >= 0) {
                continue;
            }
            PsiType type;
            try {
