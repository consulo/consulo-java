...
public abstract class TypeMigrationDialog extends RefactoringDialog {
  ...
  public TypeMigrationDialog(Project project, PsiElement[] roots, @Nullable TypeMigrationRules rules) {
    super(project, false);
    myRoots = roots;
    myRules = rules;
  ...
  protected JComponent createCenterPanel() {
    ...
    GridBagConstraints gc = new GridBagConstraints(0, GridBagConstraints.RELATIVE, 1, 1, 1, 0, GridBagConstraints.NORTHWEST, GridBagConstraints.HORIZONTAL, JBUI.insets(5, 5, 0, 0), 0, 0);
  ...
  public static class MultipleElements extends TypeMigrationDialog {
    ...
    public MultipleElements(Project project, PsiElement[] roots, Function<PsiElement, PsiType> migrationTypeFunction, TypeMigrationRules rules) {
  ...
  public static class SingleElement extends TypeMigrationDialog {
    ...
    public SingleElement(Project project, PsiElement[] roots) {
      ...
      String[] types = getValidTypes(project, root);
    ...
    private String[] getValidTypes(Project project, PsiElement root) {
      if (root instanceof PsiField || root instanceof PsiMethod) {
        PsiModifierList modifierList = ((PsiModifierListOwner) root).getModifierList();
      ...
      for (PsiReference reference : ReferencesSearch.search(root, GlobalSearchScope.fileScope(root.getContainingFile()))) {
        ...
        PsiExpression expr = PsiTreeUtil.getParentOfType(element, PsiExpression.class, false);
        if (expr != null) {
      ...
      try {
        ...
        PsiType[] psiTypes = new TypeSelectorManagerImpl(project, myTypeCodeFragment.getType(), occurrences).getTypesForAll();
