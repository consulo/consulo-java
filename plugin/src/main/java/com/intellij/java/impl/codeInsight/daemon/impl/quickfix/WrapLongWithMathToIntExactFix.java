...
public class WrapLongWithMathToIntExactFix extends LocalQuickFixAndIntentionActionOnPsiElement implements HighPriorityAction {
    ...
    public WrapLongWithMathToIntExactFix(PsiType type, PsiExpression expression) {
        super(expression);
        myType = type;
    }
    ...
    private static PsiElement getModifiedExpression(PsiElement expression) {
        return JavaPsiFacade.getElementFactory(expression.getProject())
            .createExpressionFromText("java.lang.Math.toIntExact(" + expression.getText() + ")", expression);
    }

    private static class MyMethodArgumentFix extends MethodArgumentFix implements HighPriorityAction {
        protected MyMethodArgumentFix(
        ...
        public LocalizeValue getText() {
            return myArgList.getExpressions().length == 1
                ? JavaQuickFixLocalize.wrapLongWithMathToIntParameterSingleText()
                : JavaQuickFixLocalize.wrapLongWithMathToIntParameterMultipleText(myIndex + 1);
        }
