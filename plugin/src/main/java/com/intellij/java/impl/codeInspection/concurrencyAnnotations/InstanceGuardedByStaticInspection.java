...
public class InstanceGuardedByStaticInspection extends BaseJavaLocalInspectionTool {
    ...
    private static class Visitor extends JavaElementVisitor {
        ...
        public void visitDocTag(@Nonnull PsiDocTag psiDocTag) {
            ...
            if (!(guardField != null && guardField.isStatic())) {
                return;
            }
            myHolder.newProblem(JavaAnalysisLocalize.instanceMemberGuardedByStatic0Loc(guardValue))
                .range(psiDocTag)
                .create();
        }
        ...
        public void visitAnnotation(@Nonnull PsiAnnotation annotation) {
            ...
            if (!guardField.isStatic()) {
                return;
            }
            myHolder.newProblem(JavaAnalysisLocalize.instanceMemberGuardedByStaticRefLoc())
                .range(guardRef)
                .create();
        }
    }
