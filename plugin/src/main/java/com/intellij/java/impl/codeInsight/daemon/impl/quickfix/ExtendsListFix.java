...
public class ExtendsListFix extends LocalQuickFixAndIntentionActionOnPsiElement {
    ...
    public ExtendsListFix(@Nonnull PsiClass aClass, @Nonnull PsiClass classToExtendFrom, boolean toAdd) {
        this(
            aClass,
            classToExtendFrom,
            JavaPsiFacade.getInstance(aClass.getProject()).getElementFactory().createType(classToExtendFrom),
            toAdd
        );
    }
    ...
    private ExtendsListFix(
        ...
        BiFunction<Object, Object, LocalizeValue> messageKey;
        if (classToExtendFrom != null && aClass.isInterface() == classToExtendFrom.isInterface()) {
            messageKey = toAdd ? JavaQuickFixLocalize::addClassToExtendsList : JavaQuickFixLocalize::removeClassFromExtendsList;
        }
        else {
            messageKey =
                toAdd ? JavaQuickFixLocalize::addInterfaceToImplementsList : JavaQuickFixLocalize::removeInterfaceFromImplementsList;
        }

        myName = messageKey.apply(
            aClass.getName(),
            classToExtendFrom == null ? ""
                : classToExtendFrom instanceof PsiTypeParameter ? classToExtendFrom.getName() : classToExtendFrom.getQualifiedName()
        );
    }
    ...
    protected void invokeImpl(PsiClass myClass) {
        ...
        PsiReferenceList extendsList = !(myClass instanceof PsiTypeParameter) && myClass.isInterface() != myClassToExtendFrom.isInterface()
