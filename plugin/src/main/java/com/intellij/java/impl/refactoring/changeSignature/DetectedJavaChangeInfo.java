...
import com.intellij.java.impl.codeInsight.daemon.impl.quickfix.DefineParamsDefaultValueAction;
...
class DetectedJavaChangeInfo extends JavaChangeInfoImpl
{
						...
						, method.getName(), getOldName(), delegate)
				{
					...
					protected void fillOldParams(PsiMethod method)
					{
						...
						if(!method.isConstructor())
						{
							try
							{
								isReturnTypeChanged = isReturnTypeChanged || (DetectedJavaChangeInfo.this.getNewReturnType() != null ? !Comparing.strEqual(DetectedJavaChangeInfo.this
										.getNewReturnType().getTypeText(), newReturnType.getTypeText()) : newReturnType != null);
							}
	...
	{
		return new ChangeSignatureProcessor(method.getProject(), new DetectedJavaChangeInfo(getNewVisibility(), getSuperMethod(), getNewReturnType(), (ParameterInfoImpl[]) getNewParameters(),
				getNewExceptions(), getNewName(), method.getName(), isGenerateDelegate())
		{
			@Override
			protected void fillOldParams(PsiMethod method)
			{
