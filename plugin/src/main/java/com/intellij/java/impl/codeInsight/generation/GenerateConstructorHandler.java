...
public class GenerateConstructorHandler extends GenerateMembersHandlerBase {
    ...
    public GenerateConstructorHandler() {
        super(CodeInsightLocalize.generateConstructorFieldsChooserTitle());
    }
    ...
    protected ClassMember[] getAllOriginalMembers(PsiClass aClass) {
        ...
        for (PsiField field : fields) {
            ...
            if (field.isFinal() && field.getInitializer() != null) {
                continue;
            }
            ...
            if (!array.isEmpty()) {
                if (array.size() == 1) {
                    ...
                    for (int i = 0; i < elements.size(); i++) {
                        ClassMember member = elements.get(i);
    ...
    protected MemberChooser<ClassMember> createMembersChooser(
        ...
        List<ClassMember> preselection = preselect(members);
    ...
    protected List<? extends GenerationInfo> generateMemberPrototypes(PsiClass aClass, ClassMember[] members)
        ...
        List<GenerationInfo> constructors =
    ...
    public static PsiMethod generateConstructorPrototype(
        ...
        if (baseConstructor != null) {
            ...
            if (copyJavaDoc) {
                PsiDocComment docComment = ((PsiMethod) baseConstructor.getNavigationElement()).getDocComment();
                if (docComment != null) {
        ...
        if (baseConstructor != null) {
            ...
            if (!CommonClassNames.JAVA_LANG_ENUM.equals(superClass.getQualifiedName())) {
                ...
                if (baseConstructor instanceof PsiCompiledElement) { // to get some parameter names
                    PsiClass dummyClass =
                        JVMElementFactoryProvider.forLanguageRequired(baseConstructor.getProject(), baseConstructor.getLanguage())
                            .createClass("Dummy");
        ...
        PsiMethod dummyConstructor = factory.createConstructor(aClass.getName());
        ...
        for (PsiField field : fields) {
            ...
            String paramName = javaStyle.propertyNameToVariableName(name, VariableKind.PARAMETER);
            paramName = javaStyle.suggestUniqueVariableName(paramName, dummyConstructor, true);
            PsiParameter param = factory.createParameter(paramName, field.getType(), aClass);

            NullableNotNullManager.getInstance(field.getProject()).copyNullableOrNotNullAnnotation(field, param);

            constructor.getParameterList().add(param);
            dummyConstructor.getParameterList().add(param.copy());
