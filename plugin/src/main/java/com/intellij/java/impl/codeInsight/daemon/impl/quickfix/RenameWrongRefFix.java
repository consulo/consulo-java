...
public class RenameWrongRefFix implements SyntheticIntentionAction {
    ...
    public boolean isAvailable(Project project, Editor editor, PsiFile file) {
        if (!myRefExpr.isValid() || !myRefExpr.getManager().isInProject(myRefExpr)) {
            return false;
        }
    ...
    private LookupElement[] collectItems() {
        ...
        if (!qualified && !(myRefExpr.getParent() instanceof PsiMethodCallExpression)) {
            ...
            class MyScopeProcessor extends BaseScopeProcessor {
                List<PsiElement> myResult = new ArrayList<>();
                ...
                public boolean execute(PsiElement element, ResolveState state) {
                    if (element instanceof PsiNamedElement
                        && element instanceof PsiModifierListOwner modifierListOwner
                        && myFilterMethods == element instanceof PsiMethod) {
                        if (modifierListOwner.hasModifierProperty(PsiModifier.STATIC) == myFilterStatics) {
    ...
    public void invoke(Project project, Editor editor, PsiFile file) {
        ...
        float proportion = AWTEditorUtil.calcVerticalScrollProportion(editor);
