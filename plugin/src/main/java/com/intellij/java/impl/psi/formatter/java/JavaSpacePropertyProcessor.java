...
public final class JavaSpacePropertyProcessor extends JavaElementVisitor {
        ...
        if (myChild2 != null && StdTokenSets.COMMENT_BIT_SET.contains(myType2)) {
            ...
            if (myResult == null) {
                ...
                if (prev != null && prev.getElementType() == JavaTokenType.END_OF_LINE_COMMENT) {
                    ...
                    myResult =
                        Spacing.createSpacing(1, Integer.MIN_VALUE, 0, mySettings.KEEP_LINE_BREAKS, mySettings.KEEP_BLANK_LINES_IN_CODE);
                }
    ...
    public void visitClass(PsiClass aClass) {
        ...
        if (myRole2 == ChildRole.LBRACE) {
            ...
            if (aClass.isEnum()) {
                ...
                myResult =
                    Spacing.createSpacing(spaces, spaces, 0, mySettings.KEEP_LINE_BREAKS, mySettings.KEEP_BLANK_LINES_IN_DECLARATIONS);
            }
    ...
    private static boolean isJavadocHoldingEnumConstant(ASTNode node) {
        if (!(node instanceof EnumConstantElement)) {
            return false;
        }
        return node.getFirstChildNode() instanceof PsiDocComment;
    }

    @RequiredReadAction
    private Spacing getSpaceBeforeMethodLBrace(PsiMethod method) {
    ...
    @RequiredReadAction
    private static int getMethodHeaderEndOffset(PsiMethod method) {
    ...
    @RequiredReadAction
    private Spacing getSpaceBeforeClassLBrace(PsiClass aClass) {
        ...
        if (classBraceStyle == END_OF_LINE || shouldHandleAsSimpleClass(aClass)) {
            ...
            int startOffset =
                nameIdentifier == null ? myParent.getTextRange().getStartOffset() : nameIdentifier.getTextRange().getStartOffset();
    ...
    @RequiredReadAction
    private Spacing getSpaceBeforeLBrace(
        ASTNode lBraceBlock,
    ...
    @RequiredReadAction
    private boolean shouldHandleAsSimpleClass(PsiClass aClass) {
        ...
        if (lBrace != null && rBrace != null) {
            if (lBrace.getPrevSibling() instanceof PsiWhiteSpace beforeLBrace && beforeLBrace.textContains('\n')) {
                return false;
            }
    ...
    @RequiredReadAction
    private boolean shouldHandleAsSimpleBlock(ASTNode node) {
        if (!mySettings.KEEP_SIMPLE_BLOCKS_IN_ONE_LINE) {
            return false;
        }

        if (node.getPsi().getPrevSibling() instanceof PsiWhiteSpace prev && prev.textContains('\n')) {
            return false;
        }
    ...
    @RequiredReadAction
    private boolean shouldHandleAsSimpleMethod(PsiMethod method) {
    ...
    @RequiredReadAction
    private static int getMethodHeaderStartOffset(PsiMethod method) {
    ...
    @RequiredReadAction
    private static boolean isEndOfLineCommentAfterLBrace(ASTNode node) {
    ...
    private static boolean isTheOnlyClassMember(ASTNode node) {
        ...
        if (prev == null || !(prev.getElementType() == JavaTokenType.LBRACE)) {
            return false;
        }
      ...
      myResult = Spacing.createSpacing(0, 0, 1, mySettings.KEEP_LINE_BREAKS, 0);
        ...
        if (processMethod()) {
            ...
            if (myRole1 == ChildRole.LBRACE) {
                ...
                myResult =
                    Spacing.createSpacing(0, 0, blankLines, mySettings.KEEP_LINE_BREAKS, mySettings.KEEP_BLANK_LINES_IN_DECLARATIONS);
            }
        }
    ...
    private static boolean isAbstractMethod(ASTNode node) {
        return node.getPsi() instanceof PsiMethod method && method.isAbstract();
    }
    ...
    public void visitImportList(PsiImportList list) {
        if (ElementType.IMPORT_STATEMENT_BASE_BIT_SET.contains(myType1) &&
            ...
            if (myImportHelper == null) {
                myImportHelper = new ImportHelper(myJavaSettings);
            }
    ...
    public void visitTryStatement(PsiTryStatement statement) {
        if (myRole2 == ChildRole.FINALLY_KEYWORD || myRole2 == ChildRole.CATCH_SECTION) {
            boolean putRightChildOnNewLine =
                myRole2 == ChildRole.FINALLY_KEYWORD ? mySettings.FINALLY_ON_NEW_LINE : mySettings.CATCH_ON_NEW_LINE;
    ...
    @Override
    @RequiredReadAction
    public void visitForeachPatternStatement(PsiForeachPatternStatement statement) {
        handleForeach();
    }
    ...
    private void processCodeBlock(boolean keepInOneLine, TextRange textRange) {
        ...
        if (myRole1 == ChildRoleBase.NONE && !lhsStatement || myRole2 == ChildRoleBase.NONE && !rhsStatement) {
            ...
            if ((mySettings.KEEP_MULTIPLE_EXPRESSIONS_IN_ONE_LINE &&
                (FormatterUtil.isFormatterCalledExplicitly() || Application.get().isUnitTestMode())) ||
    ...
    private boolean keepInOneLine(PsiCodeBlock block) {
        if (block.getParent() instanceof PsiMethod method) {
            return shouldHandleAsSimpleMethod(method);
        }
        else if (block.getParent() instanceof PsiLambdaExpression lambda) {
            return shouldHandleAsSimpleLambda(lambda);
        }
        else {
    ...
    public void visitIfStatement(PsiIfStatement statement) {
        if (myRole2 == ChildRole.ELSE_KEYWORD) {
            ...
            else if (mySettings.ELSE_ON_NEW_LINE) {
                myResult = Spacing.createSpacing(0, 0, 1, mySettings.KEEP_LINE_BREAKS, mySettings.KEEP_BLANK_LINES_IN_CODE);
            }
            else {
    ...
    private boolean isInsidePsiForStatement() {
        return myParent.getParent() instanceof PsiDeclarationStatement decl && decl.getParent() instanceof PsiForStatement;
    }
    ...
    public void visitDeconstructionList(PsiDeconstructionList deconstructionList) {
        ...
        if (myType2 == JavaTokenType.RPARENTH) {
            createParenthSpace(
                myJavaSettings.RPAREN_ON_NEW_LINE_IN_DECONSTRUCTION_PATTERN,
                myJavaSettings.SPACE_WITHIN_DECONSTRUCTION_LIST
            );
        }
        else if (myType1 == JavaTokenType.LPARENTH) {
            createParenthSpace(
                myJavaSettings.NEW_LINE_AFTER_LPAREN_IN_DECONSTRUCTION_PATTERN,
                myJavaSettings.SPACE_WITHIN_DECONSTRUCTION_LIST
            );
        }
        else if (myType1 == JavaTokenType.COMMA) {
    ...
    @RequiredReadAction
    private void createSpaceWithLinefeedIfListWrapped(PsiExpressionList list, boolean space) {
        ...
        int spaces = space ? 1 : 0;
        myResult =
            Spacing.createDependentLFSpacing(spaces, spaces, ranges, mySettings.KEEP_LINE_BREAKS, mySettings.KEEP_BLANK_LINES_IN_CODE);
    }
...
//    }
    ...
    private boolean isAllowedToMoveSemicolonInLongCallChain() {
        if (!myJavaSettings.WRAP_SEMICOLON_AFTER_CALL_CHAIN) {
            return false;
        }

        if (!(myChild1 instanceof PsiMethodCallExpression) || myType1 != JavaElementType.METHOD_CALL_EXPRESSION) {
            return false;
        }

        if (isInsidePsiForStatement()) {
            return false;
        }
