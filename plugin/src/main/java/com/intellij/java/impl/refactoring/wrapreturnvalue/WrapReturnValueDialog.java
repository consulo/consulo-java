...
class WrapReturnValueDialog extends RefactoringDialog {
    ...
    WrapReturnValueDialog(PsiMethod sourceMethod) {
        ...
        setTitle(JavaRefactoringLocalize.wrapReturnValueTitle());
        init();
    }
    ...
    protected void doAction() {
        ...
        invokeRefactoring(new WrapReturnValueProcessor(
            className,
            packageName,
            ((DestinationFolderComboBox)myDestinationCb).selectDirectory(
                new PackageWrapper(sourceMethod.getManager(), packageName),
                false
            ),
            sourceMethod,
            useExistingClass,
            createInnerClass,
            (PsiField)myFieldsCombo.getSelectedItem()
        ));
    }
