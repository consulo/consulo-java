...
public abstract class RenameJavaMemberProcessor extends RenamePsiElementProcessor {
    ...
    public static void qualifyMember(PsiMember member, PsiElement occurrence, String newName) throws IncorrectOperationException {
        qualifyMember(occurrence, newName, member.getContainingClass(), member.isStatic());
    }
    ...
    public static PsiReferenceExpression createMemberReference(PsiMember member, PsiElement context) throws IncorrectOperationException {
        ...
        if (manager.areElementsEquivalent(resolved, member)) {
            return ref;
        }
        return createQualifiedMemberReference(context, name, member.getContainingClass(), member.isStatic());
    }

    @RequiredWriteAction
    protected static PsiReferenceExpression createQualifiedMemberReference(
        PsiElement context,
        String name,
        ...
        if (isStatic) {
            ...
            PsiJavaCodeReferenceElement classReference =
                factory.createClassReferenceElement(contextClass != null ? contextClass : containingClass);
            qualifier.replace(classReference);
        }
        return ref;
    }

    protected static void findMemberHidesOuterMemberCollisions(PsiMember member, String newName, List<UsageInfo> result) {
        ...
        for (PsiClass aClass = fieldClass != null ? fieldClass.getContainingClass() : null;
             aClass != null;
    ...
    @RequiredReadAction
    protected static void findCollisionsAgainstNewName(
        PsiMember memberToRename,
        String newName,
        List<? super MemberHidesStaticImportUsageInfo> result
    ) {
        if (!memberToRename.isPhysical()) {
            return;
        }
        List<PsiReference> potentialConflicts = new ArrayList<>();
        if (memberToRename.getContainingFile() instanceof PsiJavaFile containingFile) {
            ...
            if (importList != null) {
                for (PsiImportStaticStatement staticImport : importList.getImportStaticStatements()) {
                    ...
                    if (targetClass != null) {
                        Set<PsiMember> importedMembers = new HashSet<>();
        ...
        for (PsiReference potentialConflict : potentialConflicts) {
            if (potentialConflict instanceof PsiJavaReference javaRef) {
                JavaResolveResult resolveResult = javaRef.advancedResolve(false);
    ...
    protected static void qualifyStaticImportReferences(List<MemberHidesStaticImportUsageInfo> staticImportHides)
        ...
        for (MemberHidesStaticImportUsageInfo info : staticImportHides) {
            ...
            if (info.getReferencedElement() instanceof PsiMember targetMember && occurrence != null) {
