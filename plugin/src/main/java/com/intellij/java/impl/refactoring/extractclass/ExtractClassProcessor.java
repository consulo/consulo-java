    ...
    for (PsiReference reference : ReferencesSearch.search(field, scope)) {
      ...
      if (element instanceof PsiReferenceExpression) {
        ...
        if (RefactoringUtil.isPlusPlusOrMinusMinus(exp.getParent())) {
          ...
          usages.add(isStatic ? new ReplaceStaticVariableAssignment(exp, qualifiedName) : new ReplaceInstanceVariableAssignment(PsiTreeUtil.getParentOfType(exp,
              PsiAssignmentExpression.class), delegateFieldName, setter, getter, field.getName()));

        } else {
          usages.add(isStatic ? new ReplaceStaticVariableAccess(exp, qualifiedName, enumConstants.contains(field)) : new ReplaceInstanceVariableAccess(exp, delegateFieldName, getter,
              field.getName()));
        }
