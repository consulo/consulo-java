	...
	private static class Builder extends JavaElementVisitor
	{
		...
		private Evaluator[] visitStatements(PsiStatement[] statements)
		{
			...
			for(PsiStatement psiStatement : statements)
			{
				...
				if(myResult != null)
				{ // for example declaration w/o initializer produces empty evaluator now
		...
		private static boolean isBinaryNumericPromotionApplicable(PsiType lType, PsiType rType, IElementType opType)
		{
			...
			return opType == JavaTokenType.ASTERISK || opType == JavaTokenType.DIV || opType == JavaTokenType.PERC || opType == JavaTokenType.PLUS || opType == JavaTokenType.MINUS || opType ==
					JavaTokenType.LT || opType == JavaTokenType.LE || opType == JavaTokenType.GT || opType == JavaTokenType.GE || opType == JavaTokenType.AND || opType == JavaTokenType.XOR || opType
					== JavaTokenType.OR;
		...
		public void visitReferenceExpression(PsiReferenceExpression expression)
		{
			...
			else if(element instanceof PsiField)
			{
				...
				{  // myContextPsiClass != fieldClass && myContextPsiClass is not a subclass of fieldClass
					int iterationCount = 0;
		...
		public void visitPostfixExpression(PsiPostfixExpression expression)
		{
			...
			Evaluator incrementImpl = createBinaryEvaluator(operandEvaluator, operandType, new LiteralEvaluator(Integer.valueOf(1), "int"), PsiType.INT, operation == JavaTokenType.PLUSPLUS ?
					JavaTokenType.PLUS : JavaTokenType.MINUS, unboxedOperandType != null ? unboxedOperandType : operandType);
		...
		public void visitPrefixExpression(final PsiPrefixExpression expression)
		{
			...
			if(operation == JavaTokenType.PLUSPLUS || operation == JavaTokenType.MINUSMINUS)
			{
				try
				{
					final Evaluator rightEval = createBinaryEvaluator(operandEvaluator, operandType, new LiteralEvaluator(Integer.valueOf(1), "int"), PsiType.INT, operation == JavaTokenType.PLUSPLUS
							? JavaTokenType.PLUS : JavaTokenType.MINUS, unboxedOperandType != null ? unboxedOperandType : operandType);
		...
		public void visitTypeCastExpression(PsiTypeCastExpression expression)
		{
			...
			{
				throw new EvaluateRuntimeException(new EvaluateException(JavaErrorMessages.message("inconvertible.type.cast", JavaHighlightUtil.formatType(operandType), JavaHighlightUtil.formatType
						(castType))));
			}
