...
public class PositionHighlighter
{
	...
	protected static class ExecutionPointDescription extends SelectionDescription
	{
		...
		public void select()
		{
			...
			myHighlighter = myEditor.getMarkupModel().addLineHighlighter(myLineIndex, DebuggerColors.EXECUTION_LINE_HIGHLIGHTERLAYER,
	...
	protected abstract static class SelectionDescription
	{
		...
		public static ExecutionPointDescription createExecutionPoint(final Editor editor, final int lineIndex)
		{
		...
		public static SelectionDescription createSelection(final Editor editor, final int lineIndex)
		{
			return new SelectionDescription(editor)
			{
				...
				public void select()
				{
					...
					editor.getSelectionModel().setSelection(doc.getLineStartOffset(lineIndex), doc.getLineEndOffset(lineIndex) + doc
							.getLineSeparatorLength(lineIndex));
				}
	...
	{
		if(myContext.getDebuggerSession() == null)
		{
			return;
		}
		...
		private SourcePosition getHighlightPosition(final List<Pair<Breakpoint, Event>> events, SourcePosition position)
		{
			for(Iterator<Pair<Breakpoint, Event>> iterator = events.iterator(); iterator.hasNext(); )
			{
				...
				else if(breakpoint instanceof MethodBreakpoint)
				{
					...
					if(breakPosition != null && breakPosition.getFile().equals(position.getFile()) && breakPosition.getLine() != position.getLine()
