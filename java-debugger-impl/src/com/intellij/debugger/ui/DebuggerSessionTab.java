...
public class DebuggerSessionTab extends DebuggerSessionTabBase implements Disposable
{
			...
			@NotNull DebuggerSession debuggerSession)
	{
		...
		// watches
		Content watches = myUi.createContent(DebuggerContentInfo.WATCHES_CONTENT, myWatchPanel, XDebuggerBundle.message("debugger.session.tab" +
				".watches.title"), AllIcons.Debugger.Watches, null);
		watches.setCloseable(false);
		watches.setAlertIcon(breakpointAlert);
		myUi.addContent(watches, 0, PlaceInGrid.right, false);

		// frames
		Content framesContent = myUi.createContent(DebuggerContentInfo.FRAME_CONTENT, myFramesPanel, XDebuggerBundle.message("debugger.session.tab" +
				".frames.title"), AllIcons.Debugger.Frame, myFramesPanel.getFramesList());
		...
		Content threadsContent = myUi.createContent(DebuggerContentInfo.THREADS_CONTENT, myThreadsPanel, XDebuggerBundle.message("debugger.session" +
				".tab.threads.title"), AllIcons.Debugger.Threads, null);
	...
	private void initUI(ExecutionResult executionResult)
	{
		...
		if(console == null)
		{
			console = myUi.createContent(DebuggerContentInfo.CONSOLE_CONTENT, myConsole.getComponent(), XDebuggerBundle.message("debugger.session" +
					".tab.console.content.name"), AllIcons.Debugger.Console, myConsole.getPreferredFocusableComponent());
