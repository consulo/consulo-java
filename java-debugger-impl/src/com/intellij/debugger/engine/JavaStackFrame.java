...
public class JavaStackFrame extends XStackFrame
{
	...
	DebuggerContextImpl getFrameDebuggerContext()
	{
		...
		{
			context = DebuggerContextImpl.createDebuggerContext(myDebugProcess.mySession, context.getSuspendContext(),
				...
				if(thisObjectReference != null && evaluationContext.getDebugProcess().getVirtualMachineProxy().canGetSyntheticAttribute())
				{
					...
					{ // makes sense for nested classes only
						...
						for(Field field : clsType.fields())
						{
							...
							{
								final FieldDescriptorImpl fieldDescriptor = myNodeManager.getFieldDescriptor(stackDescriptor, thisObjectReference,
										field);
