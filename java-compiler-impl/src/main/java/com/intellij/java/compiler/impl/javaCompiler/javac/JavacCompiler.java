...
public class JavacCompiler implements BackendCompiler {
  ...
  public static void addCommandLineOptions(
    ...
    PsiJavaModule javaModule = isJava9Version ? JavaModuleGraphUtil.findDescriptorByModule(chunk.getModule(),
                                                                                           chunk.getSourcesFilter() == ModuleChunk.TEST_SOURCES) : null;
    ...
    if (isAnnotationProcessingMode) {
      ...
      final String moduleOutputPath =
        CompilerPaths.getModuleOutputPath(chunk.getModules()[0], ProductionContentFolderTypeProvider.getInstance());
