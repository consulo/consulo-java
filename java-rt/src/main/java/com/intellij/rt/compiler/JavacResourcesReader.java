...
public class JavacResourcesReader implements JavacResourcesReaderConstans
{
	private static final String[] BUNDLE_NAMES = new String[]{
			...
			// v1.5
			"com.sun.tools.javac.v8.resources.compiler",
			// v1.3-1.4
	...
	private static final BundleKey[] MSG_NAME_KEY_PAIRS = new BundleKey[]{
			...
			new BundleKey(MSG_PARSING_COMPLETED, "benv.parsed_in"),
			// jdk 1.1-1.2
			new BundleKey(MSG_LOADING, "compiler.misc.verbose.loading"),
			new BundleKey(MSG_LOADING, "benv.loaded_in"),
			// jdk 1.1-1.2
			...
			new BundleKey(MSG_WROTE, "main.wrote"),
			// jdk 1.1-1.2
			new BundleKey(MSG_WARNING, "compiler.warn.warning"),
			new BundleKey(MSG_NOTE, new String[]{
					"compiler.note.note",
			...
			new BundleKey(MSG_STATISTICS, "main.errors"),
			//jdk 1.1 - 1.2
			new BundleKey(MSG_STATISTICS, "main.warnings"),
			//jdk 1.1 - 1.2
			new BundleKey(MSG_STATISTICS, "main.1error"),
			//jdk 1.1 - 1.2
			new BundleKey(MSG_STATISTICS, "main.1warning"),
			//jdk 1.1 - 1.2
			new IgnoredWarningBundleKey("compiler.warn.dir.path.element.not.found"),
			//jdk 1.5
  ...
  */
	...
	public static boolean dumpPatterns()
	{
		...
		try
		{
			Class listResourceClass = Class.forName("com.sun.tools.javac.resources.compiler");
			if(ListResourceBundle.class.isAssignableFrom(listResourceClass))
			{
				messagesBundle = (ResourceBundle) listResourceClass.newInstance();
			}
		}
	...
	private static class IgnoredWarningBundleKey extends BundleKey
	{
		...
		{
			super(JavacResourcesReader.MSG_IGNORED, new String[]{
