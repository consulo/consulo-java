...
public class DataFlowInspection extends DataFlowInspectionBase
{
	...
	private class OptionsPanel extends JPanel
	{
		...
		private OptionsPanel()
		{
			...
			mySuggestNullables = createCheckBoxWithHTML(message("inspection.data.flow.nullable.quickfix.option"), SUGGEST_NULLABLE_ANNOTATIONS, box -> SUGGEST_NULLABLE_ANNOTATIONS = box.isSelected
					());

			myDontReportTrueAsserts = createCheckBoxWithHTML(message("inspection.data.flow.true.asserts.option"), DONT_REPORT_TRUE_ASSERT_STATEMENTS, box -> DONT_REPORT_TRUE_ASSERT_STATEMENTS = box
					.isSelected());

			myIgnoreAssertions = createCheckBoxWithHTML("Ignore assert statements", IGNORE_ASSERT_STATEMENTS, box -> IGNORE_ASSERT_STATEMENTS = box.isSelected());

			myReportConstantReferences = createCheckBoxWithHTML("Warn when reading a value guaranteed to be constant", REPORT_CONSTANT_REFERENCE_VALUES, box -> REPORT_CONSTANT_REFERENCE_VALUES = box
					.isSelected());

			myTreatUnknownMembersAsNullable = createCheckBoxWithHTML("Treat non-annotated members and parameters as @Nullable", TREAT_UNKNOWN_MEMBERS_AS_NULLABLE, box ->
					TREAT_UNKNOWN_MEMBERS_AS_NULLABLE = box.isSelected());

			myReportNullArguments = createCheckBoxWithHTML("Report not-null required parameter with null-literal argument usages", REPORT_NULLS_PASSED_TO_NOT_NULL_PARAMETER, box ->
					REPORT_NULLS_PASSED_TO_NOT_NULL_PARAMETER = box.isSelected());

			myReportNullableMethodsReturningNotNull = createCheckBoxWithHTML("Report nullable methods that always return a non-null value", REPORT_NULLABLE_METHODS_RETURNING_NOT_NULL, box ->
					REPORT_NULLABLE_METHODS_RETURNING_NOT_NULL = box.isSelected());

			myReportUncheckedOptionals = createCheckBoxWithHTML("Report Optional.get() calls without previous isPresent check", REPORT_UNCHECKED_OPTIONALS, box -> REPORT_UNCHECKED_OPTIONALS = box
					.isSelected());
