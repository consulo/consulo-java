        ...
        if (ownerClass != null) {
          switch (type) {
            ...
            case GET_METHOD: {
              ...
              if (methods.length > 1) {
                methods =
                    ContainerUtil.filter(methods, method -> isRegularMethod(method) && isPublic(method)).toArray(PsiMethod.EMPTY_ARRAY);
                if (methods.length > 1) {
            ...
            case GET_DECLARED_METHOD: {
              ...
              if (methods.length > 1) {
                methods =
                    ContainerUtil.filter(methods, method -> isRegularMethod(method) && isPotentiallyAccessible(method, ownerClass)).toArray(PsiMethod.EMPTY_ARRAY);
                if (methods.length > 1) {
                  return findOverloadedMethod(methods);
                }
              }
