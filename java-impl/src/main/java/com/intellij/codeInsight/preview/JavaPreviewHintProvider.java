...
public class JavaPreviewHintProvider implements PreviewHintProvider
{
	...
	private static final PsiMethodPattern GET_COLOR_METHOD = PsiJavaPatterns.psiMethod().definedInClass(Color.class.getName()).withName("getColor");
	private static final PsiExpressionPattern.Capture<PsiExpression> GET_METHOD_CALL_PARAMETER = PsiJavaPatterns.psiExpression().methodCallParameter(0, GET_COLOR_METHOD);
	...
	private static boolean isInsideDecodeOrGetColorMethod(PsiElement element)
	{
		...
		return DECODE_METHOD_CALL_PARAMETER.accepts(element) || GET_METHOD_CALL_PARAMETER.accepts(element);
	}
