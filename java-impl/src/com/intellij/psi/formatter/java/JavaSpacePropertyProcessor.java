...
public class JavaSpacePropertyProcessor extends JavaElementVisitor
{
	...
	{
		if(ElementType.IMPORT_STATEMENT_BASE_BIT_SET.contains(myChild1.getElementType()) && ElementType.IMPORT_STATEMENT_BASE_BIT_SET.contains(myChild2.getElementType()))
		{
			if(myImportHelper == null)
			{
				myImportHelper = new ImportHelper(mySettings.getRootSettings());
			}
			int emptyLines = myImportHelper.getEmptyLinesBetween(SourceTreeToPsiMap.treeToPsiNotNull(myChild1), SourceTreeToPsiMap.treeToPsiNotNull(myChild2)) + 1;
			myResult = Spacing.createSpacing(0, 0, emptyLines, mySettings.KEEP_LINE_BREAKS, mySettings.KEEP_BLANK_LINES_IN_DECLARATIONS);
		}
	...
	public void visitTryStatement(PsiTryStatement statement)
	{
		if(myRole2 == ChildRole.FINALLY_KEYWORD || myRole2 == ChildRole.CATCH_SECTION)
		{
			...
			{
				boolean useSpace = myRole2 == ChildRole.CATCH_SECTION && mySettings.SPACE_BEFORE_CATCH_KEYWORD || myRole2 == ChildRole.FINALLY_KEYWORD && mySettings.SPACE_BEFORE_FINALLY_KEYWORD;
	...
	public void visitForeachStatement(PsiForeachStatement statement)
	{
		...
		else if(myRole1 == ChildRole.FOR_ITERATION_PARAMETER && myRole2 == ChildRole.COLON || myRole1 == ChildRole.COLON && myRole2 == ChildRole.FOR_ITERATED_VALUE)
		{
			createSpaceInCode(true);
		}
		else if(myRole2 == ChildRole.LOOP_BODY)
	...
	private void processCodeBlock(final boolean keepInOneLine, final TextRange textRange)
	{
		...
		else if(lhsStatement && rhsStatement)
		{
			...
			if((mySettings.KEEP_MULTIPLE_EXPRESSIONS_IN_ONE_LINE && (FormatterUtil.isFormatterCalledExplicitly() || ApplicationManager.getApplication().isUnitTestMode())) || psi != null &&
					PsiTreeUtil.hasErrorElements(psi))
			{
				minSpaces = 1;
				minLineFeeds = 0;
				if(myChild1 != null)
	...
	public void visitLocalVariable(PsiLocalVariable variable)
	{
		...
		else if(myRole1 == ChildRole.MODIFIER_LIST || myRole2 == ChildRole.TYPE_REFERENCE || myRole1 == ChildRole.TYPE_REFERENCE || myRole2 == ChildRole.TYPE || myRole1 == ChildRole.TYPE)
		{
			createSpaceInCode(true);
		}
		else if(myChild2.getElementType() == JavaTokenType.SEMICOLON)
	...
	private void createParenthSpace(final boolean onNewLine, final boolean space, final TextRange dependence)
	{
		if(onNewLine)
		{
			...
			myResult = Spacing.createDependentLFSpacing(spaces, spaces, dependence, mySettings.KEEP_LINE_BREAKS, mySettings.KEEP_BLANK_LINES_IN_CODE);
		}
		else
		{
			createSpaceInCode(space);
	...
	public void visitElement(PsiElement element)
	{
		...
		else if((myType1 == JavaDocTokenType.DOC_TAG_VALUE_TOKEN || myType1 == JavaDocElementType.DOC_TAG_VALUE_ELEMENT) && (myType2 == JavaDocTokenType.DOC_TAG_VALUE_TOKEN || myType2 ==
	...
	{
		if(myRole1 == ChildRole.TYPE && myRole2 == ChildRole.DOT || myRole1 == ChildRole.DOT && myRole2 == ChildRole.CLASS_KEYWORD)
		{
			createSpaceInCode(false);
		}
	}
	...
	{
		if((myRole1 == ChildRole.DOUBLE_COLON && myRole2 == ChildRole.REFERENCE_NAME) || (myRole1 == ChildRole.EXPRESSION && myRole2 == ChildRole.DOUBLE_COLON))
		{
	...
	{
		if(myRole1 == ChildRole.LT_IN_TYPE_LIST && myRole2 == ChildRole.GT_IN_TYPE_LIST || myRole1 == ChildRole.TYPE_IN_REFERENCE_PARAMETER_LIST && myRole2 == ChildRole.COMMA)
		{
			createSpaceInCode(false);
		}
	...
	public void visitReferenceList(PsiReferenceList list)
	{
		...
		else if(myRole1 == ChildRole.EXTENDS_KEYWORD || myRole2 == ChildRole.EXTENDS_KEYWORD || myRole1 == ChildRole.IMPLEMENTS_KEYWORD || myRole2 == ChildRole.IMPLEMENTS_KEYWORD || myRole1 ==
				ChildRole.THROWS_KEYWORD)
		{
			createSpaceInCode(true);
		}
	}
	...
	private void visitArrayInitializer()
	{
		if(myRole1 == ChildRole.LBRACE)
		{
			if(mySettings.ARRAY_INITIALIZER_LBRACE_ON_NEXT_LINE)
			{
				...
				myResult = Spacing.createDependentLFSpacing(spaces, spaces, myParent.getTextRange(), mySettings.KEEP_LINE_BREAKS, mySettings.KEEP_BLANK_LINES_IN_CODE);
			}
			else
			{
				boolean addSpace = (myRole2 != ChildRole.RBRACE) ? mySettings.SPACE_WITHIN_ARRAY_INITIALIZER_BRACES : mySettings.SPACE_WITHIN_EMPTY_ARRAY_INITIALIZER_BRACES;
		...
		else if(myRole2 == ChildRole.RBRACE)
		{
			if(mySettings.ARRAY_INITIALIZER_RBRACE_ON_NEXT_LINE)
			{
				...
				myResult = Spacing.createDependentLFSpacing(spaces, spaces, myParent.getTextRange(), mySettings.KEEP_LINE_BREAKS, mySettings.KEEP_BLANK_LINES_BEFORE_RBRACE);
			}
			else
			{
	...
	{
		OK, INCORRECT, RESTRICTED_KEYWORD
	}
