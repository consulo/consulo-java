...
public class JavaCompletionProcessor extends BaseScopeProcessor implements ElementClassHint
{
	...
	public boolean isAccessible(@Nullable final PsiElement element)
	{
		...
		if(JavaPsiFacade.getInstance(element.getProject()).getResolveHelper().isAccessible(member, member.getModifierList(), myElement, accessObjectClass, myDeclarationHolder))
		{
			return true;
		}
	...
	{
		none, staticAfterInstance, instanceAfterStatic
	}
}
