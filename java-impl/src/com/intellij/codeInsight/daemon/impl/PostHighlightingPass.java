...
public class PostHighlightingPass extends ProgressableTextEditorHighlightingPass
{
	...
	protected void collectInformationWithProgress(@NotNull final ProgressIndicator progress)
	{
		...
		for(Language language : relevantLanguages)
		{
			...
			if(!HighlightingLevelManager.getInstance(myProject).shouldHighlight(psiRoot))
			{
				continue;
			}
	...
	protected void applyInformationWithProgress()
	{
		...
		UpdateHighlightersUtil.setHighlightersToEditor(myProject, myDocument, myStartOffset, myEndOffset, myHighlights, getColorsScheme(),
		...
		ApplicationManager.getApplication().invokeLater(new Runnable()
		{
			...
			public void run()
			{
				...
				if(oldStamp != editor.getDocument().getModificationStamp())
				{
					...
					{
						LOG.error(LogMessageEx.createEvent("Import optimizer  hasn't optimized any imports", file.getViewProvider().getVirtualFile()
								.getPath(), AttachmentFactory.createAttachment(file.getViewProvider().getVirtualFile())));
					}
				}
			}
		});
	...
	private boolean collectHighlights(@NotNull Collection<PsiElement> elements, @NotNull final List<HighlightInfo> result,
   ...
   /* if (unusedImportEnabled && JspPsiUtil.isInJspFile(myFile)) {
      ...
      if (jspFile != null) {
		...
		myDeadCodeInfoType = myDeadCodeKey == null ? null : new HighlightInfoType.HighlightInfoTypeImpl(profile.getErrorLevel(myDeadCodeKey,
				myFile).getSeverity(), HighlightInfoType.UNUSED_SYMBOL.getAttributesKey());
	...
	private HighlightInfo processIdentifier(PsiIdentifier identifier, ProgressIndicator progress, GlobalUsageHelper helper)
	{
		...
		{
			if(SuppressionUtil.isSuppressed(identifier, UnusedParametersInspection.SHORT_NAME))
			{
				return null;
			}
			...
			@NotNull ProgressIndicator progress)
	{
		...
		if(isImplicitUsage(variable, progress))
		{
			return null;
		}
	...
	{
		if(UnusedSymbolLocalInspection.isInjected(element))
		{
			return true;
		}
	...
	public static HighlightInfo createUnusedSymbolInfo(@NotNull PsiElement element, @NotNull String message,
	...
	@Nullable
	private HighlightInfo processField(@NotNull final PsiField field, @NotNull PsiIdentifier identifier, @NotNull ProgressIndicator progress,
			@NotNull GlobalUsageHelper helper)
	{
		...
		if(field.hasModifierProperty(PsiModifier.PRIVATE))
		{
			...
			if(!writeReferenced && !isImplicitWrite(field, progress))
			{
				...
				QuickFixAction.registerQuickFixAction(info, HighlightMethodUtil.getFixRange(field), new CreateConstructorParameterFromFieldFix
						(field));
				SpecialAnnotationsUtilBase.createAddToSpecialAnnotationFixes(field, new Processor<String>()
				{
					...
					{
						QuickFixAction.registerQuickFixAction(info, UnusedSymbolLocalInspection.createQuickFix(annoName, "fields",
								field.getProject()));
						return true;
					}
				});
	...
	@Nullable
	private HighlightInfo processParameter(@NotNull PsiParameter parameter, @NotNull PsiIdentifier identifier, @NotNull ProgressIndicator progress)
	{
		...
		if(declarationScope instanceof PsiMethod)
		{
			...
			{
				if(UnusedSymbolLocalInspection.isInjected(method))
				{
					return null;
				}
	...
	@Nullable
	private HighlightInfo checkUnusedParameter(@NotNull PsiParameter parameter, @NotNull PsiIdentifier identifier,
			@NotNull ProgressIndicator progress)
	{
	...
	@Nullable
	private HighlightInfo processMethod(@NotNull final PsiMethod method, @NotNull PsiIdentifier identifier, @NotNull ProgressIndicator progress,
			@NotNull GlobalUsageHelper helper)
	{
		if(isMethodReferenced(method, progress, helper))
		{
			return null;
		}
		...
		SpecialAnnotationsUtilBase.createAddToSpecialAnnotationFixes(method, new Processor<String>()
		{
			...
			public boolean process(final String annoName)
			{
				QuickFixAction.registerQuickFixAction(highlightInfo, UnusedSymbolLocalInspection.createQuickFix(annoName, "methods",
						method.getProject()));
				return true;
			}
		});
	...
	public static boolean isMethodReferenced(PsiMethod method, ProgressIndicator progress, GlobalUsageHelper helper)
	{
		...
		else
		{
			...
			if(containingClass != null && method.isConstructor() && containingClass.getConstructors().length == 1 && isClassUsed(containingClass,
					progress, helper))
			{
				return true;
			}
	...
	private static boolean weAreSureThereAreNoUsages(@NotNull PsiMember member, ProgressIndicator progress, GlobalUsageHelper helper)
	{
		...
		if(useScope instanceof GlobalSearchScope)
		{
			...
			{
				if(!canBeReferencedViaWeirdNames(member))
				{
					return true;
				}
			}
			...
			if(member instanceof PsiMethod)
			{
				...
				if(propertyName != null && file != null)
				{
					...
					if(fileScope instanceof GlobalSearchScope && searchHelper.isCheapEnoughToSearch(propertyName, (GlobalSearchScope) fileScope,
							ignoreFile, progress) == TOO_MANY_OCCURRENCES)
					{
						return false;
					}
				}
		...
		if(ReferencesSearch.search(member, useScope, true).findFirst() != null)
		{
			return false;
		}
	...
	private static boolean foundUsageInText(@NotNull PsiMember member, @NotNull GlobalSearchScope scope, @NotNull PsiSearchHelper searchHelper,
	...
	@Nullable
	private HighlightInfo processClass(@NotNull PsiClass aClass, @NotNull PsiIdentifier identifier, @NotNull ProgressIndicator progress,
			@NotNull GlobalUsageHelper helper)
	{
		if(isClassUsed(aClass, progress, helper))
		{
			return null;
		}
	...
	{
		if(isImplicitUsage(aClass, progress) || helper.isLocallyUsed(aClass))
		{
			return true;
		}
		if(helper.isCurrentFileAlreadyChecked())
	...
	private static HighlightInfo formatUnusedSymbolHighlightInfo(@NotNull @PropertyKey(resourceBundle = JavaErrorMessages.BUNDLE) String pattern,
			@NotNull final PsiNameIdentifierOwner aClass, @NotNull final String element, @NotNull HighlightDisplayKey highlightDisplayKey,
			@NotNull HighlightInfoType highlightInfoType, @NotNull PsiElement identifier)
	{
		...
		SpecialAnnotationsUtilBase.createAddToSpecialAnnotationFixes((PsiModifierListOwner) aClass, new Processor<String>()
		{
			...
			public boolean process(final String annoName)
			{
				QuickFixAction.registerQuickFixAction(highlightInfo, UnusedSymbolLocalInspection.createQuickFix(annoName, element,
						aClass.getProject()));
				return true;
			}
		});
	...
	private HighlightInfo registerRedundantImport(@NotNull PsiImportStatementBase importStatement, @NotNull HighlightDisplayKey unusedImportKey)
	{
		...
		HighlightInfo info = HighlightInfo.newHighlightInfo(JavaHighlightInfoTypes.UNUSED_IMPORT).range(importStatement).descriptionAndTooltip
				(description).create();
	...
	private boolean timeToOptimizeImports()
	{
		...
		if(file == null || !codeAnalyzer.isHighlightingAvailable(file) || !(file instanceof PsiJavaFile) || file instanceof ServerPageFile)
		{
			return false;
		}
	...
	private boolean containsErrorsPreventingOptimize(@NotNull PsiFile file)
	{
		...
		boolean hasErrorsExceptUnresolvedImports = !DaemonCodeAnalyzerEx.processHighlights(myDocument, myProject, HighlightSeverity.ERROR, 0,
