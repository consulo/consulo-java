...
public class HighlightUtil extends HighlightUtilBase
{
	...
	static
	{
		...
		ourClassIncompatibleModifiers.put(PsiModifier.PACKAGE_LOCAL, ContainerUtil.newTroveSet(PsiModifier.PRIVATE, PsiModifier.PUBLIC,
				PsiModifier.PROTECTED));
		ourClassIncompatibleModifiers.put(PsiModifier.PRIVATE, ContainerUtil.newTroveSet(PsiModifier.PACKAGE_LOCAL, PsiModifier.PUBLIC,
				PsiModifier.PROTECTED));
		ourClassIncompatibleModifiers.put(PsiModifier.PUBLIC, ContainerUtil.newTroveSet(PsiModifier.PACKAGE_LOCAL, PsiModifier.PRIVATE,
				PsiModifier.PROTECTED));
		ourClassIncompatibleModifiers.put(PsiModifier.PROTECTED, ContainerUtil.newTroveSet(PsiModifier.PACKAGE_LOCAL, PsiModifier.PUBLIC,
				PsiModifier.PRIVATE));
		...
		ourInterfaceIncompatibleModifiers.put(PsiModifier.PACKAGE_LOCAL, ContainerUtil.newTroveSet(PsiModifier.PRIVATE, PsiModifier.PUBLIC,
				PsiModifier.PROTECTED));
		ourInterfaceIncompatibleModifiers.put(PsiModifier.PRIVATE, ContainerUtil.newTroveSet(PsiModifier.PACKAGE_LOCAL, PsiModifier.PUBLIC,
				PsiModifier.PROTECTED));
		ourInterfaceIncompatibleModifiers.put(PsiModifier.PUBLIC, ContainerUtil.newTroveSet(PsiModifier.PACKAGE_LOCAL, PsiModifier.PRIVATE,
				PsiModifier.PROTECTED));
		ourInterfaceIncompatibleModifiers.put(PsiModifier.PROTECTED, ContainerUtil.newTroveSet(PsiModifier.PACKAGE_LOCAL, PsiModifier.PUBLIC,
				PsiModifier.PRIVATE));
		...
		ourMethodIncompatibleModifiers.put(PsiModifier.ABSTRACT, ContainerUtil.newTroveSet(PsiModifier.NATIVE, PsiModifier.STATIC,
				PsiModifier.FINAL, PsiModifier.PRIVATE, PsiModifier.STRICTFP, PsiModifier.SYNCHRONIZED, PsiModifier.DEFAULT));
		...
		ourMethodIncompatibleModifiers.put(PsiModifier.PACKAGE_LOCAL, ContainerUtil.newTroveSet(PsiModifier.PRIVATE, PsiModifier.PUBLIC,
				PsiModifier.PROTECTED));
		ourMethodIncompatibleModifiers.put(PsiModifier.PRIVATE, ContainerUtil.newTroveSet(PsiModifier.PACKAGE_LOCAL, PsiModifier.PUBLIC,
				PsiModifier.PROTECTED));
		ourMethodIncompatibleModifiers.put(PsiModifier.PUBLIC, ContainerUtil.newTroveSet(PsiModifier.PACKAGE_LOCAL, PsiModifier.PRIVATE,
				PsiModifier.PROTECTED));
		ourMethodIncompatibleModifiers.put(PsiModifier.PROTECTED, ContainerUtil.newTroveSet(PsiModifier.PACKAGE_LOCAL, PsiModifier.PUBLIC,
				PsiModifier.PRIVATE));
		...
		ourFieldIncompatibleModifiers.put(PsiModifier.PACKAGE_LOCAL, ContainerUtil.newTroveSet(PsiModifier.PRIVATE, PsiModifier.PUBLIC,
				PsiModifier.PROTECTED));
		ourFieldIncompatibleModifiers.put(PsiModifier.PRIVATE, ContainerUtil.newTroveSet(PsiModifier.PACKAGE_LOCAL, PsiModifier.PUBLIC,
				PsiModifier.PROTECTED));
		ourFieldIncompatibleModifiers.put(PsiModifier.PUBLIC, ContainerUtil.newTroveSet(PsiModifier.PACKAGE_LOCAL, PsiModifier.PRIVATE,
				PsiModifier.PROTECTED));
		ourFieldIncompatibleModifiers.put(PsiModifier.PROTECTED, ContainerUtil.newTroveSet(PsiModifier.PACKAGE_LOCAL, PsiModifier.PUBLIC,
				PsiModifier.PRIVATE));
		...
		ourConstructorNotAllowedModifiers = ContainerUtil.newTroveSet(PsiModifier.ABSTRACT, PsiModifier.STATIC, PsiModifier.NATIVE,
	...
	static HighlightInfo checkInstanceOfApplicable(@NotNull PsiInstanceOfExpression expression)
	{
		...
		{
			return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR).range(expression).descriptionAndTooltip("Lambda expression is not " +
					"expected here").create();
		}
		if(operandType instanceof PsiMethodReferenceType)
		{
			return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR).range(expression).descriptionAndTooltip("Method reference expression is " +
					"not expected here").create();
		}
		if(TypeConversionUtil.isPrimitiveAndNotNull(operandType) || TypeConversionUtil.isPrimitiveAndNotNull(checkType) || !TypeConversionUtil
				.areTypesConvertible(operandType, checkType))
		{
			String message = JavaErrorMessages.message("inconvertible.type.cast", JavaHighlightUtil.formatType(operandType),
	...
	static HighlightInfo checkVariableExpected(@NotNull PsiExpression expression)
	{
		...
		{
			lValue = expression instanceof PsiPostfixExpression ? ((PsiPostfixExpression) expression).getOperand() : ((PsiPrefixExpression)
	...
	static HighlightInfo checkAssignmentOperatorApplicable(@NotNull PsiAssignmentExpression assignment, @NotNull PsiFile containingFile)
	{
		...
		HighlightInfo errorResult = null;
		if(!TypeConversionUtil.isBinaryOperatorApplicable(opSign, lType, rType, true) || PsiType.getJavaLangObject(containingFile.getManager(),
				assignment.getResolveScope()).equals(lType))
		{
			String operatorText = operationSign.getText().substring(0, operationSign.getText().length() - 1);
			String message = JavaErrorMessages.message("binary.operator.not.applicable", operatorText, JavaHighlightUtil.formatType(lType),
			...
			if(returnValue != null)
			{
				...
				else
				{
					...
					if(errorResult != null && valueType != null)
					{
						...
						if(returnType instanceof PsiArrayType && TypeConversionUtil.isAssignable(((PsiArrayType) returnType).getComponentType(),
								valueType))
						{
	...
	static HighlightInfo checkVariableAlreadyDefined(@NotNull PsiVariable variable)
	{
		...
		if(variable instanceof PsiLocalVariable || variable instanceof PsiParameter && ((declarationScope = ((PsiParameter) variable)
		...
		{
			@SuppressWarnings("unchecked") PsiElement scope = PsiTreeUtil.getParentOfType(variable, PsiFile.class, PsiMethod.class,
					PsiClassInitializer.class, PsiResourceList.class);
		...
		if(isIncorrect)
		{
			...
			PsiIdentifier identifier = variable.getNameIdentifier();
			assert identifier != null : variable;
			HighlightInfo highlightInfo = HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR).range(identifier).descriptionAndTooltip
					(description).create();
			if(variable instanceof PsiLocalVariable)
			{
				QuickFixAction.registerQuickFixAction(highlightInfo, QUICK_FIX_FACTORY.createReuseVariableDeclarationFix((PsiLocalVariable)
						variable));
			}
			return highlightInfo;
		}
	...
	public static HighlightInfo checkUnhandledCloserExceptions(@NotNull final PsiResourceVariable resource)
	{
		...
		final HighlightInfo highlight = HighlightInfo.newHighlightInfo(highlightType).range(resource).descriptionAndTooltip(description).create();
	...
	static HighlightInfo checkIllegalModifierCombination(@NotNull PsiKeyword keyword, @NotNull PsiModifierList modifierList)
	{
		...
		if(incompatible != null)
		{
			...
			HighlightInfo highlightInfo = HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR).range(keyword).descriptionAndTooltip(message)
					.create();
	...
	private static Map<String, Set<String>> getIncompatibleModifierMap(@NotNull PsiModifierList modifierList)
	{
		...
		return parent instanceof PsiClass ? ((PsiClass) parent).isInterface() ? ourInterfaceIncompatibleModifiers : ourClassIncompatibleModifiers :
				parent instanceof PsiMethod ? ourMethodIncompatibleModifiers : parent instanceof PsiVariable ? ourFieldIncompatibleModifiers :
	...
	public static HighlightInfo checkNotAllowedModifier(@NotNull PsiKeyword keyword, @NotNull PsiModifierList modifierList)
	{
		...
		PsiElement modifierOwnerParent = modifierOwner instanceof PsiMember ? ((PsiMember) modifierOwner).getContainingClass() : modifierOwner
				.getParent();
		...
		if(modifierOwner instanceof PsiClass)
		{
			...
			else
			{
				...
				{
					isAllowed = modifierOwnerParent instanceof PsiJavaFile || modifierOwnerParent instanceof PsiClass && (modifierOwnerParent
							instanceof PsiSyntheticClass || ((PsiClass) modifierOwnerParent).getQualifiedName() != null);
				}
		...
		else if(type == JavaTokenType.CHARACTER_LITERAL)
		{
			...
			else
			{
				...
				if(length > 1)
				{
					...
					final HighlightInfo info = HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR).range(expression).descriptionAndTooltip
							(message).create();
	...
	private static HighlightInfo checkUnderscores(PsiElement expression, String text, boolean isInt)
	{
		...
		if(isInt)
		{
			int start = 0;
			int end = text.length();
			if(text.startsWith(PsiLiteralExpressionImpl.HEX_PREFIX) || text.startsWith(PsiLiteralExpressionImpl.BIN_PREFIX))
			{
				start += 2;
			}
			...
			@NotNull final PsiClassType caughtType)
	{
		...
		final HighlightInfo errorResult = HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR).range(parameter).descriptionAndTooltip
				(description).create();
			...
			@NotNull final Collection<PsiClassType> thrownTypes)
	{
		...
		for(final PsiTypeElement typeElement : typeElements)
		{
			...
			if(!used)
			{
				...
				final HighlightInfo highlight = HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR).range(typeElement).descriptionAndTooltip
						(description).create();
			...
			@NotNull PsiFile containingFile)
	{
		...
		for(PsiTypeElement catchTypeElement : parameterTypeElements)
		{
			...
			if(caught.isEmpty())
			{
				...
				final HighlightInfo highlightInfo = HighlightInfo.newHighlightInfo(HighlightInfoType.WARNING).range(catchSection)
	...
	public static HighlightInfo checkSwitchSelectorType(@NotNull PsiSwitchStatement statement)
	{
		...
		{
			String message = JavaErrorMessages.message("incompatible.types", JavaErrorMessages.message("valid.switch.selector.types"),
		...
		else
		{
			...
			{
				String message = PsiFormatUtil.formatClass(member, PsiFormatUtilBase.SHOW_NAME | PsiFormatUtilBase.SHOW_FQ_NAME) + " is inaccessible" +
						" here";
				errorResult = HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR).range(expression).descriptionAndTooltip(message).create();
			}
		}
	...
	static HighlightInfo checkPolyadicOperatorApplicable(@NotNull PsiPolyadicExpression expression)
	{
		...
		for(int i = 1; i < operands.length; i++)
		{
			...
			if(!TypeConversionUtil.isBinaryOperatorApplicable(operationSign, lType, rType, false))
			{
				...
				String message = JavaErrorMessages.message("binary.operator.not.applicable", token.getText(), JavaHighlightUtil.formatType(lType),
						JavaHighlightUtil.formatType(rType));
				return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR).range(expression).descriptionAndTooltip(message).create();
			}
	...
	static HighlightInfo checkUnaryOperatorApplicable(@Nullable PsiJavaToken token, @Nullable PsiExpression expression)
	{
		if(token != null && expression != null && !TypeConversionUtil.isUnaryOperatorApplicable(token, expression))
		{
			...
			HighlightInfo highlightInfo = HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR).range(parentExpr).descriptionAndTooltip(message)
					.create();
			if(parentExpr instanceof PsiPrefixExpression && token.getTokenType() == JavaTokenType.EXCL)
			{
				QuickFixAction.registerQuickFixAction(highlightInfo, QUICK_FIX_FACTORY.createNegationBroadScopeFix((PsiPrefixExpression)
						parentExpr));
			}
			return highlightInfo;
		}
	...
	@Nullable
	static HighlightInfo checkThisOrSuperExpressionInIllegalContext(@NotNull PsiExpression expr, @Nullable PsiJavaCodeReferenceElement qualifier)
	{
		...
		if(!InheritanceUtil.hasEnclosingInstanceInScope(aClass, expr, false, false) && !resolvesToImmediateSuperInterface(expr, qualifier, aClass))
		{
	...
	{
		return HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR).range(expr).descriptionAndTooltip("Cannot find symbol variable this")
				.create();
	}
	...
	{
		if(!(expr instanceof PsiSuperExpression) || qualifier == null || !PsiUtil.isLanguageLevel8OrHigher(expr))
		{
			return false;
		}
			...
			@NotNull PsiJavaCodeReferenceElement place)
	{
		...
		{
			QuickFixAction.registerQuickFixAction(errorResult, QUICK_FIX_FACTORY.createModifierListFix((PsiModifierListOwner) refElement,
		...
		{
			QuickFixAction.registerQuickFixAction(errorResult, QUICK_FIX_FACTORY.createModifierListFix(staticParent, PsiModifier.STATIC, false,
					false));
		}
	...
	static HighlightInfo checkValidArrayAccessExpression(@NotNull PsiArrayAccessExpression arrayAccessExpression)
	{
		...
		if(arrayExpressionType != null && !(arrayExpressionType instanceof PsiArrayType))
		{
			...
			final HighlightInfo info = HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR).range(arrayExpression).descriptionAndTooltip
					(description).create();
	...
	@Nullable
	public static HighlightInfo checkExpressionRequired(@NotNull PsiReferenceExpression expression,
			@NotNull JavaResolveResult resultForIncompleteCode)
	{
		...
		final HighlightInfo info = HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR).range(expression).descriptionAndTooltip(description)
				.create();
    ...
    */
	...
	public static HighlightInfo checkCaseStatement(@NotNull PsiSwitchLabelStatement statement)
	{
		...
		for(PsiStatement st : statements)
		{
			...
			String description = statement.isDefaultCase() ? JavaErrorMessages.message("duplicate.default.switch.label") : JavaErrorMessages.message
					("duplicate.switch.label", value);
	...
	static PsiField findEnclosingFieldInitializer(@Nullable PsiElement element)
	{
		while(element != null)
		{
			...
			if(element instanceof PsiClass || element instanceof PsiMethod || parent instanceof PsiLambdaExpression)
			{
				return null;
			}
			element = parent;
			...
			@NotNull PsiFile containingFile)
	{
		...
		{
			// redirected ctr
			if(PsiKeyword.THIS.equals(((PsiJavaCodeReferenceElement) expression).getReferenceName()) && resolved instanceof PsiMethod && (
			...
			if(resolved instanceof PsiField)
			{
				...
				resolvedName = PsiFormatUtil.formatVariable(referencedField, PsiFormatUtilBase.SHOW_CONTAINING_CLASS | PsiFormatUtilBase.SHOW_NAME,
			...
			else if(resolved instanceof PsiMethod)
			{
				...
				PsiElement nameElement = expression instanceof PsiThisExpression ? expression : ((PsiJavaCodeReferenceElement) expression)
				...
				if(isSuperCall)
				{
					...
					if(qualifier == null)
					{
						...
						if(superClass != null && PsiUtil.isInnerClass(superClass) && InheritanceUtil.isInheritorOrSelf(referencedClass,
								superClass.getContainingClass(), true))
						{
			...
			@NotNull PsiFile containingFile)
	{
		...
		while(element != null)
		{
			...
			{
				PsiElement parentClass = new PsiMatcherImpl(element).parent(PsiMatchers.hasClass(PsiExpressionStatement.class)).parent(PsiMatchers
						.hasClass(PsiCodeBlock.class)).parent(PsiMatchers.hasClass(PsiMethod.class)).dot(JavaMatchers.isConstructor(true)).parent
				...
				if(expression instanceof PsiReferenceExpression && !thisOrSuperReference(((PsiReferenceExpression) expression)
	...
	private static boolean isSuperCalledInConstructor(@NotNull final PsiMethod constructor)
	{
		...
		final PsiElement element = new PsiMatcherImpl(statement).dot(PsiMatchers.hasClass(PsiExpressionStatement.class)).firstChild(PsiMatchers
				.hasClass(PsiMethodCallExpression.class)).firstChild(PsiMatchers.hasClass(PsiReferenceExpression.class)).firstChild(PsiMatchers
				.hasClass(PsiKeyword.class)).dot(PsiMatchers.hasText(PsiKeyword.SUPER)).getElement();
		return element != null;
	}
		...
		for(PsiTypeElement typeElement : typeElements)
		{
			...
			for(int i = startFrom; i >= 0; i--)
			{
				...
				final boolean highlight = upperCatchType instanceof PsiDisjunctionType ? checkMultipleTypes(catchClass,
						((PsiDisjunctionType) upperCatchType).getDisjunctions()) : checkSingleType(catchClass, upperCatchType);
				if(highlight)
				{
					...
					final HighlightInfo highlightInfo = HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR).range(typeElement)
	...
	@Nullable
	public static HighlightInfo checkSynchronizedExpressionType(@NotNull PsiExpression expression,
			...
			int navigationShift)
	{
		...
		String toolTip = JavaErrorMessages.message("incompatible.types.html.tooltip", redIfNotMatch(lRawType, assignable), requiredRow,
			...
			@NotNull LanguageLevel languageLevel)
	{
		...
		if(refParent instanceof PsiReferenceExpression && (granny = refParent.getParent()) instanceof PsiMethodCallExpression)
		{
			...
			if(qualifierExpression == ref)
			{
				...
				{
					return HighlightInfo.newHighlightInfo(HighlightInfoType.WRONG_REF).range(qualifierExpression).descriptionAndTooltip("Qualifier " +
							"must be an expression").create();
				}
			}
		}
		...
		if(!result.isValidResult() && !PsiUtil.isInsideJavadocComment(ref))
		{
			if(!result.isAccessible())
			{
				...
				HighlightInfo info = HighlightInfo.newHighlightInfo(HighlightInfoType.WRONG_REF).range(refName).descriptionAndTooltip(description)
						.create();
			...
			@NotNull LanguageLevel languageLevel)
	{
		...
		if(resolved instanceof PsiClass)
		{
			...
			if(refGrandParent instanceof PsiClass)
			{
				...
				{
					highlightInfo = GenericsHighlightUtil.checkElementInTypeParameterExtendsList(referenceList, (PsiClass) refGrandParent,
							resolveResult, ref, languageLevel);
				}
				else
				{
	...
	static HighlightInfo checkAnnotationMethodParameters(@NotNull PsiParameterList list)
	{
		...
		if(PsiUtil.isAnnotationMethod(parent) && list.getParametersCount() > 0)
		{
			...
			final HighlightInfo highlightInfo = HighlightInfo.newHighlightInfo(HighlightInfoType.ERROR).range(list).descriptionAndTooltip(message)
					.create();
		...
		if(refElement instanceof PsiField && place instanceof PsiReferenceExpression)
		{
			...
			if(containingClass != null)
			{
				if(PsiUtil.isOnAssignmentLeftHand((PsiExpression) place))
				{
					...
					{
						QuickFixAction.registerQuickFixAction(error, QUICK_FIX_FACTORY.createReplaceInaccessibleFieldWithGetterSetterFix(place,
								setter, true));
					}
				}
	...
	@Nullable
	public static HighlightInfo checkForEachFeature(@NotNull PsiForeachStatement statement,
			@NotNull LanguageLevel languageLevel,
			@NotNull PsiFile containingFile)
	{
	...
	@Nullable
	public static HighlightInfo checkStaticImportFeature(@NotNull PsiImportStaticStatement statement,
			@NotNull LanguageLevel languageLevel,
			@NotNull PsiFile containingFile)
	{
	...
	@Nullable
	public static HighlightInfo checkVarargFeature(@NotNull PsiParameter parameter,
			@NotNull LanguageLevel languageLevel,
			@NotNull PsiFile containingFile)
	{
	...
	@Nullable
	public static HighlightInfo checkDiamondFeature(@NotNull PsiTypeElement typeElement,
			@NotNull LanguageLevel languageLevel,
			@NotNull PsiFile containingFile)
	{
		return typeElement.getType() instanceof PsiDiamondType ? checkFeature(typeElement.getParent(), Feature.DIAMOND_TYPES, languageLevel,
				containingFile) : null;
	}

	@Nullable
	public static HighlightInfo checkMultiCatchFeature(@NotNull PsiParameter parameter,
			@NotNull LanguageLevel languageLevel,
			@NotNull PsiFile containingFile)
	{
		return parameter.getType() instanceof PsiDisjunctionType ? checkFeature(parameter, Feature.MULTI_CATCH, languageLevel,
				containingFile) : null;
	}

	@Nullable
	public static HighlightInfo checkTryWithResourcesFeature(@NotNull PsiResourceVariable resourceVariable,
			@NotNull LanguageLevel languageLevel,
			@NotNull PsiFile containingFile)
	{
	...
	@Nullable
	public static HighlightInfo checkExtensionMethodsFeature(@NotNull PsiMethod method,
			@NotNull LanguageLevel languageLevel,
			@NotNull PsiFile containingFile)
	{
	...
	@Nullable
	public static HighlightInfo checkMethodReferencesFeature(@NotNull PsiMethodReferenceExpression expression,
			@NotNull LanguageLevel languageLevel,
			@NotNull PsiFile containingFile)
	{
	...
	@Nullable
	public static HighlightInfo checkLambdaFeature(@NotNull PsiLambdaExpression expression,
			@NotNull LanguageLevel languageLevel,
			@NotNull PsiFile containingFile)
	{
	...
	@Nullable
	public static HighlightInfo checkTypeAnnotationFeature(@NotNull PsiAnnotation annotation,
			@NotNull LanguageLevel languageLevel,
			@NotNull PsiFile containingFile)
	{
