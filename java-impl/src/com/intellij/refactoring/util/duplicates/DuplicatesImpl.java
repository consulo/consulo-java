...
public class DuplicatesImpl
{
	...
	public static void invoke(@NotNull final Project project, @NotNull Editor editor, @NotNull MatchProvider provider)
	{
		...
		for(final Match match : duplicates)
		{
			...
			if(replaceMatch(project, provider, match, editor, ++idx, duplicates.size(), showAll, confirmDuplicatePrompt, true))
			{
				return;
			}
		}
			...
			boolean skipPromptWhenOne)
	{
		...
		if(!ApplicationManager.getApplication().isUnitTestMode())
		{
			if((!skipPromptWhenOne || size > 1) && (showAll.get() == null || !showAll.get()))
			{
				...
				{
					if(Messages.showOkCancelDialog(project, "In order to replace all occurrences method signature will be changed. Proceed?", CommonBundle.getWarningTitle(), Messages.getWarningIcon()) != DialogWrapper.OK_EXIT_CODE)
	...
	public static void processDuplicates(@NotNull MatchProvider provider, @NotNull Project project, @NotNull Editor editor)
	{
		...
		if(hasDuplicates)
		{
			...
			final int answer = ApplicationManager.getApplication().isUnitTestMode() ? 0 : Messages.showYesNoDialog(project, RefactoringBundle.message("0.has.detected.1.code.fragments.in.this.file" +
					".that.can.be.replaced.with.a.call.to.extracted.method", ApplicationNamesInfo.getInstance().getProductName(), duplicates.size()), "Process Duplicates",
