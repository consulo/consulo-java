...
public class MethodSignatureUtil
{
	...
	public static final TObjectHashingStrategy<MethodSignatureBackedByPsiMethod> METHOD_BASED_HASHING_STRATEGY = new
			TObjectHashingStrategy<MethodSignatureBackedByPsiMethod>()
	{
		@Override
	...
	public static final TObjectHashingStrategy<MethodSignature> METHOD_PARAMETERS_ERASURE_EQUALITY = new
			TObjectHashingStrategy<MethodSignature>()
	{
		@Override
	...
	{
		PsiType[] erased1 = method1 instanceof MethodSignatureBase ? ((MethodSignatureBase) method1)
				.getErasedParameterTypes() : calcErasedParameterTypes(method1);
		PsiType[] erased2 = method2 instanceof MethodSignatureBase ? ((MethodSignatureBase) method2)
				.getErasedParameterTypes() : calcErasedParameterTypes(method2);
	...
	{
		return areSignaturesEqual(method1.getSignature(PsiSubstitutor.EMPTY), method2.getSignature(PsiSubstitutor
				.EMPTY));
	}
	...
	public static boolean areSignaturesEqual(@NotNull MethodSignature method1, @NotNull MethodSignature method2)
	{
		...
		if(!areSignaturesEqualLightweight(method1, method2))
		{
			return false;
		}
		return checkSignaturesEqualInner(method1, method2, getSuperMethodSignatureSubstitutor(method1,
				method2)) || checkSignaturesEqualInner(method2, method1, getSuperMethodSignatureSubstitutor(method2,
				method1));
	}
			...
			final PsiSubstitutor unifyingSubstitutor)
	{
		...
		if(!areErasedParametersEqual(subSignature, superSignature))
		{
			return false;
		}
	...
	public static boolean areSignaturesEqualLightweight(@NotNull MethodSignature sig1, @NotNull MethodSignature sig2)
	{
		...
		if(!isConstructor1 || !(sig1 instanceof HierarchicalMethodSignature || sig2 instanceof
	...
	public static boolean isSuperMethod(@NotNull PsiMethod superMethodCandidate, @NotNull PsiMethod derivedMethod)
	{
		...
		final PsiSubstitutor superSubstitutor = TypeConversionUtil.getMaybeSuperClassSubstitutor(superClassCandidate,
				derivedClass, PsiSubstitutor.EMPTY, null);
	...
	{
		PsiSubstitutor superSubstitutor = TypeConversionUtil.getSuperClassSubstitutor(superClass, aClass,
			...
			final boolean checkDeep)
	{
		...
		if(checkDeep)
		{
			...
			{
				PsiSubstitutor substitutor1 = TypeConversionUtil.getSuperClassSubstitutor(clazz, superClass,
						superSubstitutor);
	...
	public static PsiMethod findMethodBySignature(@NotNull PsiClass aClass,
			@NotNull PsiMethod pattenMethod,
			boolean checkBases)
	{
	...
	@Nullable
	public static PsiMethod findMethodBySignature(@NotNull PsiClass aClass,
			@NotNull MethodSignature methodSignature,
			boolean checkBases)
	{
		...
		for(Pair<PsiMethod, PsiSubstitutor> pair : pairs)
		{
			...
			if(methodSignature.equals(foundMethodSignature))
			{
				return method;
			}
		}
	...
	@Nullable
	public static PsiMethod findMethodBySuperSignature(@NotNull PsiClass aClass,
			@NotNull MethodSignature methodSignature,
			final boolean checkBases)
	{
		...
		for(Pair<PsiMethod, PsiSubstitutor> pair : pairs)
		{
			...
			PsiSubstitutor substitutor = pair.second;
			MethodSignature foundMethodSignature = method.getSignature(substitutor);
			if(isSubsignature(methodSignature, foundMethodSignature))
			{
				return method;
			}
		}
	...
	@Nullable
	public static PsiMethod findMethodBySuperMethod(@NotNull PsiClass aClass,
			@NotNull PsiMethod method,
			final boolean checkBases)
	{
		List<Pair<PsiMethod, PsiSubstitutor>> pairs = aClass.findMethodsAndTheirSubstitutorsByName(method.getName(),
				checkBases);
		for(Pair<PsiMethod, PsiSubstitutor> pair : pairs)
		{
			...
			if(methodClass == null || candidateClass == null)
			{
				continue;
			}
			PsiSubstitutor superSubstitutor = TypeConversionUtil.getClassSubstitutor(methodClass, candidateClass,
					substitutor);
			if(superSubstitutor == null)
			{
				continue;
			...
			if(isSubsignature(superSignature, candidateSignature))
			{
				return candidate;
			}
		}
	...
	public static boolean areParametersErasureEqual(@NotNull PsiMethod method1, @NotNull PsiMethod method2)
	{
		...
		return areSignaturesErasureEqual(method1.getSignature(PsiSubstitutor.EMPTY),
				method2.getSignature(PsiSubstitutor.EMPTY));
	}

	public static boolean areSignaturesErasureEqual(@NotNull MethodSignature signature1,
			@NotNull MethodSignature signature2)
	{
	 ...
	 */
	@Nullable
	public static PsiSubstitutor getSuperMethodSignatureSubstitutor(@NotNull MethodSignature methodSignature,
			@NotNull MethodSignature superMethodSignature)
	{
		...
		for(int i = 0; i < methodTypeParameters.length; i++)
		{
			...
			{
				superSupers.add(methodSubstitutor.substitute(PsiUtil.captureToplevelWildcards(result.substitute
						(superSuper), methodTypeParameter)));
			}
			methodSupers.remove(PsiType.getJavaLangObject(methodTypeParameter.getManager(),
					methodTypeParameter.getResolveScope()));
			superSupers.remove(PsiType.getJavaLangObject(superTypeParameter.getManager(),
					superTypeParameter.getResolveScope()));
			if(!methodSupers.equals(superSupers))
			{
				return null;
			}
		}
	...
	@Nullable
	private static PsiSubstitutor getSuperMethodSignatureSubstitutorImpl(@NotNull MethodSignature methodSignature,
			@NotNull MethodSignature superSignature)
	{
		...
		if(methodTypeParameters.length != superTypeParameters.length)
		{
			return null;
		}
		...
		{
			PsiTypeParameter methodTypeParameter = methodTypeParameters[i];
			PsiElementFactory factory = JavaPsiFacade.getInstance(methodTypeParameter.getProject())
					.getElementFactory();
	...
	@NotNull
	public static PsiSubstitutor combineSubstitutors(@NotNull PsiSubstitutor substitutor1,
			@NotNull PsiSubstitutor substitutor2)
	{
		...
		for(PsiTypeParameter typeParameter : typeParameters)
		{
			...
			if(type instanceof PsiClassType)
			{
				...
				otherSubstituted = resolved instanceof PsiTypeParameter ? substitutor2.substitute((PsiTypeParameter)
						resolved) : substitutor2.substitute(type);
			}
			else
			{
	...
	@NotNull
	public static PsiMethod[] convertMethodSignaturesToMethods(@NotNull List<? extends
			MethodSignatureBackedByPsiMethod> sameNameMethodList)
	{
			...
			@NotNull MethodSignature subSignature)
	{
		...
		if(!areSignaturesEqualLightweight(superSignature, subSignature))
		{
			return false;
		}
