		...
		if(types.size() > 1)
		{
			...
			for(Iterator<PsiType> iterator = types.iterator(); iterator.hasNext(); )
			{
				...
				for(PsiType existing : array)
				{
					if(type != existing)
					{
						final boolean allowUncheckedConversion = type instanceof PsiClassType && ((PsiClassType) type)
								.isRaw();
						if(TypeConversionUtil.isAssignable(type, existing, allowUncheckedConversion) ||
								TypeConversionUtil.isAssignable(GenericsUtil.eliminateWildcards(type),
