...
public class JavaMethodsConflictResolver implements PsiConflictResolver {
  ...
  public JavaMethodsConflictResolver(PsiExpressionList list, LanguageLevel languageLevel) {
    this(list, null, languageLevel);
  }

  public JavaMethodsConflictResolver(PsiElement argumentsList, PsiType[] actualParameterTypes, LanguageLevel languageLevel) {
  ...
  @Override
  public final CandidateInfo resolveConflict(final List<CandidateInfo> conflicts) {
  ...
  @Nullable
  protected CandidateInfo guardedOverloadResolution(List<CandidateInfo> conflicts) {
  ...
  private static void checkPotentiallyCompatibleMethods(List<CandidateInfo> conflicts) {
  ...
  public void checkSpecifics(List<CandidateInfo> conflicts, @MethodCandidateInfo.ApplicabilityLevelConstant int applicabilityLevel, LanguageLevel languageLevel) {
    checkSpecifics(conflicts, applicabilityLevel, null, languageLevel);
  }

  public void checkSpecifics(List<CandidateInfo> conflicts,
  ...
  protected void checkSameSignatures(List<CandidateInfo> conflicts) {
    checkSameSignatures(conflicts, null);
  }

  protected void checkSameSignatures(List<CandidateInfo> conflicts, Map<MethodCandidateInfo, PsiSubstitutor> map) {
    ...
    for (int i = 0; i < conflicts.size(); i++) {
      ...
      if (InheritanceUtil.isInheritorOrSelf(class1, existingClass, true) || InheritanceUtil.isInheritorOrSelf(existingClass, class1, true)) {
        ...
        for (int i1 = 0, parametersLength = parameters.length; i1 < parametersLength; i1++) {
          if (parameters[i1].getType() instanceof PsiArrayType && !(existingParameters[i1].getType() instanceof PsiArrayType)) {//prefer more specific type
            signatures.put(signature, info);
        ...
        if (methodCallExpression != null) {
          ...
          if (currentClass != null && existingClass != null && class1 != null) {
            final PsiSubstitutor eSubstitutor = TypeConversionUtil.getMaybeSuperClassSubstitutor(existingClass, currentClass, PsiSubstitutor.EMPTY);
   ...
   */
  private void checkStaticMethodsOfInterfaces(List<CandidateInfo> conflicts) {
  ...
  public boolean checkParametersNumber(List<CandidateInfo> conflicts, final int argumentsCount, Map<MethodCandidateInfo, PsiSubstitutor> map, boolean ignoreIfStaticsProblem) {
  ...
  public int checkApplicability(List<CandidateInfo> conflicts) {
  ...
  private Specifics isMoreSpecific(MethodCandidateInfo info1,
  ...
  private static boolean isBoxingUsed(PsiType parameterType, @Nullable PsiType argType, PsiExpression arg) {
    ProgressManager.checkCanceled();
    final boolean isExpressionTypePrimitive = argType != null ? argType instanceof PsiPrimitiveType : PsiPolyExpressionUtil.isExpressionOfPrimitiveType(arg);
  ...
  private static boolean isSubSignature(PsiMethod method1, PsiMethod method2, PsiSubstitutor classSubstitutor1, PsiSubstitutor classSubstitutor2, boolean[] boxingHappened) {
    return MethodSignatureUtil.areErasedParametersEqual(method1.getSignature(PsiSubstitutor.EMPTY), method2.getSignature(PsiSubstitutor.EMPTY)) && MethodSignatureUtil.isSubsignature(method1
        .getSignature(classSubstitutor1), method2.getSignature(classSubstitutor2)) || method1.hasModifierProperty(PsiModifier.STATIC) && method2.hasModifierProperty(PsiModifier.STATIC) &&
        !boxingHappened[0];
  }
  ...
  private boolean isApplicableTo(PsiType[] types2AtSite,
                                 ...
                                 PsiSubstitutor methodSubstitutor1,
                                 PsiMethod method2,
  ...
  private static PsiSubstitutor calculateMethodSubstitutor(PsiTypeParameter[] typeParameters,
                                                           PsiMethod method,
  ...
  public void checkPrimitiveVarargs(List<CandidateInfo> conflicts, final int argumentsCount) {
  ...
  private static boolean isFunctionalTypeMoreSpecific(PsiExpression expr, PsiType sType, PsiType tType) {
    ...
    if (expr instanceof PsiConditionalExpression) {
      return isFunctionalTypeMoreSpecific(((PsiConditionalExpression) expr).getThenExpression(), sType, tType) && isFunctionalTypeMoreSpecific(((PsiConditionalExpression) expr)
