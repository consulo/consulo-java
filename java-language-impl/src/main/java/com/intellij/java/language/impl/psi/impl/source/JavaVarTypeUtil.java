...
public class JavaVarTypeUtil
{
	...
	private static class UpwardProjectionTypeVisitor extends PsiTypeVisitorEx<PsiType>
	{
		...
		{
			return PsiIntersectionType.createIntersection(Arrays.stream(intersectionType.getConjuncts()).map(conjunct -> conjunct.accept(this)).toArray(PsiType[]::new));
		}
		...
		private static boolean tryUpperBound(PsiClass aClass, PsiTypeParameter parameter, PsiType U)
		{
			...
			return PsiPolyExpressionUtil.mentionsTypeParameters(bi, ContainerUtil.newHashSet(aClass.getTypeParameters())) || !U.isAssignableFrom(bi);
		}
	}
