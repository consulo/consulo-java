...
public class ReferenceParser {
  ...
  private TypeInfo parseTypeInfo(PsiBuilder builder, int flags, boolean badWildcard) {
    ...
    IElementType tokenType = builder.getTokenType();
    if (tokenType == JavaTokenType.IDENTIFIER && isSet(flags,
                                                       VAR_TYPE) && PsiKeyword.VAR.equals(builder.getTokenText()) && getLanguageLevel(
      builder).isAtLeast(LanguageLevel.JDK_10)) {
