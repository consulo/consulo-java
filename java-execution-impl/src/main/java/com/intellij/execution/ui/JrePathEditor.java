...
public class JrePathEditor extends JPanel implements PanelWithAnchor
{
	...
	{
		myLabel = new JBLabel(JavaExecutionBundle.message("run.configuration.jre.label"));
		...
		final List<Sdk> allJDKs = SdkTable.getInstance().getSdksOfType(JavaSdk.getInstance());
		for(Sdk sdk : allJDKs)
		{
		...
		myPathField.addBrowseFolderListener(ExecutionBundle.message("run.configuration.select.alternate.jre.label"), ExecutionBundle.message("run.configuration.select.jre.dir.label"), null,
	...
	private JreComboBoxItem findOrAddCustomJre(@NotNull String pathOrName)
	{
		...
		{
			if(item instanceof CustomJreItem && FileUtil.pathsEqual(pathOrName, ((CustomJreItem) item).myPath) || pathOrName.equals(item.getPathOrName()))
			{
				return item;
			}
		}
	...
	private static class SdkAsJreItem implements JreComboBoxItem
	{
		...
		{
			OrderEntryAppearanceService.getInstance().forSdk(mySdk, false, selected, true).customize(component);
		}
