...
public final class JavaPsiSwitchUtil {
    ...
    @Nullable
    public static PsiEnumConstant getEnumConstant(@Nullable PsiCaseLabelElement element) {
    ...
    public static boolean isDominated(@Nonnull PsiCaseLabelElement overWhom,
        ...
        if (who instanceof PsiCaseLabelElement currentElement) {
            if (JavaPsiPatternUtil.isGuarded(currentElement)) {
                return false;
            }
            if (isConstantLabelElement(overWhom)) {
                PsiExpression constExpr = ObjectUtil.tryCast(overWhom, PsiExpression.class);
    ...
    public static @Nonnull Map<PsiCaseLabelElement, PsiElement> findDominatedLabels(@Nonnull PsiSwitchBlock block) {
        ...
        for (int i = 0; i < elementsToCheckDominance.size() - 1; i++) {
            ...
            for (int j = i + 1; j < elementsToCheckDominance.size(); j++) {
                ...
                if (!(next instanceof PsiCaseLabelElement nextElement)) {
                    continue;
                }
    ...
    public static boolean isBooleanSwitchWithTrueAndFalse(@Nonnull PsiSwitchBlock block) {
        ...
        PsiType selectorType = selector.getType();
        if (selectorType == null || getSwitchSelectorKind(selector.getType()) != SelectorKind.BOOLEAN) {
            return false;
        }
        ...
        for (PsiStatement st : body.getStatements()) {
            ...
            for (PsiCaseLabelElement labelElement : labelElementList.getElements()) {
                if (!(labelElement instanceof PsiExpression expression)) {
                    continue;
                }
    ...
    public static @Nullable PsiElement findDefaultElement(@Nonnull PsiSwitchBlock switchBlock) {
        ...
        for (PsiStatement statement : body.getStatements()) {
            PsiSwitchLabelStatementBase switchLabelStatement = ObjectUtil.tryCast(statement, PsiSwitchLabelStatementBase.class);
    ...
    public enum SelectorKind {
        ...
        SelectorKind(@Nullable JavaFeature feature) {
            myFeature = feature;
        }
