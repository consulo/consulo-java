...
public class AnnotationUtil {
    ...
    public static final class AnnotationAndOwner {
        final PsiModifierListOwner owner;
        final PsiAnnotation annotation;

        AnnotationAndOwner(PsiModifierListOwner owner, PsiAnnotation annotation) {
            this.owner = owner;
    ...
    @Nullable
    static AnnotationAndOwner findAnnotationAndOwnerInHierarchy(PsiModifierListOwner listOwner,
    ...
    @Nullable
    public static PsiAnnotation findAnnotation(@Nullable PsiModifierListOwner listOwner,
                                               boolean skipExternal,
                                               String... annotationNames) {
        if (annotationNames.length == 0) {
            return null;
        }
        Set<String> set = annotationNames.length == 1 ? Collections.singleton(annotationNames[0]) : Set.of(annotationNames);
    ...
    private static PsiAnnotation findOwnAnnotation(final PsiModifierListOwner listOwner, Collection<String> annotationNames) {
        Map<Collection<String>, PsiAnnotation> map = LanguageCachedValueUtil.getCachedValue(listOwner, () ->
        {
            Map<Collection<String>, PsiAnnotation> value = ConcurrentFactoryMap.createMap(annotationNames1 ->
            {
                final PsiModifierList list =
                    listOwner.getModifierList();
                if (list == null) {
                    return null;
                }
        ...
        Map<Collection<String>, PsiAnnotation> map = LanguageCachedValueUtil.getCachedValue(listOwner, () ->
        {
            Map<Collection<String>, PsiAnnotation> value = ConcurrentFactoryMap.createMap(annotationNames1 ->
            {
                ...
                for (String annotationName : annotationNames1) {
                    final PsiAnnotation annotation =
                        annotationsManager.findExternalAnnotation(
                            listOwner,
                            annotationName);
                    if (annotation != null) {
                        return annotation;
                ...
                final InferredAnnotationsManager
                    inferredAnnotationsManager =
                    InferredAnnotationsManager.getInstance(project);
                for (String annotationName : annotationNames1) {
                    final PsiAnnotation annotation =
                        inferredAnnotationsManager.findInferredAnnotation(
                            listOwner,
                            annotationName);
                    if (annotation != null) {
                        return annotation;
    ...
    public static PsiAnnotation[] findAnnotations(@Nullable PsiModifierListOwner modifierListOwner,
                                                  Collection<String> annotationNames) {
    ...
    @Nullable
    public static PsiAnnotation findAnnotationInHierarchy(final PsiModifierListOwner listOwner,
                                                          Set<String> annotationNames, boolean skipExternal) {
        PsiAnnotation directAnnotation = findAnnotation(listOwner, annotationNames, skipExternal);
        if (directAnnotation != null) {
    ...
    private static void collectSuperParameters(final Set<PsiModifierListOwner> result, PsiParameter parameter) {
        ...
        if (scope instanceof PsiLambdaExpression) {
            ...
            if (method != null) {
                forEachSuperMethod.accept(method);
                for (PsiMethod superMethod : getSuperAnnotationOwners(method)) {
                    forEachSuperMethod.accept(superMethod);
                }
            }
        }
    ...
    private static void collectSuperMethods(Set<PsiModifierListOwner> result,
        ...
        for (final HierarchicalMethodSignature superSignature : signature.getSuperSignatures()) {
            ...
            if (!resolveHelper.isAccessible(superMethod, place, null)) {
                continue;
            }
            if (!result.add(superMethod)) {
                continue;
            }
    ...
    public static boolean isAnnotated(PsiModifierListOwner listOwner, Collection<String> annotations, @Flags int flags) {
        return annotations.stream().anyMatch(annotation -> isAnnotated(listOwner, annotation, flags, null));
    }

    public static boolean isAnnotated(PsiModifierListOwner listOwner, String annotationFqn, @Flags int flags) {
    ...
    public static boolean isAnnotatingApplicable(PsiElement elt, String annotationFQN) {
        final Project project = elt.getProject();
        return PsiUtil.isLanguageLevel5OrHigher(elt) && JavaPsiFacade.getInstance(project)
            .findClass(annotationFQN, elt.getResolveScope()) != null;
    }
    ...
    public static boolean checkAnnotatedUsingPatterns(@Nullable PsiModifierListOwner owner, Collection<String> annotations) {
    ...
    public static PsiAnnotation[] getAllAnnotations(PsiModifierListOwner owner,
                                                    boolean inHierarchy,
    ...
    @Nullable
    public static Long getLongAttributeValue(PsiAnnotation anno, @Nullable final String attributeName) {
        PsiAnnotationMemberValue attrValue = anno.findAttributeValue(attributeName);
        Object constValue = JavaPsiFacade.getInstance(anno.getProject()).getConstantEvaluationHelper().computeConstantExpression(attrValue);
    ...
    @Nullable
    public static String getDeclaredStringAttributeValue(PsiAnnotation anno, @Nullable final String attributeName) {
    ...
    @Nullable
    public static <T extends Annotation> T findAnnotationInHierarchy(PsiModifierListOwner listOwner,
        ...
        @SuppressWarnings("unchecked") T t =
            (T) Proxy.newProxyInstance(annotationClass.getClassLoader(), new Class<?>[]{annotationClass}, handler);
        return t;
    }
     ...
     */
    @Nullable
    public static PsiNameValuePair findDeclaredAttribute(PsiAnnotation annotation, @Nullable() String attributeName) {
    ...
    public static boolean equal(@Nullable PsiAnnotation a, @Nullable PsiAnnotation b) {
        ...
        if (!fillValueMap(a.getParameterList(), valueMap1) || !fillValueMap(b.getParameterList(),
            valueMap2) || valueMap1.size() != valueMap2.size()) {
            return false;
        }
     ...
     */
    public static boolean isJetbrainsAnnotation(String simpleName) {
    ...
    private static
    @Flags
    int flags(boolean checkHierarchy, boolean skipExternal, boolean skipInferred) {
        int flags = CHECK_TYPE;
        if (checkHierarchy) {
            flags |= CHECK_HIERARCHY;
        }
        if (!skipExternal) {
            flags |= CHECK_EXTERNAL;
        }
        if (!skipInferred) {
            flags |= CHECK_INFERRED;
        }
        return flags;
    }
    //</editor-fold>
                    ...
                    if (targets != null && targets.contains(PsiAnnotation.TargetType.TYPE_USE) &&
                        ...
                        if (typeElement != null && targets.size() == 1) {
                            ...
                            while (ref != null && ref.isQualified()) {
                                ref = ObjectUtil.tryCast(ref.getQualifier(), PsiJavaCodeReferenceElement.class);
                            }
                            if (ref != null) {
