...
public class PsiOldInferenceHelper implements PsiInferenceHelper
{
	...
	public static final Pair<PsiType, ConstraintType> RAW_INFERENCE = new Pair<PsiType, ConstraintType>(null,
			...
			@NotNull ParameterTypeInferencePolicy policy)
	{
		...
		return inferTypeForMethodTypeParameterInner(typeParameter, paramTypes, argTypes, partialSubstitutor, parent,
				policy);
	}
			...
			@NotNull ParameterTypeInferencePolicy policy)
	{
		...
		if(paramTypes.length > 0)
		{
			...
			for(int j = 0; j < argTypes.length; j++)
			{
				...
				if(parameterType instanceof PsiEllipsisType)
				{
					...
					if(argTypes.length == paramTypes.length && argumentType instanceof PsiArrayType && !((
				...
				{
					currentSubstitution = inferSubstitutionFromLambda(typeParameter,
							(PsiLambdaExpressionType) argumentType, lowerBound, partialSubstitutor);
					if(rawType)
					{
						if(currentSubstitution == FAILED_INFERENCE || currentSubstitution == null && lowerBound == PsiType.NULL)
						{
					...
					{
						return new Pair<PsiType, ConstraintType>(PsiType.getJavaLangObject(myManager,
								typeParameter.getResolveScope()), ConstraintType.EQUALS);
					}
				}
				else if(argumentType instanceof PsiMethodReferenceType)
				{
					final PsiMethodReferenceExpression referenceExpression = ((PsiMethodReferenceType) argumentType)
							.getExpression();
					currentSubstitution = inferConstraintFromFunctionalInterfaceMethod(typeParameter,
							referenceExpression, partialSubstitutor.substitute(parameterType), partialSubstitutor,
							policy);
				}
				else
				{
		...
		if(parent != null)
		{
			final Pair<PsiType, ConstraintType> constraint = inferMethodTypeParameterFromParent(typeParameter,
					partialSubstitutor, parent, policy);
			if(constraint != null)
			{
	...
	private static void sortLambdaExpressionsLast(@NotNull PsiType[] paramTypes, @NotNull PsiType[] argTypes)
	{
		for(int i = 0; i < argTypes.length; i++)
		{
			...
			if((argType instanceof PsiLambdaExpressionType || argType instanceof PsiMethodReferenceType) && i <
					argTypes.length - 1)
			{
				int k = i + 1;
				while((argTypes[k] instanceof PsiLambdaExpressionType || argTypes[k] instanceof
						PsiMethodReferenceType) && k < argTypes.length - 1)
				{
					k++;
				}
	...
	{
		return new Pair<PsiType, ConstraintType>(JavaPsiFacade.getInstance(typeParameter.getProject())
				.getElementFactory().createType(typeParameter), ConstraintType.EQUALS);
	}
			...
			@NotNull final ParameterTypeInferencePolicy policy)
	{
		...
		final Pair<PsiType, ConstraintType> constraint = inferTypeForMethodTypeParameterInner(typeParameter,
				parameters, arguments, partialSubstitutor, parent, policy);
		if(constraint == null)
		{
			return PsiType.NULL;
		}
			...
			@NotNull LanguageLevel languageLevel)
	{
		...
		for(int i = 0; i < typeParameters.length; i++)
		{
			...
			final Pair<PsiType, ConstraintType> constraint = inferTypeForMethodTypeParameterInner(typeParameters[i],
					parameters, arguments, partialSubstitutor, null, policy);
					...
					if(i != j)
					{
						...
						for(PsiClassType bound : bounds)
						{
							...
							final Pair<PsiType, ConstraintType> currentConstraint =
									getSubstitutionForTypeParameterConstraint(typeParameter, substitutedBound,
											otherSubstitution, true, languageLevel);
							...
							else if(currentConstraintType == ConstraintType.SUPERTYPE)
							{
								...
								{
									substitutionFromBounds = GenericsUtil.getLeastUpperBound(substitutionFromBounds,
											currentSubstitution, myManager);
								}
							}
						}
		...
		try
		{
			for(int i = 0; i < typeParameters.length; i++)
			{
				...
				{
					Pair<PsiType, ConstraintType> otherConstraint = inferMethodTypeParameterFromParent(typeParameter,
							partialSubstitutor, parent, policy);
					if(otherConstraint != null)
					{
						if(otherConstraint.getSecond() == ConstraintType.EQUALS || otherConstraint.getSecond() ==
			...
			@NotNull LanguageLevel languageLevel)
	{
		...
		for(PsiTypeParameter typeParameter : typeParameters)
		{
			...
			for(int i1 = 0; i1 < leftTypes.length; i1++)
			{
				...
				final Pair<PsiType, ConstraintType> constraint = getSubstitutionForTypeParameterConstraint
						(typeParameter, leftType, rightType, true, languageLevel);
				if(constraint != null)
				{
	...
	@Nullable
	private static Pair<PsiType, ConstraintType> processArgType(PsiType arg,
			...
			@NotNull ParameterTypeInferencePolicy policy)
	{
		...
		if(owner instanceof PsiMethod && parent instanceof PsiCallExpression)
		{
			...
			substitution = inferMethodTypeParameterFromParent(PsiUtil.skipParenthesizedExprUp(methodCall.getParent()),
					methodCall, typeParameter, substitutor, policy);
		}
		return substitution;
	}
	...
	{
		final Pair<PsiType, ConstraintType> constraint = getSubstitutionForTypeParameterConstraint(typeParam, param,
				arg, isContraVariantPosition, languageLevel);
			...
			final LanguageLevel languageLevel)
	{
		...
		{
			return getSubstitutionForTypeParameterConstraint(typeParam, ((PsiArrayType) param).getComponentType(),
					((PsiArrayType) arg).getComponentType(), isContraVariantPosition, languageLevel);
		}
		...
		if(typeParam == paramClass)
		{
			...
			if(arg == null ||
					...
					(psiClass != null && (isContraVariantPosition || !CommonClassNames.JAVA_LANG_OBJECT.equals
							(psiClass.getQualifiedName()) || (arg instanceof PsiArrayType))))
			{
		...
		{
			PsiSubstitutor substitutor = TypeConversionUtil.getClassSubstitutor(paramClass, argClass,
					argResult.getSubstitutor());
			if(substitutor == null)
			{
				return null;
			}
		...
		{
			PsiSubstitutor substitutor = TypeConversionUtil.getClassSubstitutor(argClass, paramClass,
					paramResult.getSubstitutor());
			if(substitutor == null)
			{
				return null;
			}
			...
			PsiSubstitutor partialSubstitutor)
	{
		...
		if(PsiUtil.getLanguageLevel(lambdaExpression).isAtLeast(LanguageLevel.JDK_1_8))
		{
			...
			if(parent instanceof PsiExpressionList)
			{
				...
				final Map<PsiElement, Pair<PsiMethod, PsiSubstitutor>> methodMap = MethodCandidateInfo
				...
				if(pair != null)
				{
					...
					return inferConstraintFromFunctionalInterfaceMethod(typeParam, lambdaExpression,
							combinedSubst.substitute(parameters[i].getType()), lowerBound);
				}
			}
			else
			{
	...
	@Nullable
	private Pair<PsiType, ConstraintType> inferConstraintFromFunctionalInterfaceMethod(final PsiTypeParameter
			typeParam,
			...
			final ParameterTypeInferencePolicy policy)
	{
		...
		if(functionalInterfaceMethod != null)
		{
			...
			if(methodParamsDependOn(typeParam, methodReferenceExpression, functionalInterfaceType, methodParameters,
					subst))
			{
				return null;
			}
			...
			if(resolved instanceof PsiMethod)
			{
				...
				{
					args[i] = methReferenceResolveResult.getSubstitutor().substitute(subst.substitute(parameters[i]
							.getType()));
				}

				final PsiType[] typesToInfer = hasReceiver ? ArrayUtil.remove(methodParamTypes, 0) : methodParamTypes;
				final Pair<PsiType, ConstraintType> constraint = inferTypeForMethodTypeParameterInner(typeParam,
						typesToInfer, args, subst, null, DefaultParameterTypeInferencePolicy.INSTANCE);
				if(constraint != null)
				{
					return constraint;
				}
				...
				{
					functionalInterfaceReturnType = GenericsUtil.eliminateWildcards(subst.substitute
							(functionalInterfaceReturnType));
					final PsiType argType;
					if(method.isConstructor())
					{
						argType = JavaPsiFacade.getElementFactory(functionalInterfaceMethod.getProject()).createType
								(method.getContainingClass(), methReferenceResolveResult.getSubstitutor());
					}
					else
					{
						argType = methReferenceResolveResult.getSubstitutor().substitute(subst.substitute(method
								.getReturnType()));
					}
					final Pair<PsiType, ConstraintType> typeParameterConstraint =
							getSubstitutionForTypeParameterConstraint(typeParam, functionalInterfaceReturnType,
									argType, true, PsiUtil.getLanguageLevel(functionalInterfaceMethod));
					if(typeParameterConstraint != null && typeParameterConstraint.getSecond() != ConstraintType.EQUALS
							&& method.isConstructor())
					{
						final Pair<PsiType, ConstraintType> constraintFromParent = inferMethodTypeParameterFromParent
								(typeParam, partialSubstitutor, methodReferenceExpression.getParent().getParent(),
										policy);
						if(constraintFromParent != null && constraintFromParent.getSecond() == ConstraintType.EQUALS)
						{
			...
			PsiType lowerBound)
	{
		...
		if(method != null)
		{
			...
			final Pair<PsiType, ConstraintType> constraintFromFormalParams = inferConstraintFromLambdaFormalParams
					(typeParam, subst, method, lambdaExpression);
			...
			if(returnType != null && returnType != PsiType.VOID)
			{
				...
				{
					final boolean independent = lambdaExpression.hasFormalParameterTypes() || LambdaUtil
							.isFreeFromTypeInferenceArgs(methodParameters, lambdaExpression, expression, subst, functionalInterfaceType, typeParam);
					if(!independent)
					{
						if(lowerBound != PsiType.NULL)
						{
					...
					if(expression instanceof PsiReferenceExpression && ((PsiReferenceExpression) expression).resolve() == null)
					{
						continue;
					}
					PsiType exprType = PsiResolveHelper.ourGraphGuard.doPreventingRecursion(expression, true,
					...
					{
						return inferConstraintFromFunctionalInterfaceMethod(typeParam,
								((PsiLambdaExpressionType) exprType).getExpression(), returnType, lowerBound);
					}
					...
					final Pair<PsiType, ConstraintType> returnExprConstraint =
							getSubstitutionForTypeParameterConstraint(typeParam, GenericsUtil.eliminateWildcards
									(returnType), exprType, true, PsiUtil.getLanguageLevel(method));
					if(returnExprConstraint != null)
					{
						...
						if(constraint != null)
						{
							final PsiType leastUpperBound = GenericsUtil.getLeastUpperBound(constraint.getFirst(),
									returnExprConstraint.getFirst(), myManager);
	...
	private static boolean methodParamsDependOn(PsiTypeParameter typeParam,
			...
			PsiSubstitutor subst)
	{
		...
		{
			if(LambdaUtil.dependsOnTypeParams(subst.substitute(parameter.getType()), functionalInterfaceType,
					psiElement, typeParam))
			{
				return true;
			}
		}
			...
			PsiLambdaExpression lambdaExpression)
	{
		...
		return inferTypeForMethodTypeParameterInner(typeParam, methodParamTypes, lambdaArgs, subst, null,
	...
	//represents the result of failed type inference: in case we failed inferring from parameters,
	// do not perform inference from context
	private static final Pair<PsiType, ConstraintType> FAILED_INFERENCE = new Pair<PsiType,
			ConstraintType>(PsiType.NULL, ConstraintType.EQUALS);
			...
			final int depth)
	{
		...
		if(param instanceof PsiWildcardType)
		{
			...
			{
				if(((PsiWildcardType) arg).isExtends() == wildcardParam.isExtends() && ((PsiWildcardType) arg)
						.isBounded() == wildcardParam.isBounded())
				{
					Pair<PsiType, ConstraintType> res = getSubstitutionForTypeParameterInner(paramBound,
							((PsiWildcardType) arg).getBound(), patternType, constrType, depth);
					if(res != null)
					{
						return res;
					}
			...
			{
				Pair<PsiType, ConstraintType> res = getSubstitutionForTypeParameterInner(paramBound, arg, patternType,
						constrType, depth);
				if(res != null)
				{
					return res;
			...
			{
				Pair<PsiType, ConstraintType> res = getSubstitutionForTypeParameterInner(((PsiArrayType) paramBound)
						.getComponentType(), ((PsiArrayType) arg).getComponentType(), patternType, constrType, depth);
				if(res != null)
				{
					return res;
				}
			...
			else if(paramBound instanceof PsiClassType && arg instanceof PsiClassType)
			{
				...
				if(boundClass != null)
				{
					...
					if(argClass != null)
					{
						...
						{
							PsiSubstitutor superSubstitutor = TypeConversionUtil.getClassSubstitutor(boundClass,
									argClass, argResult.getSubstitutor());
								...
								for(PsiTypeParameter typeParameter : PsiUtil.typeParametersIterable(boundClass))
								{
									...
									if(substituted != null)
									{
										Pair<PsiType, ConstraintType> res = getSubstitutionForTypeParameterInner
												(boundResult.getSubstitutor().substitute(typeParameter), substituted,
						...
						{
							PsiSubstitutor superSubstitutor = TypeConversionUtil.getClassSubstitutor(argClass,
									boundClass, boundResult.getSubstitutor());
							if(superSubstitutor != null)
							{
								for(PsiTypeParameter typeParameter : PsiUtil.typeParametersIterable(argClass))
								{
									...
									{
										Pair<PsiType, ConstraintType> res = getSubstitutionForTypeParameterInner
												(superSubstitutor.substitute(typeParameter), substituted, patternType,
		...
		if(param instanceof PsiArrayType && arg instanceof PsiArrayType)
		{
			return getSubstitutionForTypeParameterInner(((PsiArrayType) param).getComponentType(),
					((PsiArrayType) arg).getComponentType(), patternType, constraintType, depth);
		}
		...
		if(param instanceof PsiClassType && arg instanceof PsiClassType)
		{
			...
			for(PsiTypeParameter typeParameter : PsiUtil.typeParametersIterable(paramClass))
			{
				...
				Pair<PsiType, ConstraintType> res = getSubstitutionForTypeParameterInner(paramType, argType,
						patternType, ConstraintType.EQUALS, depth + 1);
	...
	private Pair<PsiType, ConstraintType> inferBySubtypingConstraint(PsiType patternType,
			...
			PsiClass argClass)
	{
		if(argClass instanceof PsiTypeParameter && paramClass instanceof PsiTypeParameter && PsiUtil
				.isLanguageLevel8OrHigher(argClass))
		{
			...
			if(argExtendsListTypes.length == paramExtendsListTypes.length)
			{
				try
				{
					...
					for(int i = 0; i < argExtendsListTypes.length; i++)
					{
						...
						if(argBoundClass != null && paramBoundClass != null && paramBoundClass != argBoundClass)
						{
							if(argBoundClass.isInheritor(paramBoundClass, true))
							{
								final PsiSubstitutor superClassSubstitutor = TypeConversionUtil
										.getSuperClassSubstitutor(paramBoundClass, argBoundClass, argResolveResult.getSubstitutor());
								argBoundType = JavaPsiFacade.getElementFactory(argClass.getProject()).createType
										(paramBoundClass, superClassSubstitutor);
							}
							else
							{
								return null;
						...
						final Pair<PsiType, ConstraintType> constraint = getSubstitutionForTypeParameterInner
								(paramBoundType, argBoundType, patternType, constraintType, depth);
						if(constraint != null)
						{
							return constraint;
						}
			...
			@NotNull ParameterTypeInferencePolicy policy)
	{
		...
		if(expectedType == null)
		{
			...
			{
				final PsiLambdaExpression lambdaExpression = PsiTreeUtil.getParentOfType(parent,
			...
			else if(parent instanceof PsiExpressionList)
			{
				...
				if(pParent instanceof PsiCallExpression && parent.equals(((PsiCallExpression) pParent).getArgumentList
						()))
				{
					constraint = policy.inferTypeConstraintFromCallContext(methodCall, (PsiExpressionList) parent,
							(PsiCallExpression) pParent, typeParameter);
					if(constraint == null && PsiUtil.isLanguageLevel8OrHigher(methodCall))
					{
						constraint = graphInferenceFromCallContext(methodCall, typeParameter,
								(PsiCallExpression) pParent);
						if(constraint != null)
						{
							final PsiType constraintFirst = constraint.getFirst();
							if(constraintFirst == null || constraintFirst.equalsToText(CommonClassNames
									.JAVA_LANG_OBJECT))
							{
								constraint = null;
							}
			...
			{
				expectedType = PsiResolveHelper.ourGraphGuard.doPreventingRecursion(methodCall, true,
						new Computable<PsiType>()
				{
					@Override
					public PsiType compute()
					{
						return LambdaUtil.getFunctionalInterfaceReturnType(((PsiLambdaExpression) parent)
			...
			else if(parent instanceof PsiConditionalExpression)
			{
				if(PsiUtil.isLanguageLevel8OrHigher(parent))
				{
					try
					{
						final Pair<PsiType, ConstraintType> pair = inferFromConditionalExpression(parent, methodCall,
								typeParameter, substitutor, policy);
						if(pair != null)
						{
							return pair;
						}
		...
		if(constraint == null)
		{
			if(expectedType == null)
			{
				expectedType = methodCall instanceof PsiCallExpression ? policy.getDefaultExpectedType(
						(PsiCallExpression) methodCall) : null;
			}

			returnType = ((PsiMethod) typeParameter.getOwner()).getReturnType();

			constraint = getSubstitutionForTypeParameterConstraint(typeParameter, returnType, expectedType, false,
		...
		if(constraint == null)
		{
			if(methodCall instanceof PsiCallExpression)
			{
				...
				if(argumentList != null && PsiUtil.getLanguageLevel(argumentList).isAtLeast(LanguageLevel.JDK_1_8))
				{
					...
					{
						if(expression instanceof PsiLambdaExpression || expression instanceof
								PsiMethodReferenceExpression)
						{
							final PsiType functionalInterfaceType = LambdaUtil.getFunctionalInterfaceType(expression,
									false);
							if(functionalInterfaceType == null || PsiUtil.resolveClassInType(functionalInterfaceType)
									== typeParameter)
							{
								return getFailedInferenceConstraint(typeParameter);
							}
							...
							final Pair<PsiType, ConstraintType> inferredExceptionTypeConstraint =
									inferExceptionConstrains(typeParameter, expression, method,
				...
				return policy.getInferredTypeWithNoConstraint(myManager, PsiIntersectionType.createIntersection
						(types));
			}
			return null;
		}
		...
		for(PsiClassType exception : declaredExceptions)
		{
			...
			if(PsiUtil.resolveClassInType(substitute) == typeParameter)
			{
				...
				else if(expression instanceof PsiMethodReferenceExpression)
				{
					...
					{
						final PsiClassType[] declaredThrowsList = ((PsiMethod) resolve).getThrowsList()
								.getReferencedTypes();
			...
			PsiMethod method)
	{
		...
		final PsiClass runtimeException = JavaPsiFacade.getInstance(project).findClass(CommonClassNames
				.JAVA_LANG_RUNTIME_EXCEPTION, method.getResolveScope());
		if(runtimeException != null)
		{
			...
			return Pair.<PsiType, ConstraintType>create(JavaPsiFacade.getElementFactory(project).createType
					(runtimeException, PsiSubstitutor.EMPTY), ConstraintType.EQUALS);
		}
		return null;
	}
	...
	private Pair<PsiType, ConstraintType> inferFromConditionalExpression(@NotNull PsiElement parent,
			...
			@NotNull ParameterTypeInferencePolicy policy)
	{
		Pair<PsiType, ConstraintType> pair = inferMethodTypeParameterFromParent(PsiUtil.skipParenthesizedExprUp(parent
				.getParent()), (PsiExpression) parent, typeParameter, substitutor, policy);
		if(pair == null)
		{
			...
			{
				final PsiType thenType = PsiResolveHelper.ourGraphGuard.doPreventingRecursion(parent, true,
						new Computable<PsiType>()
				{
					@Override
					public PsiType compute()
				...
				if(thenType != null)
				{
					pair = inferTypeForMethodTypeParameterInner(typeParameter, paramTypes, new PsiType[]{thenType},
							substitutor, null, policy);
				}
			}
			else if(methodCall.equals(PsiUtil.skipParenthesizedExprDown(thenExpression)) && elseExpression != null)
			{
				final PsiType elseType = PsiResolveHelper.ourGraphGuard.doPreventingRecursion(parent, true,
						new Computable<PsiType>()
				{
					@Override
					public PsiType compute()
				...
				if(elseType != null)
				{
					pair = inferTypeForMethodTypeParameterInner(typeParameter, paramTypes, new PsiType[]{elseType},
							substitutor, null, policy);
				}
			}
		}
	...
	private static final ProcessCandidateParameterTypeInferencePolicy GRAPH_INFERENCE_POLICY = new
			...
			@NotNull final PsiCallExpression parentCall)
	{
		...
		return PsiResolveHelper.ourGraphGuard.doPreventingRecursion(methodCall, true, new Computable<Pair<PsiType,
				ConstraintType>>()
		{
			@Override
			public Pair<PsiType, ConstraintType> compute()
			{
				return GRAPH_INFERENCE_POLICY.inferTypeConstraintFromCallContext(methodCall, argumentList, parentCall,
						typeParameter);
			}
		});
	}
